...
describe('FileDiffComponent', () => {
  it('allows lines of a hunk to be selected, clearing the selection on the other hunks', async () => {
    ...
    const component = new FileDiffComponent({fileDiff})
    const element = component.element

    var linesToSelect = hunk1.getLines().slice(1, 3)
    component.componentForHunk(hunk1).didSelectLines(new Set(linesToSelect))
    await etch.getScheduler().getNextUpdatePromise()
    assert.deepEqual(Array.from(component.componentForHunk(hunk1).selectedLines), linesToSelect)
    assert.deepEqual(Array.from(component.componentForHunk(hunk2).selectedLines), [])
    assert(component.componentForHunk(hunk1).isSelected)
    assert(!component.componentForHunk(hunk2).isSelected)

    var linesToSelect = hunk2.getLines().slice(0, 1)
    component.componentForHunk(hunk2).didSelectLines(new Set(linesToSelect))
    await etch.getScheduler().getNextUpdatePromise()
    assert.deepEqual(Array.from(component.componentForHunk(hunk1).selectedLines), [])
    assert.deepEqual(Array.from(component.componentForHunk(hunk2).selectedLines), linesToSelect)
    assert(!component.componentForHunk(hunk1).isSelected)
    assert(component.componentForHunk(hunk2).isSelected)
  })
})
