...
describe('integration: package initialization', function() {
  ...
  it('reveals the tabs on first run when the welcome package has been dismissed', async function() {
    context = await setup({
      initConfigDir: configDirPath =>
        fs.remove(path.join(configDirPath, 'github.cson')),
    ...
    await assert.async.includeMembers(getPaneItemURIs(), [
      GitTabItem.buildURI(),
      GitHubTabItem.buildURI(),
    ]);
  ...
  it('respects serialized state on existing projects after the first run', async function() {
    ...
    const paneItemURIs = context.atomEnv.workspace
      .getPaneItems()
      .map(i => i.getURI());
  ...
  it('honors firstRun from legacy serialized state', async function() {
    ...
    const getPaneItemURIs = ctx =>
      ctx.atomEnv.workspace.getPaneItems().map(i => i.getURI());
    ...
    await assert.async.includeMembers(getPaneItemURIs(prevContext), [
      GitTabItem.buildURI(),
      GitHubTabItem.buildURI(),
    ]);

    const prevWorkspace = prevContext.atomEnv.workspace;
    const item = prevWorkspace
      .getPaneItems()
      .find(i => i.getURI() === GitTabItem.buildURI());
