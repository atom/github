...
describe('BranchSet', function() {
  ...
  describe('getPullTargets() and getPushSources()', function() {
    ...
    beforeEach(function() {
      ...
      boo = new Branch(
        ...
        Branch.createRemoteTracking(
          'upstream/boo',
          'upstream',
          'refs/heads/fetch-from-here',
        ),
        Branch.createRemoteTracking(
          'origin/boo',
          'origin',
          'refs/heads/push-to-here',
        ),
      );
      bs.add(boo);
      ...
      sharedUpPush = new Branch(
        'shared/up/push',
        Branch.createRemoteTracking(
          'upstream/shared/up/push',
          'upstream',
          'refs/heads/shared/up',
        ),
        Branch.createRemoteTracking(
          'origin/shared/up/push',
          'origin',
          'refs/heads/shared/push',
        ),
      );
      bs.add(sharedUpPush);
    ...
    it('locates multiple branches that fetch from the same ref', function() {
      assert.sameMembers(
        bs.getPullTargets('upstream', 'refs/heads/shared/up'),
        [sharedUpPush, sharedUp],
      );
    });

    it('locates branches that push to a remote ref', function() {
      assert.deepEqual(bs.getPushSources('origin', 'refs/heads/push-to-here'), [
        boo,
      ]);
    });
    ...
    it('locates multiple branches that push to the same ref', function() {
      assert.sameMembers(
        bs.getPushSources('origin', 'refs/heads/shared/push'),
        [sharedUpPush, sharedPush],
      );
    });
  });
});
