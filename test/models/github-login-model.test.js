...
describe('GithubLoginModel', function() {
  [null, KeytarStrategy, SecurityBinaryStrategy, InMemoryStrategy].forEach(
    function(Strategy) {
      describe((Strategy && Strategy.name) || 'default strategy', function() {
        ...
        if (process.env.CI_PROVIDER === 'VSTS') {
          return;
        }
        ...
        it('manages passwords', async function() {
          if (!Strategy || (await Strategy.isValid())) {
            ...
            assert.equal(
              await loginModel.getToken('test-account'),
              UNAUTHENTICATED,
            );
          } else {
            ...
            console.warn(
              `Skipping tests for ${
                Strategy.name
              } as they are not supported on this platform (or maybe your Atom is unsigned?)`,
            );
          }
        });
      });
  ...
  describe('required OAuth scopes', function() {
    ...
    it('returns INSUFFICIENT if scopes are present', async function() {
      ...
      assert.strictEqual(
        await loginModel.getToken('https://api.github.com'),
        INSUFFICIENT,
      );
    });
    ...
    it('returns the token if at least the required scopes are present', async function() {
      sinon
        ...
        .resolves(['repo', 'read:org', 'user:email', 'extra']);
    ...
    it('caches checked tokens', async function() {
      sinon
        .stub(loginModel, 'getScopes')
        .resolves(['repo', 'read:org', 'user:email']);
    ...
    it('does not cache network errors', async function() {
      sinon
        .stub(loginModel, 'getScopes')
        .rejects(new Error('You unplugged your ethernet cable'));
