...
describe('Patch', function() {
  ...
  describe('stage patch generation', function() {
    ...
    it('creates a patch that applies selected lines from only the first hunk', function() {
      ...
      const stagePatch = patch.buildStagePatchForLines(
        originalBuffer,
        stageLayeredBuffer,
        new Set([2, 3, 4, 5]),
      );
      ...
      assert.strictEqual(
        stageLayeredBuffer.buffer.getText(),
        expectedBufferText,
      );
    ...
    it('marks ranges for each change region on the correct marker layer', function() {
      ...
      assert.deepEqual(layerRanges, {
        hunk: [[[0, 0], [4, 4]], [[5, 0], [14, 4]], [[15, 0], [17, 26]]],
        unchanged: [
          [[0, 0], [0, 4]],
          [[2, 0], [2, 4]],
          [[4, 0], [4, 4]],
        ...
        addition: [[[3, 0], [3, 4]], [[13, 0], [13, 4]], [[16, 0], [16, 4]]],
        deletion: [[[1, 0], [1, 4]], [[11, 0], [12, 4]]],
        noNewline: [[[17, 0], [17, 26]]],
      });
    });
    ...
    it('returns a modification patch if original patch is a deletion', function() {
      ...
      const hunks = [
        new Hunk({
          ...
          sectionHeading: 'zero',
          marker: markRange(layers.hunk, 0, 5),
          regions: [new Deletion(markRange(layers.deletion, 0, 5))],
        }),
      ];
      ...
      const stagedPatch = patch.buildStagePatchForLines(
        buffer,
        stageLayeredBuffer,
        new Set([1, 3, 4]),
      );
    ...
    it('returns an deletion when staging an entire deletion patch', function() {
      ...
      const stagePatch0 = patch.buildStagePatchForLines(
        buffer,
        stageLayeredBuffer,
        new Set([0, 1, 2]),
      );
    ...
    it('returns a nullPatch as a nullPatch', function() {
      const nullPatch = Patch.createNull();
      assert.strictEqual(
        nullPatch.buildStagePatchForLines(new Set([1, 2, 3])),
        nullPatch,
      );
    });
  });
  ...
  describe('unstage patch generation', function() {
    ...
    it('marks ranges for each change region on the correct marker layer', function() {
      const {patch, buffer: originalBuffer} = buildPatchFixture();
      patch.buildUnstagePatchForLines(
        originalBuffer,
        unstageLayeredBuffer,
        new Set([1, 4, 5, 16, 17, 20, 25]),
      );
    ...
    it('returns a nullPatch as a nullPatch', function() {
      const nullPatch = Patch.createNull();
      assert.strictEqual(
        nullPatch.buildUnstagePatchForLines(new Set([1, 2, 3])),
        nullPatch,
      );
    });
  });
  ...
  describe('getFirstChangeRange', function() {
    it('accesses the range of the first change from the first hunk', function() {
      const {patch} = buildPatchFixture();
      assert.deepEqual(patch.getFirstChangeRange().serialize(), [
        [1, 0],
        [1, Infinity],
      ]);
    });
  ...
  it('has a stubbed nullPatch counterpart', function() {
    ...
    assert.deepEqual(nullPatch.getFirstChangeRange().serialize(), [
      [0, 0],
      [0, 0],
    ]);
