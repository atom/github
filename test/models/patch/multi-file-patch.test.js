...
describe('MultiFilePatch', function() {
  ...
  it('has an accessor for its file patches', function() {
    const {multiFilePatch} = multiFilePatchBuilder()
      ...
      .addFilePatch(filePatch =>
        filePatch.setOldFile(file => file.path('file-1.txt')),
      )
      .build();
  ...
  describe('anyHaveTypechange()', function() {
    it('detects when at least one patch contains a symlink change', function() {
      const {multiFilePatch: yes} = multiFilePatchBuilder()
        ...
        .addFilePatch(filePatch => {
          filePatch.setOldFile(file => file.path('file-0.txt'));
          filePatch.setNewFile(file =>
            file.path('file-0.txt').symlinkTo('somewhere.txt'),
          );
        })
        .build();
  ...
  it('creates a set of all unique paths referenced by patches', function() {
    ...
    assert.sameMembers(Array.from(multiFilePatch.getPathSet()), [
      'file-0-before.txt',
      'file-0-after.txt',
      'file-1.txt',
      'file-2.txt',
  ...
  it('adopts a buffer from a previous patch', function() {
    const {
      multiFilePatch: lastMultiPatch,
      buffer: lastBuffer,
      layers: lastLayers,
    } = multiFilePatchBuilder()
      .addFilePatch(fp => {
    ...
    assert.strictEqual(
      nextMultiPatch.getUnchangedLayer(),
      lastLayers.unchanged,
    );
    ...
    assert.strictEqual(
      nextMultiPatch.getNoNewlineLayer(),
      lastLayers.noNewline,
    );
    ...
    const assertMarkedLayerRanges = (layer, ranges) => {
      assert.deepEqual(
        layer.getMarkers().map(m => m.getRange().serialize()),
        ranges,
      );
    };
    ...
    assertMarkedLayerRanges(lastLayers.noNewline, [[[13, 0], [13, 26]]]);

    assert.strictEqual(
      nextMultiPatch.getBufferRowForDiffPosition('B0.txt', 1),
      0,
    );
    assert.strictEqual(
      nextMultiPatch.getBufferRowForDiffPosition('B2.txt', 5),
      12,
    );
  });
  ...
  describe('next selection range derivation', function() {
    ...
    it('preserves the numeric index of the highest selected change row', function() {
      ...
      assert.deepEqual(nextSelectionRange.serialize(), [
        [11, 0],
        [11, Infinity],
      ]);
    });
  ...
  describe('diff position translation', function() {
    it('offsets rows in the first hunk by the first hunk header', function() {
      ...
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('file.txt', 1),
        0,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('file.txt', 7),
        6,
      );
    });
    ...
    it('offsets rows by the number of hunks before the diff row', function() {
      ...
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('file.txt', 11),
        9,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('file.txt', 13),
        10,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('file.txt', 15),
        12,
      );
    });
    ...
    it('resets the offset at the start of each file patch', function() {
      ...
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('0.txt', 1),
        0,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('0.txt', 4),
        3,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('0.txt', 6),
        4,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('0.txt', 10),
        8,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('0.txt', 12),
        9,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('0.txt', 14),
        11,
      );

      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('1.txt', 1),
        12,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('1.txt', 3),
        14,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('1.txt', 5),
        15,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('1.txt', 9),
        19,
      );

      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('2.txt', 1),
        20,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('2.txt', 6),
        25,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('2.txt', 8),
        26,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('2.txt', 11),
        29,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('2.txt', 13),
        30,
      );
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('2.txt', 15),
        32,
      );
    });
  });
