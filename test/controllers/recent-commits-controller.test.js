...
describe('RecentCommitsController', function() {
  ...
  it('passes recent commits to the RecentCommitsView', function() {
    ...
    assert.deepEqual(
      wrapper.find('RecentCommitsView').prop('commits'),
      commits,
    );
  });
  ...
  describe('openCommit({sha, preserveFocus})', function() {
    it('opens a commit detail item', async function() {
      ...
      const wrapper = shallow(app);
      await wrapper.find('RecentCommitsView').prop('openCommit')({
        sha: 'asdf1234',
        preserveFocus: false,
      });
    ...
    it('preserves keyboard focus within the RecentCommitsView when requested', async function() {
      ...
      const focusSpy = sinon
        .stub(wrapper.find('RecentCommitsView').instance(), 'setFocus')
        .returns(true);
    ...
    it('records an event', async function() {
      ...
      await wrapper
        .instance()
        .openCommit({sha: 'asdf1234', preserveFocus: true});
      assert.isTrue(
  ...
  describe('commit navigation', function() {
    ...
    describe('selectNextCommit', function() {
      it('selects the first commit if there is no selection', async function() {
        assert.strictEqual(
          wrapper.find('RecentCommitsView').prop('selectedCommitSha'),
          '',
        );
  ...
  describe('focus management', function() {
    it('forwards focus management methods to its view', async function() {
      ...
      const rememberFocusSpy = sinon.spy(
        wrapper.find('RecentCommitsView').instance(),
        'getFocus',
      );
      const advanceFocusSpy = sinon.spy(
        wrapper.find('RecentCommitsView').instance(),
        'advanceFocusFrom',
      );
      const retreatFocusSpy = sinon.spy(
        wrapper.find('RecentCommitsView').instance(),
        'retreatFocusFrom',
      );
      ...
      assert.isTrue(
        setFocusSpy.calledWith(RecentCommitsController.focus.RECENT_COMMIT),
      );
      ...
      await wrapper
        .instance()
        .advanceFocusFrom(RecentCommitsController.focus.RECENT_COMMIT);
      assert.isTrue(
        advanceFocusSpy.calledWith(RecentCommitsController.focus.RECENT_COMMIT),
      );

      await wrapper
        .instance()
        .retreatFocusFrom(RecentCommitsController.focus.RECENT_COMMIT);
      assert.isTrue(
        retreatFocusSpy.calledWith(RecentCommitsController.focus.RECENT_COMMIT),
      );
    });

    it('selects the first commit when focus enters the component', async function() {
      const commits = ['0', '1', '2'].map(s =>
        commitBuilder()
          .sha(s)
          .build(),
      );
      const wrapper = mount(
        React.cloneElement(app, {commits, selectedCommitSha: ''}),
      );
      ...
      assert.isTrue(
        wrapper
          .instance()
          .setFocus(RecentCommitsController.focus.RECENT_COMMIT),
      );
  ...
  describe('workspace tracking', function() {
    ...
    it('updates the selected sha when its CommitDetailItem is activated', async function() {
      ...
      await atomEnv.workspace.open(
        CommitDetailItem.buildURI(workdirPath, 'abcdef'),
      );
      assert.strictEqual(
        wrapper.find('RecentCommitsView').prop('selectedCommitSha'),
        'abcdef',
