...
describe('PullRequestReviewsController', function() {
  function buildApp(opts, overrideProps = {}) {
    ...
    const reviews = {
      ...
      pageInfo: {
        ...
        hasNextPage:
          o.reviewStartCursor + o.reviewSpecs.length < o.reviewItemTotal,
  ...
  it('renders a PullRequestReviewCommentsContainer for every review', function() {
    ...
    const containers = wrapper.find(
      'ForwardRef(Relay(BarePullRequestReviewCommentsContainer))',
    );
  ...
  it('renders a PullRequestReviewCommentsView and passes props through', function() {
    ...
    assert.strictEqual(
      wrapper.instance().props.passThroughProp,
      view.prop('passThroughProp'),
    );
  });
  ...
  describe('collectComments', function() {
    ...
    it('calls groupCommentsByThread if there are no more reviews or comments to be fetched', function() {
      const wrapper = shallow(buildApp());
      const groupCommentsStub = sinon.stub(
        wrapper.instance(),
  ...
  describe('grouping and ordering comments', function() {
    it('groups the comments into threads based on replyId', function() {
      ...
      const review2 = reviewBuilder()
        ...
        .addComment(c =>
          c
            ...
            .body('reply to OG comment'),
        )
        .addComment(c =>
          c
            .id(singleCommentId)
            .path('file0.txt')
            .body('I am single and free'),
        )
        .build();
      ...
      wrapper.instance().reviewsById.set(review1.id, {
        submittedAt: review1.submittedAt,
        comments: review1.comments,
        fetchingMoreComments: false,
      });

      wrapper.instance().collectComments({
        reviewId: review2.id,
    ...
    it('comments are ordered based on the order in which their reviews were submitted', function() {
      ...
      const review2 = reviewBuilder()
        ...
        .addComment(c =>
          c
            ...
            .path('file0.txt')
            .replyTo(originalCommentId)
            .body('first reply to OG comment'),
        )
        .build();
      ...
      const review3 = reviewBuilder()
        ...
        .addComment(c =>
          c
            ...
            .path('file0.txt')
            .replyTo(originalCommentId)
            .body('second reply to OG comment'),
        )
        .build();

      const wrapper = shallow(
        buildApp({reviewSpecs: [review1, review2, review3]}),
      );
      ...
      wrapper.instance().reviewsById.set(review2.id, {
        submittedAt: review2.submittedAt,
        comments: review2.comments,
        fetchingMoreComments: false,
      });
      ...
      wrapper.instance().collectComments({
        reviewId: review3.id,
        submittedAt: review3.submittedAt,
        comments: review3.comments,
        fetchingMoreComments: false,
      });
      const threadedComments = wrapper.instance().state[originalCommentId];
      ...
      assert.strictEqual(
        threadedComments[2].body,
        'second reply to OG comment',
      );
    });
    ...
    it('comments with a replyTo id that does not point to an existing comment are threaded separately', function() {
      ...
      const review = reviewBuilder()
        ...
        .addComment(c =>
          c
            .id(replyToOutdatedCommentId)
            .path('file0.txt')
            .replyTo(outdatedCommentId)
            .body('reply to outdated comment'),
        )
        .build();
  ...
  describe('compareReviewsByDate', function() {
    let wrapper;
    const reviewA = reviewBuilder()
      .submittedAt('2018-12-28T20:40:55Z')
      .build();
    const reviewB = reviewBuilder()
      .submittedAt('2018-12-27T20:40:55Z')
      .build();
    ...
    it('returns 1 if reviewA is older', function() {
      assert.strictEqual(
        wrapper.instance().compareReviewsByDate(reviewA, reviewB),
        1,
      );
    });
    it('return -1 if reviewB is older', function() {
      assert.strictEqual(
        wrapper.instance().compareReviewsByDate(reviewB, reviewA),
        -1,
      );
    });
    it('returns 0 if reviews have the same date', function() {
      assert.strictEqual(
        wrapper.instance().compareReviewsByDate(reviewA, reviewA),
        0,
      );
    });
  });
