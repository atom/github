...
describe('IssueishDetailController', function() {
  ...
  function buildApp(opts, overrideProps = {}) {
    const props = issueishDetailControllerProps(opts, {
  ...
  it('updates the pane title for a pull request on mount', function() {
    ...
    assert.isTrue(
      onTitleChange.calledWith('PR: ownername/reponame#12 — the title'),
    );
  });
  ...
  it('updates the pane title for an issue on mount', function() {
    ...
    assert.isTrue(
      onTitleChange.calledWith('Issue: ownername/reponame#34 — the title'),
    );
  });
  ...
  it('leaves the title alone and renders a message if no repository was found', function() {
    const onTitleChange = sinon.stub();
    const wrapper = shallow(
      buildApp({}, {onTitleChange, repository: null, issueishNumber: 123}),
    );
  ...
  it('leaves the title alone and renders a message if no issueish was found', function() {
    ...
    const wrapper = shallow(
      buildApp(
        {omitIssueData: true, omitPullRequestData: true},
        {onTitleChange, issueishNumber: 123},
      ),
    );
    assert.isFalse(onTitleChange.called);
    assert.match(wrapper.find('div').text(), /#123 not found/);
  ...
  describe('checkoutOp', function() {
    ...
    it('is disabled if the current branch already corresponds to the pull request', function() {
      const upstream = Branch.createRemoteTracking(
    ...
    it('creates a new remote, fetches a PR branch, and checks it out into a new local branch', async function() {
      ...
      const addRemote = sinon
        .stub()
        .resolves(new Remote('ccc', 'git@github.com:ccc/ddd.git'));
      ...
      assert.isTrue(
        fetch.calledWith('refs/heads/feature', {remoteName: 'ccc'}),
      );
      assert.isTrue(
        checkout.calledWith('pr-456/ccc/feature', {
          createNew: true,
          track: true,
    ...
    it('fetches a PR branch from an existing remote and checks it out into a new local branch', async function() {
      ...
      assert.isTrue(
        fetch.calledWith('refs/heads/clever-name', {remoteName: 'existing'}),
      );
      assert.isTrue(
        checkout.calledWith('pr-789/ccc/clever-name', {
          createNew: true,
          track: true,
    ...
    it('checks out an existing local branch that corresponds to the pull request', async function() {
      const currentUpstream = Branch.createRemoteTracking(
        'remotes/origin/current',
        'origin',
        'refs/heads/current',
      );
      ...
      assert.isTrue(
        pull.calledWith('refs/heads/yes', {
          remoteName: 'upstream',
          ffOnly: true,
        }),
      );
    ...
    it('squelches git errors', async function() {
      const addRemote = sinon
        .stub()
        .rejects(new GitError('handled by the pipeline'));
    ...
    it('propagates non-git errors', async function() {
      const addRemote = sinon
        .stub()
        .rejects(new Error('not handled by the pipeline'));
      const wrapper = shallow(buildApp({}, {addRemote}));
    ...
    it('reports an event', async function() {
      ...
      assert.isTrue(
        reporterProxy.addEvent.calledWith('open-commit-in-pane', {
          package: 'github',
          from: 'BareIssueishDetailController',
        }),
      );
    });
  });
