...
const defaultProps = {
  ...
  url:
    'https://github.com/atom/github/pull/1684/commits/bad1deaea3d816383721478fc631b5edd0c2b370',
};
...
describe('PrCommitView', function() {
  ...
  it('renders the commit view for commits without message body', function() {
    ...
    assert.deepEqual(
      wrapper.find('.github-PrCommitView-title').text(),
      defaultProps.messageHeadline,
    );
    ...
    const expectedMetaText = `${
      defaultProps.committer.name
    } committed ${humanizedTimeSince}`;
    assert.deepEqual(
      wrapper.find('.github-PrCommitView-metaText').text(),
      expectedMetaText,
    );
  ...
  it('toggles the commit message body when button is clicked', function() {
    ...
    assert.deepEqual(
      wrapper.find('.github-PrCommitView-moreButton').text(),
      'hide more...',
    );
  ...
  describe('if PR is checked out', function() {
    it('shows message headlines as clickable', function() {
      const wrapper = shallow(buildApp({}));
      assert.isTrue(
        wrapper.find('.github-PrCommitView-messageHeadline').is('button'),
      );
    });
    ...
    it('opens a commit with the full sha when title is clicked', function() {
      ...
      wrapper
        .find('button.github-PrCommitView-messageHeadline')
        .at(0)
        .simulate('click');
  ...
  it('does not show message headlines as clickable if PR is not checked out', function() {
    ...
    assert.isTrue(
      wrapper.find('.github-PrCommitView-messageHeadline').is('span'),
    );
  });
});
