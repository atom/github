...
describe('PrStatusContextView', function() {
  function buildApp(opts = {}) {
    return (
      <BarePrStatusContextView context={createStatusContextResult(opts)} />
    );
  }
  ...
  it('renders the context name and description', function() {
    const wrapper = shallow(
      buildApp({context: 'the context', description: 'the description'}),
    );
    assert.match(
      wrapper.find('.github-PrStatuses-list-item-context').text(),
      /the context/,
    );
    assert.match(
      wrapper.find('.github-PrStatuses-list-item-context').text(),
