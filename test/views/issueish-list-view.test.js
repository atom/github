...
const allRed = new Issueish(
  createPullRequestResult({number: 3, states: ['FAILURE', 'ERROR', 'FAILURE']}),
);
const noStatus = new Issueish(
  createPullRequestResult({number: 4, states: null}),
);
...
describe('IssueishListView', function() {
  ...
  it('sets the accordion title to the search name', function() {
    ...
    assert.strictEqual(
      wrapper.find('Accordion').prop('leftTitle'),
      'the search name',
    );
  });
  ...
  describe('with empty results', function() {
    it('uses a custom EmptyComponent if one is provided', function() {
      const wrapper = mount(
        buildApp({isLoading: false, emptyComponent: CustomComponent}),
      );
  ...
  describe('with nonempty results', function() {
    ...
    it('renders a check if all status checks are successful', function() {
      const wrapper = mount(
        buildApp({isLoading: false, total: 1, issueishes: [allGreen]}),
      );
      assert.isTrue(
        wrapper
          .find('span.github-IssueishList-item--status')
          .hasClass('icon-check'),
      );
    });
    ...
    it('renders an x if all status checks have failed', function() {
      const wrapper = mount(
        buildApp({isLoading: false, total: 1, issueishes: [allRed]}),
      );
      assert.isTrue(
        wrapper
          .find('span.github-IssueishList-item--status')
    ...
    it('renders a donut chart if status checks are mixed', function() {
      const wrapper = mount(
        buildApp({isLoading: false, total: 1, issueishes: [mixed]}),
      );
    ...
    it('renders nothing with no status checks are present', function() {
      const wrapper = mount(
        buildApp({isLoading: false, total: 1, issueishes: [noStatus]}),
      );
      assert.isTrue(
        wrapper
          .find('span.github-IssueishList-item--status')
    ...
    it('calls its onIssueishClick handler when an item is clicked', function() {
      ...
      wrapper
        .find('.github-Accordion-listItem')
        .at(1)
        .simulate('click');
