...
describe('MultiFilePatchView', function() {
  ...
  it('undoes the last discard from the file header button', function() {
    ...
    wrapper
      .find('FilePatchHeaderView')
      .first()
      .prop('undoLastDiscard')();

    assert.lengthOf(filePatches.getFilePatches(), 1);
    const [filePatch] = filePatches.getFilePatches();
    assert.isTrue(
      undoLastDiscard.calledWith(filePatch, {eventSource: 'button'}),
    );
  });
  ...
  it('passes hasMultipleFileSelections to all file headers', function() {
    ...
    const wrapper = shallow(
      buildApp({multiFilePatch, hasMultipleFileSelections: true}),
    );

    assert.isTrue(
      wrapper
        .find('FilePatchHeaderView[relPath="0"]')
        .prop('hasMultipleFileSelections'),
    );
    assert.isTrue(
      wrapper
        .find('FilePatchHeaderView[relPath="1"]')
        .prop('hasMultipleFileSelections'),
    );
  ...
  it('renders a PullRequestsReviewsContainer if itemType is IssueishDetailItem', function() {
    ...
    assert.lengthOf(
      wrapper.find('ForwardRef(Relay(PullRequestReviewsController))'),
      1,
    );
  });
  ...
  describe('initial selection', function() {
    it('selects the origin with an empty FilePatch', function() {
      ...
      assert.deepEqual(
        editor.getSelectedBufferRanges().map(r => r.serialize()),
        [[[0, 0], [0, 0]]],
      );
    });
  ...
  it('selects the next full hunk when a new file patch arrives in hunk selection mode', function() {
    ...
    assert.sameMembers(Array.from(selectedRowsChanged.lastCall.args[0]), [
      6,
      7,
    ]);
  ...
  describe('executable mode changes', function() {
    it('does not render if the mode has not changed', function() {
      ...
      assert.isFalse(
        wrapper.find('FilePatchMetaView[title="Mode change"]').exists(),
      );
    });
    ...
    it('renders change details within a meta container', function() {
      ...
      const wrapper = mount(
        buildApp({multiFilePatch: mfp, stagingStatus: 'unstaged'}),
      );
      ...
      assert.strictEqual(
        details.text(),
        'File changed modefrom non executable 100644to executable 100755',
      );
    });
  ...
  describe('symlink changes', function() {
    it('does not render if the symlink status is unchanged', function() {
      ...
      assert.lengthOf(
        wrapper
          .find('FilePatchMetaView')
          .filterWhere(v => v.prop('title').startsWith('Symlink')),
        0,
      );
    });
      ...
      const mfp = filePatches.clone({
        filePatches: [
          fp.clone({
            oldFile: fp
              .getOldFile()
              .clone({mode: '120000', symlink: '/old.txt'}),
            newFile: fp
              .getNewFile()
              .clone({mode: '120000', symlink: '/new.txt'}),
          }),
        ],
      });
  ...
  describe('hunk headers', function() {
    it('renders one for each hunk', function() {
      ...
      assert.isTrue(
        wrapper
          .find('HunkHeaderView')
          .someWhere(h => h.prop('hunk') === hunks[0]),
      );
      assert.isTrue(
        wrapper
          .find('HunkHeaderView')
          .someWhere(h => h.prop('hunk') === hunks[1]),
      );
    });

    it('pluralizes the toggle and discard button labels', function() {
      const wrapper = shallow(
        buildApp({selectedRows: new Set([2]), selectionMode: 'line'}),
      );
      assert.strictEqual(
        wrapper
          .find('HunkHeaderView')
          .at(0)
          .prop('toggleSelectionLabel'),
        'Stage Selected Line',
      );
      assert.strictEqual(
        wrapper
          .find('HunkHeaderView')
          .at(0)
          .prop('discardSelectionLabel'),
        'Discard Selected Line',
      );
      assert.strictEqual(
        wrapper
          .find('HunkHeaderView')
      ...
      wrapper.setProps({
        selectedRows: new Set([1, 2, 3, 6, 7]),
        selectionMode: 'hunk',
      });
      assert.strictEqual(
        wrapper
    ...
    it('handles mousedown as a selection event', function() {
      ...
      wrapper
        .find('HunkHeaderView')
        .at(1)
        .prop('mouseDown')({button: 0}, mfp.getFilePatches()[0].getHunks()[1]);
  ...
  describe('custom gutters', function() {
    ...
    it('computes the old line number for a buffer row', function() {
      assert.strictEqual(
        instance.oldLineNumberLabel({bufferRow: 5, softWrapped: false}),
        '\u00a08',
      );
      assert.strictEqual(
        instance.oldLineNumberLabel({bufferRow: 6, softWrapped: false}),
        '\u00a0\u00a0',
      );
      assert.strictEqual(
        instance.oldLineNumberLabel({bufferRow: 6, softWrapped: true}),
        '\u00a0\u00a0',
      );
      assert.strictEqual(
        instance.oldLineNumberLabel({bufferRow: 7, softWrapped: false}),
        '\u00a09',
      );
      assert.strictEqual(
        instance.oldLineNumberLabel({bufferRow: 8, softWrapped: false}),
        '10',
      );
      assert.strictEqual(
        instance.oldLineNumberLabel({bufferRow: 8, softWrapped: true}),
        '\u00a0•',
      );

      assert.strictEqual(
        instance.oldLineNumberLabel({bufferRow: 999, softWrapped: false}),
        '\u00a0\u00a0',
      );
    });
    ...
    it('computes the new line number for a buffer row', function() {
      assert.strictEqual(
        instance.newLineNumberLabel({bufferRow: 5, softWrapped: false}),
        '\u00a09',
      );
      assert.strictEqual(
        instance.newLineNumberLabel({bufferRow: 6, softWrapped: false}),
        '10',
      );
      assert.strictEqual(
        instance.newLineNumberLabel({bufferRow: 6, softWrapped: true}),
        '\u00a0•',
      );
      assert.strictEqual(
        instance.newLineNumberLabel({bufferRow: 7, softWrapped: false}),
        '\u00a0\u00a0',
      );
      assert.strictEqual(
        instance.newLineNumberLabel({bufferRow: 7, softWrapped: true}),
        '\u00a0\u00a0',
      );
      assert.strictEqual(
        instance.newLineNumberLabel({bufferRow: 8, softWrapped: false}),
        '11',
      );

      assert.strictEqual(
        instance.newLineNumberLabel({bufferRow: 999, softWrapped: false}),
        '\u00a0\u00a0',
      );
    });
    ...
    it('renders diff region scope characters when the config option is enabled', function() {
      ...
      const assertLayerDecorated = layer => {
        const layerWrapper = wrapper
          .find('MarkerLayer')
          .filterWhere(each => each.prop('external') === layer);
        const decorations = layerWrapper.find(
          'Decoration[type="line-number"][gutterName="diff-icons"]',
        );
    ...
    if (process.platform !== 'win32') {
      it('ignores ctrl-clicks on non-Windows platforms', function() {
        instance.didMouseDownOnLineNumber({
          bufferRow: 2,
          domEvent: {button: 0, ctrlKey: true},
        });
        assert.deepEqual(
          editor.getSelectedBufferRanges().map(r => r.serialize()),
          [[[0, 0], [4, 4]]],
    ...
    describe('shift-click', function() {
      it('selects a range of lines', function() {
        ...
        instance.didMouseDownOnLineNumber({
          bufferRow: 4,
          domEvent: {shiftKey: true, button: 0},
        });
        instance.didMouseUp();
        assert.deepEqual(
          editor.getSelectedBufferRanges().map(r => r.serialize()),
    ...
    describe('ctrl- or meta-click', function() {
      ...
      it('deselects a line at the beginning of an existing selection', function() {
        instance.didMouseDownOnLineNumber({
          bufferRow: 2,
          domEvent: {metaKey: true, button: 0},
        });
        assert.deepEqual(
          editor.getSelectedBufferRanges().map(r => r.serialize()),
          [[[3, 0], [5, 4]]],
      ...
      it('bonus points: understands ranges that do not cleanly align with editor rows', function() {
        instance.handleSelectionEvent({metaKey: true, button: 0}, [
          [3, 1],
          [5, 2],
        ]);
        assert.deepEqual(
  ...
  describe('hunk lines', function() {
      ...
      const {multiFilePatch} = multiFilePatchBuilder()
        .addFilePatch(fp => {
          ...
          fp.addHunk(h => {
            h.oldRow(10);
            h.unchanged('0007')
              .deleted('0008', '0009', '0010')
              .unchanged('0011')
              .added('0012', '0013', '0014')
              .deleted('0015')
              .unchanged('0016')
              .noNewline();
          });
    ...
    it('decorates added lines', function() {
      ...
      const decorationSelector =
        'Decoration[type="line"][className="github-FilePatchView-line--added"]';
      const decoration = wrapper.find(decorationSelector);
      assert.isTrue(decoration.exists());

      const layer = wrapper
        .find('MarkerLayer')
        .filterWhere(each => each.find(decorationSelector).exists());
    ...
    it('decorates deleted lines', function() {
      ...
      const decorationSelector =
        'Decoration[type="line"][className="github-FilePatchView-line--deleted"]';
      const decoration = wrapper.find(decorationSelector);
      assert.isTrue(decoration.exists());
    ...
    it('decorates the nonewline line', function() {
      ...
      const decorationSelector =
        'Decoration[type="line"][className="github-FilePatchView-line--nonewline"]';
      const decoration = wrapper.find(decorationSelector);
      assert.isTrue(decoration.exists());
  ...
  describe('editor selection change notification', function() {
    ...
    it('notifies a callback when cursors span multiple files', function() {
      ...
      selectedRowsChanged.resetHistory();
      editor.setSelectedBufferRanges([[[5, 0], [5, 0]], [[16, 0], [16, 0]]]);
    ...
    it('renders an empty patch message', function() {
      ...
      assert.isTrue(
        wrapper
          ...
          .hasClass('github-FilePatchView--blank'),
      );
  ...
  describe('registers Atom commands', function() {
    it('toggles all mode changes', function() {
      ...
      const editor = wrapper
        ...
        .instance()
        .getModel();
      editor.setSelectedBufferRanges([[[5, 0], [5, 2]], [[37, 0], [42, 0]]]);
    ...
    it('undoes the last discard', function() {
      ...
      const [filePatch] = filePatches.getFilePatches();
      assert.isTrue(
        undoLastDiscard.calledWith(filePatch, {
    ...
    it('toggles the patch selection mode from line to hunk', function() {
      ...
      const wrapper = mount(
        buildApp({selectedRowsChanged, selectedRows, selectionMode: 'line'}),
      );
      const editor = wrapper
        .find('AtomTextEditor')
        .instance()
    ...
    it('toggles the patch selection mode from hunk to line', function() {
      ...
      const wrapper = mount(
        buildApp({selectedRowsChanged, selectedRows, selectionMode: 'hunk'}),
      );
      const editor = wrapper
        .find('AtomTextEditor')
        .instance()
    ...
    describe('jump to file', function() {
      ...
      it('preserves multiple cursors', function() {
        ...
        assert.isTrue(
          openFile.calledWith(fp, [[10, 2], [11, 2], [2, 3], [15, 0]], true),
        );
      });
      ...
      it('opens non-pending editors when opening multiple', function() {
        ...
        const editor = wrapper
          ...
          .instance()
          .getModel();
        editor.setSelectedBufferRanges([[[4, 0], [4, 0]], [[12, 0], [12, 0]]]);

        atomEnv.commands.dispatch(wrapper.getDOMNode(), 'github:jump-to-file');

        assert.isTrue(
          openFile.calledWith(mfp.getFilePatches()[0], [[11, 0]], false),
        );
        assert.isTrue(
          openFile.calledWith(mfp.getFilePatches()[1], [[10, 0]], false),
        );
      });
      ...
      describe('didOpenFile(selectedFilePatch)', function() {
        describe('when there is a selection in the selectedFilePatch', function() {
          it('opens the file and places the cursor corresponding to the selection', function() {
            ...
            wrapper
              .find('.github-FilePatchHeaderView-jumpToFileButton')
              .at(0)
              .simulate('click');

            assert.isTrue(
              openFile.calledWith(mfp.getFilePatches()[0], [[11, 0]], true),
            );
          });
        });
        ...
        describe('when there are no selections in the selectedFilePatch', function() {
          it('opens the file and places the cursor at the beginning of the first hunk', function() {
            ...
            const firstHunkBufferRow =
              secondFilePatch.getHunks()[0].getNewStartRow() - 1;

            assert.isTrue(
              openFile.calledWith(
                secondFilePatch,
                [[firstHunkBufferRow, 0]],
                true,
              ),
            );
          });
