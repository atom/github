...
describe('PullRequestDetailView', function() {
  function buildApp(opts, overrideProps = {}) {
    return (
      <BarePullRequestDetailView
        {...pullRequestDetailViewProps(opts, overrideProps)}
      />
    );
  }
  ...
  it('renders pull request information', function() {
    ...
    assert.strictEqual(
      link.prop('href'),
      'https://github.com/user0/repo/pull/100',
    );
    ...
    assert.isDefined(
      wrapper
        .find('ForwardRef(Relay(BarePrStatusesView))[displayType="check"]')
        .prop('pullRequest'),
    );
    ...
    assert.isTrue(
      wrapper
        ...
        .someWhere(n => n.prop('html') === '<code>stuff</code>'),
    );
    ...
    assert.isNotNull(
      wrapper
        .find('ForwardRef(Relay(BarePrStatusesView))[displayType="full"]')
        .prop('pullRequest'),
    );

    assert.strictEqual(
      wrapper.find('.github-IssueishDetailView-baseRefName').text(),
      baseRefName,
    );
    assert.strictEqual(
      wrapper.find('.github-IssueishDetailView-headRefName').text(),
      headRefName,
    );
  });
  ...
  it('renders tabs', function() {
    ...
    const wrapper = shallow(
      buildApp({pullRequestCommitCount, pullRequestChangedFileCount}),
    );
    ...
    assert.deepEqual(tab0Children[0].props, {
      icon: 'info',
    ...
    assert.deepEqual(tab1Children[0].props, {
      icon: 'checklist',
      className: 'github-IssueishDetailView-tab-icon',
    });
    ...
    assert.deepEqual(tab2Children[0].props, {
      icon: 'git-commit',
      className: 'github-IssueishDetailView-tab-icon',
    });
    ...
    assert.deepEqual(tab3Children[0].props, {
      icon: 'diff',
      className: 'github-IssueishDetailView-tab-icon',
    });
  ...
  it('tells its tabs when the pull request is currently checked out', function() {
    const wrapper = shallow(
      buildApp(
        {},
        {
          checkoutOp: new EnableableOperation(() => {}).disable(
            checkoutStates.CURRENT,
          ),
        },
      ),
    );
    ...
    assert.isTrue(
      wrapper.find('ForwardRef(Relay(IssueishTimelineView))').prop('onBranch'),
    );
    assert.isTrue(
      wrapper.find('ForwardRef(Relay(IssueishTimelineView))').prop('onBranch'),
    );
    assert.isTrue(
      wrapper.find('ForwardRef(Relay(PrCommitsView))').prop('onBranch'),
    );
  });
  ...
  it('tells its tabs when the pull request is not checked out', function() {
    ...
    wrapper.setProps({
      checkoutOp: checkoutOp.disable(checkoutStates.HIDDEN, 'message'),
    });
    assert.isFalse(
      wrapper.find('ForwardRef(Relay(IssueishTimelineView))').prop('onBranch'),
    );
    ...
    wrapper.setProps({
      checkoutOp: checkoutOp.disable(checkoutStates.DISABLED, 'message'),
    });
    assert.isFalse(
      wrapper.find('ForwardRef(Relay(IssueishTimelineView))').prop('onBranch'),
    );
    ...
    wrapper.setProps({
      checkoutOp: checkoutOp.disable(checkoutStates.BUSY, 'message'),
    });
    assert.isFalse(
      wrapper.find('ForwardRef(Relay(IssueishTimelineView))').prop('onBranch'),
    );
  ...
  it('renders pull request information for cross repository PR', function() {
    ...
    assert.strictEqual(
      wrapper.find('.github-IssueishDetailView-baseRefName').text(),
      `${ownerLogin}/${baseRefName}`,
    );
    assert.strictEqual(
      wrapper.find('.github-IssueishDetailView-headRefName').text(),
      `${authorLogin}/${headRefName}`,
    );
  });
  ...
  describe('Checkout button', function() {
    ...
    it('renders as disabled with hover text set to the disablement message', function() {
      const checkoutOp = new EnableableOperation(() => {}).disable(
  ...
  describe('metrics', function() {
    ...
    it('records clicking the link to view an issueish', function() {
      ...
      assert.isTrue(
        reporterProxy.addEvent.calledWith('open-pull-request-in-browser', {
          package: 'github',
          component: 'BarePullRequestDetailView',
        }),
      );
    });
    ...
    it('records opening the Overview tab', function() {
      ...
      assert.isTrue(
        reporterProxy.addEvent.calledWith('open-pr-tab-overview', {
          package: 'github',
          component: 'BarePullRequestDetailView',
        }),
      );
    ...
    it('records opening the Build Status tab', function() {
      ...
      assert.isTrue(
        reporterProxy.addEvent.calledWith('open-pr-tab-build-status', {
          package: 'github',
          component: 'BarePullRequestDetailView',
        }),
      );
    ...
    it('records opening the Commits tab', function() {
      ...
      assert.isTrue(
        reporterProxy.addEvent.calledWith('open-pr-tab-commits', {
          package: 'github',
          component: 'BarePullRequestDetailView',
        }),
      );
    ...
    it('records opening the "Files Changed" tab', function() {
      ...
      assert.isTrue(
        reporterProxy.addEvent.calledWith('open-pr-tab-files-changed', {
          package: 'github',
          component: 'BarePullRequestDetailView',
        }),
      );
