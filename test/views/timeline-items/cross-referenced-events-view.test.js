...
describe('CrossReferencedEventsView', function() {
  ...
  it('renders a child component for each grouped child event', function() {
    ...
    assert.lengthOf(
      wrapper.find('ForwardRef(Relay(BareCrossReferencedEventView))'),
      3,
    );
  });
  ...
  it('generates a summary based on a single pull request cross-reference', function() {
    ...
    assert.isTrue(
      wrapper
        .find('span')
        .someWhere(s =>
          /referenced this from a pull request in aaa\/bbb/.test(s.text()),
        ),
    );
  ...
  it('generates a summary based on a single issue cross-reference', function() {
    ...
    assert.isTrue(
      wrapper
        .find('span')
        .someWhere(s =>
          /referenced this from an issue in ccc\/ddd/.test(s.text()),
        ),
    );
  });
