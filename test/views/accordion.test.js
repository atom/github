...
describe('Accordion', function() {
  ...
  it('renders a left title', function() {
    ...
    assert.strictEqual(
      wrapper
        .find(
          'summary.github-Accordion-header span.github-Accordion--leftTitle',
        )
        .text(),
      'left',
    );
  ...
  it('renders a right title', function() {
    ...
    assert.strictEqual(
      wrapper
        .find(
          'summary.github-Accordion-header span.github-Accordion--rightTitle',
        )
        .text(),
      'right',
    );
  ...
  it('toggles expansion state on a header click', function() {
    ...
    assert.isFalse(
      wrapper.find('details.github-Accordion[open="false"]').exists(),
    );
  ...
  describe('while loading', function() {
    it('defaults to rendering no children', function() {
      const wrapper = shallow(
        buildApp({isLoading: true, children: WrongChild}),
      );
  ...
  describe('with results', function() {
    it('renders its child render prop with each', function() {
      ...
      for (const i of results) {
        assert.isTrue(
          wrapper.find('CustomChild').someWhere(c => c.prop('item') === i),
        );
      }
    });
