...
describe('IssueishTimelineView', function() {
  ...
  it('separates timeline issues by typename and renders a grouped child component for each', function() {
    ...
    const commentGroup0 = wrapper.find('Grouped(ForwardRef(Relay(BareIssueCommentView)))').filterWhere(c => c.prop('nodes').length === 1);
    assert.deepEqual(commentGroup0.prop('nodes').map(n => n.id), [2]);

    const mergedGroup = wrapper.find('Grouped(ForwardRef(Relay(BareMergedEventView)))').filterWhere(c => c.prop('nodes').length === 1);
    ...
    const commentGroup1 = wrapper.find('Grouped(ForwardRef(Relay(BareIssueCommentView)))').filterWhere(c => c.prop('nodes').length === 2);
