...
import IssueishTimelineView, {
  collectionRenderer,
} from '../../lib/views/issueish-timeline-view';
...
describe('IssueishTimelineView', function() {
  function buildApp(opts, overloadProps = {}) {
    ...
    const timeline = {
      ...
      pageInfo: {
        ...
        endCursor: `result${o.timelineStartCursor +
          o.timelineItemSpecs.length}`,
        hasNextPage:
          o.timelineStartCursor + o.timelineItemSpecs.length <
          o.timelineItemTotal,
  ...
  it('separates timeline issues by typename and renders a grouped child component for each', function() {
    ...
    const commitGroup0 = wrapper
      .find('ForwardRef(Relay(BareCommitsView))')
      .filterWhere(c => c.prop('nodes').length === 2);
    assert.deepEqual(commitGroup0.prop('nodes').map(n => n.id), [0, 1]);

    const commentGroup0 = wrapper
      .find('Grouped(Relay(BareIssueCommentView))')
      .filterWhere(c => c.prop('nodes').length === 1);
    assert.deepEqual(commentGroup0.prop('nodes').map(n => n.id), [2]);

    const mergedGroup = wrapper
      .find('Grouped(Relay(BareMergedEventView))')
      .filterWhere(c => c.prop('nodes').length === 1);
    assert.deepEqual(mergedGroup.prop('nodes').map(n => n.id), [3]);

    const commitGroup1 = wrapper
      .find('ForwardRef(Relay(BareCommitsView))')
      .filterWhere(c => c.prop('nodes').length === 4);
    assert.deepEqual(commitGroup1.prop('nodes').map(n => n.id), [4, 5, 6, 7]);

    const commentGroup1 = wrapper
      .find('Grouped(Relay(BareIssueCommentView))')
      .filterWhere(c => c.prop('nodes').length === 2);
  ...
  describe('collectionRenderer', function() {
    ...
    it('renders a child component for each node', function() {
      ...
      assert.isTrue(
        wrapper
          .find('Item')
          .everyWhere(i => i.prop('issueish') === props.issueish),
      );
      assert.isTrue(
        wrapper
          .find('Item')
          .everyWhere(
            i => i.prop('switchToIssueish') === props.switchToIssueish,
          ),
      );
      assert.deepEqual(wrapper.find('Item').map(i => i.prop('item')), [
        1,
        2,
        3,
      ]);
    ...
    it('translates the static getFragment call', function() {
      ...
      assert.strictEqual(
        Component.getFragment('something'),
        'item fragment for something',
      );
      assert.strictEqual(
        Component.getFragment('nodes'),
        'item fragment for item',
      );
    });
  });
});
