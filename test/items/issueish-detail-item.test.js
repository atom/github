...
describe('IssueishDetailItem', function() {
  ...
  describe('issueish switching', function() {
    ...
    beforeEach(async function() {
      ...
      atomGithubRepo = workdirContextPool
        .add(atomGithubWorkdir)
        .getRepository();
      await atomGithubRepo.getLoadPromise();
      await atomGithubRepo.addRemote(
        'upstream',
    ...
    it('switches to a different issueish', async function() {
      ...
      await atomEnv.workspace.open(
        IssueishDetailItem.buildURI('host.com', 'me', 'original', 1, __dirname),
      );
      ...
      await wrapper.find('IssueishDetailContainer').prop('switchToIssueish')(
        'you',
        'switched',
        2,
      );
    ...
    it('changes the active repository when its issueish changes', async function() {
      ...
      wrapper.update();
      assert.strictEqual(
        wrapper.find('IssueishDetailContainer').prop('repository'),
        atomGithubRepo,
      );
    ...
    it('reverts to an absent repository when no matching repository is found', async function() {
      ...
      const wrapper = mount(buildApp({workdirContextPool}));
      await atomEnv.workspace.open(
        IssueishDetailItem.buildURI('github.com', 'atom', 'atom', 5, workdir),
      );
    ...
    it('aborts a repository swap when pre-empted', async function() {
      ...
      const {resolve: resolve0, started: started0} = deferSetState(
        item.getRealItem(),
      );
      const swap0 = wrapper
        .find('IssueishDetailContainer')
        .prop('switchToIssueish')('atom', 'github', 100);
      await started0;

      const {resolve: resolve1} = deferSetState(item.getRealItem());
      const swap1 = wrapper
        .find('IssueishDetailContainer')
        .prop('switchToIssueish')('atom', 'atom', 200);
    ...
    it('reverts to an absent repository when multiple potential repositories are found', async function() {
      ...
      assert.strictEqual(
        wrapper.find('IssueishDetailContainer').prop('owner'),
        'atom',
      );
      assert.strictEqual(
        wrapper.find('IssueishDetailContainer').prop('repo'),
        'atom',
      );
      assert.strictEqual(
        wrapper.find('IssueishDetailContainer').prop('issueishNumber'),
        100,
      );
      assert.isTrue(
        wrapper
  ...
  it('reconstitutes its original URI', async function() {
    ...
    wrapper
      .update()
      .find('IssueishDetailContainer')
      .prop('switchToIssueish')('you', 'switched', 2);
  ...
  it('broadcasts title changes', async function() {
    const wrapper = mount(buildApp({}));
    const item = await atomEnv.workspace.open(
      IssueishDetailItem.buildURI('host.com', 'user', 'repo', 1, __dirname),
    );
