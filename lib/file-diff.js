...
export default class FileDiff {
  ...
  openDiff ({pending}) {
    return atom.workspace.open(Common.DiffURI + this.getNewPathName(), {pending})
  }
  ...
  async createHunksFromDiff (diff, isStaged) {
    if (!diff) return []
  ...
  async fromGitUtilsObject ({diff, stagedDiff, unstagedDiff}) {
    if (!diff) return
