...
export default class FileDiff {
  ...
  constructor (options: {oldPathName?: string, newPathName?: string, changeStatus?: string} = {}) {
  ...
  setChangeStatus (changeStatus: string) {
    switch (changeStatus) {
      case 'added':
        this.added = true
        this.renamed = false
  ...
  openDiff (): Promise<void> {
    ...
    if (pathName) {
      return atom.workspace.open(DiffURI + pathName, {pending: true})
    } else {
  ...
  // $FlowFixMe: diff should be NodeGitDiff.
  async createHunksFromDiff (diff: NodeGitDiff, isStaged: boolean): Promise<Array<DiffHunk>> {
    if (!diff) return []
  ...
  // $FlowFixMe: diff should be NodeGitDiff.
  async fromGitUtilsObject ({diff, stagedDiff, unstagedDiff}: {diff: NodeGitDiff, stagedDiff: NodeGitDiff, unstagedDiff: NodeGitDiff}) {
    if (!diff) return
