...
import type FileListViewModel from '../file-list/file-list-view-model'
import type {SelectionMode} from './diff-selection' // eslint-disable-line no-duplicate-imports
  ...
  getSelectedLines (): Array<HunkLine> {
    ...
    for (let diffHunkIndex in selectedHunkIndices) {
      ...
      if (selectedLineIndices instanceof Set) {
        ...
        const stagableLines = hunkLines.filter(line => line.isStagable())
        lines.push(...stagableLines)
      }
    }
