...
import {DiffURI, ifNotNull} from '../../common'

import type {ConvenientPatch, StatusFile} from 'nodegit'
import type {StageStatus} from '../diff-hunk/diff-hunk' // eslint-disable-line no-duplicate-imports
...
export default class FileDiff {
  ...
  async createHunksFromDiff (diff: ConvenientPatch, isStaged: boolean): Promise<Array<DiffHunk>> {
    ...
    for (const hunk of (await diff.hunks())) {
      ...
      // TODO [mkt]: Can this be done in parallel?
      await diffHunk.fromGitObject({hunk, isStaged, diff: this}) // eslint-disable-line babel/no-await-in-loop
      hunks.push(diffHunk)
    }
    return hunks
  }

  async fromGitObject ({diff, stagedDiff, unstagedDiff, statusFile}: {diff: ConvenientPatch, stagedDiff: ConvenientPatch, unstagedDiff: ConvenientPatch, statusFile: StatusFile}): Promise<void> {
    if (!diff) return
