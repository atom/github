...
import FileDiff from './file-diff/file-diff'
...
export default class GitStore {
  ...
  async loadFromGit (): Promise<void> {
    ...
    for (let diff of unifiedDiffs) {
      ...
      await fileDiff.fromGitObject({diff, stagedDiff, unstagedDiff, statusFile}) // eslint-disable-line babel/no-await-in-loop
      files.push(fileDiff)
    }
  ...
  async createAndCheckoutBranch (name: string): Promise<void> {
    await this.gitService.createAndCheckoutBranch(name)
