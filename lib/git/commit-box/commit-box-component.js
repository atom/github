...
export default class CommitBoxComponent {
  ...
  getCountdownClassNames (): Array<string> {
    ...
    if (isFinite(remainingCharacters)) {
      if (remainingCharacters < 0) {
        className = 'is-error'
      } else if (remainingCharacters < this.viewModel.getSummaryPreferredLength() / 5) {
  ...
  render () {
    const style = {width: this.getWidth() + 'px'}
    return (
      <div className='git-Panel-item git-CommitBox' style={style}>
        {this.renderError()}
  ...
  getWidth (): number {
    if (this.refs.editor) {
      const padding = 35
      const width = this.viewModel.getSummaryPreferredLength() * this.refs.editor.getCharacterWidth()
      return Math.round(width + padding)
    } else {
      return 300
    }
  }
