...
export default class CommitBoxViewModel {
  ...
  static NoMessageErrorName (): string {
    return 'CommitBoxViewModel.noMessage'
  }
  ...
  commit (message: string): Promise<void> {
    if (!message.length) {
      ...
      error.name = CommitBoxViewModel.NoMessageErrorName()
  ...
  calculateRemainingCharacters (msg: string, cursorPosition: Point): number {
    ...
    if (editingState === 'summary') {
      ...
      return this.getSummaryPreferredLength() - len
    } else {
      return Infinity
    }
  }

  getSummaryPreferredLength (): number {
    return 50
  }
}
