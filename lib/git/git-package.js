...
export default class GitPackage {
  ...
  async didChangeActivePaneItem (item: Object): Promise {
    ...
    const fileListComponent = await this.getFileListComponent()
    const statusBarViewModel = await this.getActiveStatusBarViewModel()
    const statusBarComponent = await this.getStatusBarComponent()

    await fileListComponent.update({fileListViewModel}, [])
    await statusBarComponent.update({statusBarViewModel}, [])
  }
  ...
  async getStatusBarComponent (): Promise<GitStatusBarComponent> {
    if (this.statusBarComponent == null) {
      const statusBarViewModel = await this.getActiveStatusBarViewModel()
  ...
  async getActiveStatusBarViewModel (): Promise<GitStatusBarViewModel> {
    const gitStore = await this.getActiveGitStore()
    ...
    if (statusBarViewModel == null) {
      ...
      this.subscriptions.add(new Disposable(() => {
        ...
        this.statusBarViewModelsByGitStore.delete(gitStore)
      }))
    }
