...
export default class GitPackage {
  ...
  async activate (state: GitState = {panelVisible: false}): Promise {
    this.state = state
    ...
    this.update(await this.getActiveGitStore(), 'reload')
  ...
  async getActiveGitStore (): Promise<GitStore> {
    ...
    if (projectPath != null) {
      ...
      this.activeGitStore = this.gitStoreForRepository(repository.async)
