...
export default class GitPackage {
  ...
  getFileListComponent (): FileListComponent {
    if (!this.fileListComponent) {
      this.fileListComponent = new FileListComponent({
        fileListViewModel: new FileListViewModel(this.getGitStore())
      })
    }
  ...
  getFileListViewModel (): FileListViewModel {
    return this.getFileListComponent().getViewModel()
  }

  getActiveFileListViewModel (): FileListViewModel {
    // find or create a file list view model for the active git store
  }
  ...
  getActiveGitStore (): GitStore {
    ...
    if (activeItem && typeof activeItem.getPath === 'function') {
      const repository = // find the repository for the directory matching the path, if one exists
      this.activeGitStore = this.gitStoreForRepository(repository)
      ...
      this.activeGitStore = this.gitStoreForRepository(atom.project.getRepositories()[0].async)
    }

    return this.activeGitStore
  }

  // replace getGitStore with this method, which stores them in a WeakMap
  ...
  didChangeActivePaneItem (item: Object) {
    // TODO: make sure we only etch.update the component if the view model *changes*
    this.fileListComponent.update({viewModel: this.getActiveFileListViewModel()})
  }
  ...
  async openChangesPanel (): Promise<void> {
    ...
    if (changesPanel) {
      ...
      this.changesPanel = atom.workspace.addRightPanel({item: this.getFileListComponent()})
    }
