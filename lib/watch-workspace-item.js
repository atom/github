...
class ItemWatcher {
  constructor(workspace, pattern, component, stateKey) {
    ...
    this.pattern =
      pattern instanceof URIPattern ? pattern : new URIPattern(pattern);
  ...
  subscribeToWorkspace() {
    ...
    this.subs = new CompositeDisposable(
      this.workspace
        .getCenter()
        .onDidChangeActivePaneItem(this.updateActiveState),
    );
    return this;
  }
  ...
  updateActiveState = () => {
    ...
    if (wasActive && !this.activeItem) {
      return new Promise(resolve =>
        this.component.setState({[this.stateKey]: false}, resolve),
      );
    } else if (!wasActive && this.activeItem) {
      return new Promise(resolve =>
        this.component.setState({[this.stateKey]: true}, resolve),
      );
    } else {
  ...
  itemMatches = item =>
    item && item.getURI && this.pattern.matches(item.getURI()).ok();
