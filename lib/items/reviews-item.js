...
export default class ReviewsItem extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      initThreadID: null,
      uncreatedThreadPositions: new Set(),
    };
  }
  ...
  render() {
    ...
    return (
      <ReviewsContainer
        ...
        uncreatedThreadPositions={Array.from(this.state.uncreatedThreadPositions)}
        {...this.props}
      />
    );
  }
  ...
  async addUncreatedThreadAt(bufferRow) {
    if (this.state.uncreatedThreadPositions.has(bufferRow)) {
      return;
    }

    await new Promise(resolve => this.setState(prev => {
      prev.uncreatedThreadPositions.add(bufferRow);
      return {};
    }));
  }
}
