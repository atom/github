...
export default class ReviewsItem extends React.Component {
  ...
  render() {
    ...
    return (
      <ReviewsContainer
        ...
        uncreatedThreadRows={Array.from(this.state.uncreatedThreadRows)}
        {...this.props}
      />
    );
  }
  ...
  async startCommentThreadAt(bufferRow) {
    if (this.state.uncreatedThreadRows.has(bufferRow)) {
      return;
    }

    await new Promise(resolve => this.setState(prev => {
      prev.uncreatedThreadRows.add(bufferRow);
      return {initThreadID: `uncreated-thread-${bufferRow}`};
    }));
  }

  openPendingReviewsTab() {
