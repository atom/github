...
function getPropsFromUri(uri) {
  // atom-github://file-patch/file.txt/staged?&activate&amending&lineNum=12
  ...
  if (protocol === 'atom-github:' && hostname === 'file-patch') {
    ...
    const {amending, activate, lineNum} = query;
    return {
      filePath,
      stagingStatus,
...
export default class FilePatchPaneItem {
  ...
  static create({filePath, stagingStatus, amending, activate, lineNumber}, uri) {
    return new FilePatchPaneItem({filePath, stagingStatus, amending, activate, lineNumber}, uri);
  }

  constructor({filePath, stagingStatus, amending, activate, lineNumber}, uri) {
  ...
  async createComponent() {
    ...
    const props = {
      ...
      didDiveIntoFilePath: rc.diveIntoFilePatchForPath,
