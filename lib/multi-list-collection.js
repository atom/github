...
export default class MultiListCollection {
  ...
  selectAllItemsForKey (key, addToExisting) {
    ...
    this.selectItems(this.list.getItemsForKey(key), {addToExisting: true})
  }

  selectFirstItemForKey (key, addToExisting) {
    if (!addToExisting) this.clearSelections()
    this.selectedKeys.add(key)
    this.selectItems([this.list.getItemsForKey(key)[0]], {addToExisting: true})
  }
  ...
  selectItemsAndKeysInRange (endPoint1, endPoint2, addToExisting) {
    ...
    if (startKeyIndex === endKeyIndex) {
      ...
      this.selectKeys([startPoint.key], {addToExisting: true})
  ...
  selectNextItem ({addToExisting} = {}) {
    this.list.selectNextItem()
    this.updateSelections({addToExisting})
  }

  selectPreviousItem ({addToExisting} = {}) {
