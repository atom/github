...
export class BarePrStatusesView extends React.Component {
  static propTypes = {
    ...
    displayType: PropTypes.oneOf(['check', 'full']),
  ...
  render() {
    const headCommit = this.props.pullRequest.recentCommits.edges[0].node
      .commit;
    ...
    if (this.props.displayType === 'check') {
        ...
        <div className="github-PrStatuses">
          ...
          <ul className="github-PrStatuses-list">
            {contexts.map(context => (
              <PrStatusContextView key={context.id} context={context} />
            ))}
          </ul>
        </div>
      );
      ...
      throw new Error(
        `Invalid \`displayType\` prop value: ${this.props.displayType}`,
      );
    }
  }
  ...
  renderDonutChart(status) {
    ...
    const pendingLen = contexts.filter(c => category(c.state) === 'PENDING')
      .length;
    const failedLen = contexts.filter(c => category(c.state) === 'FAILURE')
      .length;
    const succeededLen = contexts.filter(c => category(c.state) === 'SUCCESS')
      .length;
