...
export default class PushPullMenuView {
  ...
  checkRemote () {
    if (!this.props.remoteName) {
      this.errorMessage = `No remote detected for branch ${this.props.branchName}. Pushing will set up a remote tracking branch on remote repo 'origin'`
    } else {
      this.errorMessage = ''
    }
  }
    ...
    return (
      <div className='git-Panel-item'>
        <div className='git-PushPullMenuView '>
          ...
          <button className='git-PushPullMenuView-item btn' ref='fetchButton' onclick={this.props.fetch} disabled={!this.props.remoteName}>Fetch</button>

          <div className='git-PushPullMenuView-item is-flexible btn-group'>
            <button ref='pullButton' className='btn' onclick={this.props.pull} disabled={!this.props.remoteName || this.props.pullDisabled}>
