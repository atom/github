...
export default class FilePatchHeaderView extends React.Component {
  ...
  renderPath() {
    ...
    if (dirname === '.') {
      ...
      // TODO: double check that the forward-slash works cross-platform...
      // iirc git always uses `/` for paths, even on Windows
      return (
        <span>
          {dirname}/<span className="gitub-FilePatchHeaderView-basename">{basename}</span>
        </span>
      );
    }
  }
