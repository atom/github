...
export default class StagingView {
  ...
  selectList (listKey, {suppressCallback} = {}) {
    if (this.multiListCollection.getItemsForKey(listKey).length) {
      this.multiListCollection.selectKeys([listKey], {suppressCallback})
    }
  ...
  getSelectedListKey () {
    return this.multiListCollection.getActiveListKey()
  }
  ...
  focusFilePatchView () {
    const item = this.multiListCollection.getActiveItem()
    const listKey = this.multiListCollection.getActiveListKey()
  ...
  render () {
    ...
    const mergeConflictsView = (
      <div className={`git-StagingView-group git-MergeConflictPaths ${conflictsClassName}`}>
        ...
        <ListView
          onmousedown={() => this.selectList(ListTypes.CONFLICTS, {suppressCallback: true}) }
    ...
    return (
      <div className={`git-StagingView ${ListNames[selectedList]}-changes-focused`} style={{width: 200}} tabIndex='-1'
        ...
        <div className={`git-StagingView-group git-UnstagedChanges ${unstagedClassName}`}>
          ...
          <ListView
            onmousedown={() => this.selectList(ListTypes.UNSTAGED, {suppressCallback: true})}
            className='git-StagingView-list git-FilePatchListView'
        ...
        <div className={`git-StagingView-group git-StagedChanges ${stagedClassName}`} >
          <header className='git-StagingView-header'>
            ...
            <span className='git-StagingView-title'>Staged Changes</span>
          </header>
          <ListView
            onmousedown={() => this.selectList(ListTypes.STAGED, {suppressCallback: true})}
            className='git-StagingView-list git-FilePatchListView'
