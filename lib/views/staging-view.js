...
export default class StagingView {
  ...
  constructor(props) {
    ...
    this.props.workspace.onDidChangeActivePaneItem(item => {
      if (item instanceof FilePatchPaneItem) {
  ...
  async didSelectSingleItem(selectedItem, openNew) {
    if (this.selection.getActiveListKey() === 'conflicts') {
      ...
      if (openNew || this.pendingFilePatchItemPresent()) {
  ...
  pendingFilePatchItemPresent() {
    const pendingItems = this.props.workspace.getPanes().map(pane => pane.getPendingItem());
    for (let i = 0; i < pendingItems.length; i++) {
      const item = pendingItems[i];
      if (item && item instanceof FilePatchPaneItem) {
        return true;
      }
    }
    return false;
