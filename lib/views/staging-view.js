...
export default class StagingView extends React.Component {
    ...
    this.subs = new CompositeDisposable(
      atom.config.observe('github.keyboardNavigationDelay', value => {
        if (value === 0) {
          ...
          this.debouncedDidChangeSelectedItem = debounce(
  ...
  componentDidUpdate(prevProps, prevState) {
    const isRepoSame =
      prevProps.workingDirectoryPath === this.props.workingDirectoryPath;
  ...
  renderBody() {
    ...
    return (
      ...
      >
        {this.renderCommands()}
        <div
          className={`github-StagingView-group github-UnstagedChanges ${this.getFocusClass(
            'unstaged',
          )}`}
        >
          ...
          <div className="github-StagingView-list github-FilePatchListView github-StagingView-unstaged">
            {this.state.unstagedChanges.map(filePatch => (
              <FilePatchListItemView
                ...
                onContextMenu={event =>
                  this.contextMenuOnItem(event, filePatch)
                }
        ...
        <div
          className={`github-StagingView-group github-StagedChanges ${this.getFocusClass(
            'staged',
          )}`}
        >
          <header className="github-StagingView-header">
            <span className="icon icon-tasklist" />
            <span className="github-StagingView-title">Staged Changes</span>
            <button
  ...
  undoLastDiscardFromCommand = () => {
    this.undoLastDiscard({
      eventSource: {command: 'github:undo-last-discard-in-git-tab'},
    });
  };
  ...
  discardChangesFromCommand = () => {
    this.discardChanges({
      eventSource: {command: 'github:discard-changes-in-selected-files'},
    });
  };
  ...
  renderMergeConflicts() {
    ...
    if (mergeConflicts && mergeConflicts.length > 0) {
      ...
      const anyUnresolved = mergeConflicts
        ...
        .some(
          conflictPath => resolutionProgress.getRemaining(conflictPath) !== 0,
        );
      ...
      return (
        <div
          className={`github-StagingView-group github-MergeConflictPaths ${this.getFocusClass(
            'conflicts',
          )}`}
        >
          <header className="github-StagingView-header">
            <span
              className={
                'github-FilePatchListView-icon icon icon-alert status-modified'
              }
            />
            ...
            {mergeConflicts.map(mergeConflict => {
              ...
              return (
                <MergeConflictListItemView
                  ...
                  onContextMenu={event =>
                    this.contextMenuOnItem(event, mergeConflict)
                  }
                  onMouseDown={event =>
                    this.mousedownOnItem(event, mergeConflict)
                  }
                  onMouseMove={event =>
                    this.mousemoveOnItem(event, mergeConflict)
                  }
  ...
  getSelectedItemFilePaths() {
    return Array.from(
      this.state.selection.getSelectedItems(),
      item => item.filePath,
    );
  }
  ...
  stageAllMergeConflicts() {
    ...
    const filePaths = this.props.mergeConflicts.map(
  ...
  confirmSelectedItems = async () => {
    ...
    await new Promise(resolve => {
      this.setState(
        prevState => ({selection: prevState.selection.coalesce()}),
        resolve,
      );
    });
  };
  ...
  async syncWithWorkspace() {
    ...
    const realItemPromise =
      item.getRealItemPromise && item.getRealItemPromise();
    ...
    const isFilePatchItem =
      realItem.isFilePatchItem && realItem.isFilePatchItem();
    const isMatch =
      realItem.getWorkingDirectory &&
      realItem.getWorkingDirectory() === this.props.workingDirectoryPath;

    if (isFilePatchItem && isMatch) {
      this.quietlySelectItem(
        realItem.getFilePath(),
        realItem.getStagingStatus(),
      );
    }
  }
    ...
    return new Promise(resolve => {
      this.setState(prevState => {
        ...
        if (!item) {
          ...
          console.log(
            `Unable to find item at path ${filePath} with staging status ${stagingStatus}`,
          );
          return null;
        }
  ...
  getPanesWithStalePendingFilePatchItem() {
    ...
    return this.props.workspace.getPanes().filter(pane => {
      ...
      const isInActiveRepo =
        realItem.getWorkingDirectory() === this.props.workingDirectoryPath;
      const isStale = !this.changedFileExists(
        realItem.getFilePath(),
        realItem.getStagingStatus(),
      );
  ...
  async showFilePatchItem(
    filePath,
    stagingStatus,
    {activate, pane} = {activate: false},
  ) {
    ...
    if (activate) {
      ...
      this.props.workspace
        .paneForItem(changedFileItem)
        .activateItem(changedFileItem);
    }
  }

  async showMergeConflictFileForPath(
    relativeFilePath,
  ...
  getFocusClass(listKey) {
    return this.state.selection.getActiveListKey() === listKey
      ? 'is-focused'
      : '';
  }
