...
export default class StagingView {
  ...
  async update(props) {
    ...
    if (isRepoSame && selectionChanged) {
      // Fixes bug when focusing diff view from non-active repo. refactor to make more readable
      const item = this.props.workspace.getActivePaneItem();
      const isDiffViewItem = item.getRealItem && item.getRealItem() instanceof FilePatchController;
      if (isDiffViewItem) {
        this.quietlySelectItem(item.getFilePath(), item.getStagingStatus());
      } else {
