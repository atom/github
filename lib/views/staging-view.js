...
export default class StagingView {
  ...
  async diveIntoSelection() {
    ...
    if (stagingStatus === 'conflicts') {
      ...
      const filePatchView = await this.props.workspace.open(
  ...
  async didSelectSingleItem(selectedItem) {
    if (this.selection.getActiveListKey() === 'conflicts') {
      ...
      const amending = this.props.isAmending && this.selection.getActiveListKey() === 'staged';
      const filePatchView = await this.props.workspace.open(
        `atom-github://file-patch/${selectedItem.filePath}/${this.selection.getActiveListKey()}?${amending ? 'amending' : ''}&activate`,
        {pending: true, activatePane: false, activateItem: false},
      );
      this.props.workspace.paneForItem(filePatchView).activateItem(filePatchView);
    }
  }
