...
export default class StagingView {
  ...
  constructor(props) {
    ...
    this.subscriptions.add(
      ...
      this.props.workspace.onDidChangeActivePaneItem(item => {
        if (item && item.getRealItem && item.getRealItem() instanceof FilePatchController && item.getWorkingDirectory() === this.props.workingDirectoryPath) {
  ...
  getPendingFilePatchItem() {
    ...
    if (activeItem && activeItem.getRealItem && activeItem.getRealItem() instanceof FilePatchController && activeItem === activePendingItem) {
      return activeItem;
    }
