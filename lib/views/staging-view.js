...
export default class StagingView {
  ...
  renderMergeConflicts() {
    ...
    if (mergeConflicts && mergeConflicts.length > 0) {
      ...
      const headerButton = anyUnresolved ? this.renderAbortMergeButton() : this.renderStageAllConflictsButton();
  ...
  renderAbortMergeButton() {
    return (
      <button className="github-StagingView-headerButton icon icon-flame"
        onclick={this.props.abortMerge}>Abort Merge</button>
    );
  }

  renderStageAllConflictsButton() {
    return (
      <button className="github-StagingView-headerButton icon icon-move-down"
        onclick={this.stageAllMergeConflicts}>Stage All</button>
    );
  }
