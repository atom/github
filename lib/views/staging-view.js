...
export default class StagingView {
  ...
  constructor(props) {
    ...
    this.subscriptions.add(
      ...
      this.props.workspace.onDidChangeActivePaneItem(item => {
        if (item instanceof FilePatchPaneItem && item.getWorkingDirectory() === this.props.workingDirectoryPath) {
          this.quietlySelectItem(item.filePath, item.stagingStatus);
        }
      }),
    );
  }
  ...
  getSelectedItem() {
    ...
    if (selectedItems.length === 1) {
      return {
        ...
        stagingStatus: this.selection.getActiveListKey(),
      };
    } else {
      return null;
    }
  ...
  async showFilePatchItem(filePath, stagingStatus, {activate} = {activate: false}) {
    const amending = this.props.isAmending && stagingStatus === 'staged';
    const filePatchItem = await this.props.workspace.open(
      `atom-github://file-patch/${filePath}?stagingStatus=${stagingStatus}&${amending ? 'amending' : ''}`,
  ...
  async showMergeConflictFileForPath(relativeFilePath, {activate} = {activate: false}) {
    ...
    if (await new File(absolutePath).exists()) {
      return this.props.workspace.open(absolutePath, {activatePane: activate, pending: true});
    } else {
  ...
  getPendingFilePatchItem() {
    ...
    if (activeItem && activeItem instanceof FilePatchPaneItem && activeItem === activePendingItem) {
      return activeItem;
    }
    ...
    for (let i = 0; i < panes.length; i++) {
      ...
      if (item && item instanceof FilePatchPaneItem && item === pendingItem) {
        return item;
      }
    }
    return null;
