...
export default class StagingView {
  ...
  constructor(props) {
    ...
    this.subscriptions.add(
      ...
      this.props.workspace.onDidChangeActivePaneItem(item => {
        const isFilePatchController = item && item.getRealItem && item.getRealItem() instanceof FilePatchController;
        const isMatch = item && item.getWorkingDirectory() === this.props.workingDirectoryPath;
        if (isFilePatchController && isMatch) {
          this.quietlySelectItem(item.filePath, item.getStagingStatus());
        }
      }),
    );
  }
  ...
  getPendingFilePatchItem() {
    ...
    const isController = activeItem && activeItem.getRealItem && activeItem.getRealItem() instanceof FilePatchController;
    if (isController && activeItem === activePendingItem) {
      return activeItem;
    }
