...
export default class StagingView {
  ...
  async update(props) {
    ...
    if (this.activeFilePatch) {
      ...
      const selectionsPresent = previouslySelectedItems.size > 0 && this.selection.getSelectedItems() > 0; // ignore when data has not yet been fetched
      const selectionChanged = selectionsPresent && !isEqual(previouslySelectedItems, this.selection.getSelectedItems());
  ...
  getPanesWithStalePendingFilePatchItem() {
    // "stale" meaning there is no longer a changed file associated with it due to changes being fully staged/unstaged/stashed/deleted/etc
