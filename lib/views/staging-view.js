...
export default class StagingView {
    ...
    this.subscriptions.add(
      ...
      this.props.workspace.onDidChangeActivePaneItem(item => {
        if (item) {
          ...
          this.activeFilePatch = isFilePatchController ? item : null;
        } else {
          this.activeFilePatch = null;
        }
      }),
    );
  }
  ...
  async update(props) {
    ...
    this.selection = this.selection.updateLists([
      ['unstaged', this.props.unstagedChanges],
      ['conflicts', this.props.mergeConflicts || []],
      ['staged', this.props.stagedChanges],
    ]);
    const currentlySelectedItems = this.selection.getSelectedItems();
    ...
    if (this.activeFilePatch) {
      ...
      const selectionsPresent = previouslySelectedItems.size > 0 && currentlySelectedItems.size > 0;
      // ignore when data has not yet been fetched and no selection is present
      const selectionChanged = selectionsPresent && !isEqual(previouslySelectedItems, currentlySelectedItems);
      if (isRepoSame && selectionChanged) {
  ...
  @autobind
  activateNextList() {
    const next = this.selection.activateNextSelection();
    if (this.selection === next) {
      return false;
    }
    this.selection = next.coalesce();
  ...
  activatePreviousList() {
    const next = this.selection.activatePreviousSelection();
    if (this.selection === next) {
      return false;
    }
  ...
  async confirmSelectedItems() {
    ...
    this.selection = this.selection.coalesce();
    this.debouncedDidChangeSelectedItem();
    return etch.update(this);
  }
  ...
  selectPrevious(preserveTail = false) {
    this.selection = this.selection.selectPreviousItem(preserveTail).coalesce();
    if (!preserveTail) { this.debouncedDidChangeSelectedItem(); }
    return etch.update(this);
  }

  selectNext(preserveTail = false) {
    this.selection = this.selection.selectNextItem(preserveTail).coalesce();
    if (!preserveTail) { this.debouncedDidChangeSelectedItem(); }
    return etch.update(this);
  }

  selectAll() {
    this.selection = this.selection.selectAllItems().coalesce();
    return etch.update(this);
  }

  selectFirst(preserveTail = false) {
    this.selection = this.selection.selectFirstItem(preserveTail).coalesce();
    if (!preserveTail) { this.debouncedDidChangeSelectedItem(); }
    return etch.update(this);
  }

  selectLast(preserveTail = false) {
    this.selection = this.selection.selectLastItem(preserveTail).coalesce();
    if (!preserveTail) { this.debouncedDidChangeSelectedItem(); }
    return etch.update(this);
  }
  ...
  quietlySelectItem(filePath, stagingStatus) {
    ...
    this.selection = this.selection.selectItem(item);
    return etch.update(this);
  }
  ...
  async didSelectSingleItem(selectedItem, openNew = false) {
    if (this.selection.getActiveListKey() === 'conflicts') {
      ...
      if (openNew) {
        ...
        await this.showFilePatchItem(selectedItem.filePath, this.selection.getActiveListKey(), {activate: false});
      } else {
  ...
  changedFileExists(filePath, stagingStatus) {
    return this.selection.findItem((item, key) => {
  ...
  async contextMenuOnItem(event, item) {
    if (!this.selection.getSelectedItems().has(item)) {
      ...
      this.selection = this.selection.selectItem(item, event.shiftKey);
  ...
  async mousedownOnItem(event, item) {
    ...
    if (event.button === 0) {
      ...
      if (event.metaKey || (event.ctrlKey && windows)) {
        this.selection = this.selection.addOrSubtractSelection(item);
      } else {
        this.selection = this.selection.selectItem(item, event.shiftKey);
      }
      await etch.update(this);
  ...
  async mousemoveOnItem(event, item) {
    if (this.mouseSelectionInProgress) {
      this.selectionChanged = true;
      this.selection = this.selection.selectItem(item, true);
      await etch.update(this);
    }
  }
