...
export default class StagingView {
  ...
  async showFilePatchItem(filePath, stagingStatus, {activate} = {activate: false}) {
    const filePatchItem = await this.props.workspace.open(
      `atom-github://file-patch/${filePath}?workdir=${this.props.workingDirectoryPath}&stagingStatus=${stagingStatus}`,
        ...
        <div className={`github-StagingView-group github-UnstagedChanges ${this.getFocusClass('unstaged')}`}>
          <header className="github-StagingView-header">
            ...
            <span className="github-StagingView-title">
              Unstaged Changes
              <span className="github-StagingView-title-count">(3/457)</span>
            </span>
            <span className="icon icon-settings" />
            {this.props.unstagedChanges.length ? this.renderStageAllButton() : null}
          </header>

          <div className="github-StagingView-filters native-key-bindings">
            <div className="github-StagingView-filters-row">
              <input className="input-text" type="text" placeholder="Filter file list (globs accepted)" value="spec/token.*coffee" />
            </div>
            <div className="github-StagingView-filters-row">
              <label className="input-label">
                <input className="input-checkbox" type="checkbox" checked /> Untracked Files
              </label>
            </div>
          </div>
          ...
          <div ref="unstagedChanges" className="github-StagingView-list github-FilePatchListView">
            {
              this.truncatedLists.unstagedChanges.filter(f => f.filePath.match(/^spec\/token.*coffee$/)).map(filePatch => (
            ...
            <button className="github-StagingView-headerButton github-StagingView-headerButton--fullWidth github-StagingView-moreFilesButton icon icon-eye">454 more files hidden by filters</button>
          </div>
        ...
        <div className={`github-StagingView-group github-StagedChanges ${this.getFocusClass('staged')}`} >
          <header className="github-StagingView-header">
            ...
            <span className="github-StagingView-title">
              ...
              <span className="github-StagingView-title-count">(58)</span>
            </span>
            <span className="icon icon-settings" />
