...
export default class PushPullView {
  constructor ({repository, branchName}) {
    ...
    this.refreshAheadBehindCount()
  }

  update ({repository, branchName}) {
    this.repository = repository
    this.branchName = branchName
    return this.refreshAheadBehindCount()
  }
    ...
    return (
      <div className='git-Panel-item'>
        <div className='git-PushPull '>
          ...
          <button className='git-PushPull-item btn' onclick={() => this.repository.fetch(this.branchName)}>Fetch</button>

          <div className='git-PushPull-item is-flexible btn-group'>
            <button className='btn' onclick={() => this.repository.pull(this.branchName)}>
              <span className='icon icon-arrow-down'/>
              Pull {this.behindCount !== 0 ? `(${this.behindCount})` : ''}
            </button>
            <button className='btn' onclick={() => this.repository.push(this.branchName)}>
