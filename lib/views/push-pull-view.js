...
export default class PushPullView extends React.Component {
  ...
  setTooltip = () => {
    this.tooltip = atom.tooltips.add(this.tileNode, {
      title: `<div style="text-align: left; line-height: 1.2em;">${this.tooltipTarget.dataset.tooltip}</div>`,
  ...
  render() {
    ...
    return (<div
      ...
      {isAhead && !isBehind && !isUnpublished && (
        <a
          ref={node => (this.tooltipTarget = node)}
      ...
      {isUnpublished && !isDetached && hasOrigin && (
        <a
          ref={node => (this.tooltipTarget = node)}
          onClick={this.onClickPublish}
          className="push-pull-target"
          data-tooltip="Click to set up a remote tracking branch<br />Right-click for more">
      ...
      {isUnpublished && !isDetached && !hasOrigin && (
        <span ref={node => (this.tooltipTarget = node)}
          className="push-pull-target"
          data-tooltip={'There is no remote named "origin"'}>
          <span className={getIconClass('stop')} />
          No remote
        </span>
      )}
