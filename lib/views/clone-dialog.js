...
export default class CloneDialog extends React.Component {
  ...
  clone() {
    if (
      this.getRemoteUrl().length === 0 ||
      this.getProjectPath().length === 0
    ) {
      return;
    }
  ...
  didChangeRemoteUrl() {
    if (!this.projectPathModified) {
      const name =
        path.basename(url.parse(this.getRemoteUrl()).pathname, '.git') || '';
  ...
  editorRefs(baseName) {
    ...
    const changeMethodName = `didChange${baseName[0].toUpperCase()}${baseName.substring(
      1,
    )}`;
  ...
  setCloneEnablement() {
    const disabled =
      this.getRemoteUrl().length === 0 || this.getProjectPath().length === 0;
