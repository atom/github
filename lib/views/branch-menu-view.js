...
export default class BranchMenuView extends React.Component {
  ...
  async checkout(branchName, options) {
    ...
    try {
      ...
      // restore controls immediately, otherwise wait until branch appears in branch list
      this.restoreControls();
      this.setState({checkedOutBranch: null});
  ...
  disableControls(branchName) {
    if (this.editorElement) {
      this.editorElement.getComponent().setInputEnabled(false);
  ...
  restoreControls() {
    if (this.editorElement) {
      this.editorElement.getComponent().setInputEnabled(true);
      this.editorElement.focus();
