  ...
  async didSelectItem(event) {
    ...
    try {
      ...
      if (e.stdErr.match(/local changes.*would be overwritten/)) {
        ...
        this.props.notificationManager.addError('Checkout aborted', {detail: e.stdErr, dismissable: true});
      }
    }
    return null;
  }
    ...
    if (this.state.createNew) {
      ...
      try {
        ...
        if (e.stdErr.match(/branch.*already exists/)) {
          ...
          this.props.notificationManager.addError('Cannot create branch', {detail: e.stdErr, dismissable: true});
        }
