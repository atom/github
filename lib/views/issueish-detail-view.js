...
export class BareIssueishDetailView extends React.Component {
  ...
  renderCheckoutButton() {
    ...
    if (!checkoutOp.isEnabled()) {
      buttonText = checkoutOp.getMessage();
      extraClass = 'github-IssueishDetailView-checkoutButton--' + checkoutOp.why()({
        disabled: 'disabled',
        busy: 'busy',
        current: 'current',
      });
    ...
    return (
      <button className={classNames} disabled={!checkoutOp.isEnabled()} onClick={() => checkoutOp.run()}>
        {buttonText}
      </button>
    );
  }
