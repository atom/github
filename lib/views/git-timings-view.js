...
export default class GitTimingsView extends React.Component {
  ...
  static markerByOperationId = new Map();
  ...
  static generateMarker(label, operationId) {
    const existingMarker = GitTimingsView.markerByOperationId.get(operationId);
    if (existingMarker) { return existingMarker; }
    const marker = new Marker(label, () => {
      GitTimingsView.scheduleUpdate();
    });
    GitTimingsView.markerByOperationId.set(operationId, marker);
