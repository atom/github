...
export default class ReviewsView extends React.Component {
  static propTypes = {
    ...
    uncreatedCommentThreads: PropTypes.arrayOf(PropTypes.shape({
      thread: PropTypes.object.isRequired,
      comments: PropTypes.arrayOf(PropTypes.object).isRequired,
    ...
    addCommentToReview: PropTypes.func.isRequired,
    startPendingReview: PropTypes.func.isRequired,
    replyToStartReview: PropTypes.func.isRequired,
  ...
  reviewTypesByState = this.reviewTypes.reduce((map, type) => map.set(type.state, type), new Map());

  reviewTypesByAction = this.reviewTypes.reduce((map, type) => map.set(type.action, type), new Map());
  ...
  getPendingReview = () => this.props.summaries.find(summary => summary.state === 'PENDING');

  render() {
    const pendingReview = this.getPendingReview();
    const pendingCommentThreads = this.props.commentThreads.filter(thread =>
      thread.comments.some(comment => comment.state === 'PENDING'));
    const pendingCommentCount = pendingCommentThreads.reduce(
      (count, thread) => count + thread.comments.filter(comment => comment.state === 'PENDING').length
      , 0);
      ...
      <div className="github-Reviews" ref={this.rootHolder.setter}>
        ...
        <Tabs>
          <TabList className="github-tablist">
            <Tab className="github-tab">All Reviews</Tab>
            {!pendingReview &&
              <li className="github-tab github-Reviews-startReviewButton">
                <button className="btn btn-primary" onClick={this.props.startPendingReview}>Start a new review</button>
              </li>}
            {pendingReview &&
              <Tab className="github-tab">
                Pending Review
                <span className="github-tab-count">{pendingCommentCount}</span>
              </Tab>}
          </TabList>
          <TabPanel>
          ...
          {pendingReview &&
            <TabPanel>
              <PendingReviewView
                ...
                pendingCommentThreads={pendingCommentThreads}
                reviewTypesByAction={this.reviewTypesByAction}
                renderReviewCommentThread={this.renderReviewCommentThread}
              />
            </TabPanel>}
        </Tabs>
      </div>
  ...
  renderReviewSummaries() {
    if (this.props.summaries.length + this.props.uncreatedCommentThreads.length === 0) {
  ...
  renderReviewSummary = review => {
    ...
    const {icon, copy} = this.reviewTypesByState.get(review.state) || {icon: '', copy: ''};
  ...
  renderReviewCommentThreads() {
    ...
    if (commentThreads.length + this.props.uncreatedCommentThreads.length === 0) {
      return null;
    }
    ...
    return (
      <details
        ...
        {unresolvedThreads.length + this.props.uncreatedCommentThreads.length > 0 &&
          <main className="github-Reviews-container">
            {unresolvedThreads.map(this.renderReviewCommentThread)}
            {this.props.uncreatedCommentThreads.map(this.renderReviewCommentThread)}
          </main>}
  ...
  renderReviewCommentThread = commentThread => {
    ...
    const {lineNumber, positionText} = this.getTranslatedPosition(rootComment.path, rootComment.position);
    ...
    return (
      <Fragment>
        <main className="github-Review-comments">
          ...
          {comments.filter(comment => !comment.isUncreated).map(comment => {
            return (
              <ReviewCommentView
                key={comment.id}
                comment={comment}
          ...
          <div className="github-Review-replyButtonRow">
            {pendingReview &&
              <button
                className="btn btn-primary btn-sm github-Review-replyButton"
                title="Add your comment to review"
                disabled={isPosting}
                onClick={() => this.submitReply(replyHolder, thread, lastComment, 'EXISTING_REVIEW')}>
            ...
            {!pendingReview &&
              <Fragment>
                <button
                  className="btn btn-sm github-Review-singleCommentButton"
                  title="Add a single comment"
                  disabled={isPosting}
                  onClick={() => this.submitReply(replyHolder, thread, lastComment, 'SINGLE_COMMENT')}>
                  Add a single comment
                </button>
                <button
                  className="btn btn-primary btn-sm github-Review-startReviewButton"
                  title="Add your comment and start a review"
                  disabled={isPosting}
                  onClick={() => this.submitReply(replyHolder, thread, lastComment, 'NEW_REVIEW')}>
                  Start a review
                </button>
              </Fragment>
            }
  ...
  submitReply(replyHolder, thread, lastComment, action) {
    const body = replyHolder.map(editor => editor.getText()).getOr('');
    const replyToID = lastComment.isUncreated ? null : lastComment.id;

    const didSucceed = () => replyHolder.map(editor => editor.setText('', {bypassReadOnly: true}));
    const didFail = () => replyHolder.map(editor => editor.setText(body, {bypassReadOnly: true}));
    ...
    case 'SINGLE_COMMENT':
      replyAction = this.props.addSingleComment(
        body, thread.id, replyToID, lastComment.path, lastComment.position, {didSucceed, didFail},
      );
      break;
    case 'NEW_REVIEW':
      replyAction = this.props.replyToStartReview(
        body, thread.id, replyToID, lastComment.path, lastComment.position, {didSucceed, didFail},
      );
      break;
    case 'EXISTING_REVIEW':
      replyAction = this.props.addCommentToReview(
        this.getPendingReview().id, body, thread.id, replyToID,
        lastComment.path, lastComment.position, 'PENDING',
        {didSucceed, didFail},
      );
      break;
    default:
  ...
  getTranslatedPosition(commentPath, commentPosition) {
    ...
    if (translations === null) {
      lineNumber = null;
      positionText = '';
    } else if (commentPosition === null) {
      lineNumber = null;
      positionText = 'outdated';
    } else {
      const translationsForFile = translations.get(commentPath);
      lineNumber = translationsForFile.diffToFilePosition.get(parseInt(commentPosition, 10));
