    ...
    return (
      <div className={commentClass} key={comment.id}>
        <header className="github-Review-header">
          ...
          <a className="github-Review-reportAbuseLink" title="report abuse" href={`https://github.com/contact/report-content?report=${comment.author.login}&content_url=${encodeURIComponent(comment.url)}`}>
            <Octicon icon="alert" />
          </a>
        </header>
