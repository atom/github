...
export default class ReviewsView extends React.Component {
  ...
  renderReviewSummaries = summaries => {
    ...
    const toggleSummarySection = this.props.summarySectionOpen
      ? evt => { evt.preventDefault(); this.props.hideSummaries(); }
      : evt => { evt.preventDefault(); this.props.showSummaries(); };
  ...
  renderReviewCommentThreads = commentThreads => {
    ...
    const toggleComments = this.props.commentSectionOpen
      ? evt => { evt.preventDefault(); this.props.hideComments(); }
      : evt => { evt.preventDefault(); this.props.showComments(); };
  ...
  renderReviewCommentThread = commentThread => {
    ...
    return (
      <details
        className={cx('github-Review', {resolved: thread.isResolved})}
        key={elementId}
        id={elementId}
        open={isOpen}
        onClick={toggle}>
