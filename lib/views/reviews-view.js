...
export default class ReviewsView extends React.Component {
  ...
  renderReviewCommentThread = commentThread => {
    ...
    const threadVisible = !thread.isResolved || this.props.resolvedThreadsMarkedVisible.has(thread.id);
  ...
  resolveReviewThread = thread => {
    if (thread.viewerCanResolve) {
      ...
      const onCompleted = () => this.props.collapseThread(thread.id);
      const onError = () => this.props.uncollapseThread(thread.id);
