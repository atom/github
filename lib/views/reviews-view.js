...
export default class ReviewsView extends React.Component {
  static propTypes = {
    ...
    commentIDsOpen: PropTypes.shape({
      has: PropTypes.func.isRequired,
    }),
    resolvedThreadsMarkedVisible: PropTypes.shape({
      has: PropTypes.func.isRequired,
    }).isRequired,
    ...
    showCommentID: PropTypes.func.isRequired,
    hideCommentID: PropTypes.func.isRequired,
    collapseThread: PropTypes.func.isRequired,
    uncollapseThread: PropTypes.func.isRequired,
  ...
  render() {
    return (
      <div className="github-Reviews" ref={this.rootHolder.setter}>
        ...
        <AggregatedReviewsContainer pullRequest={this.props.pullRequest}>
  ...
  renderReviewCommentThread = commentThread => {
    ...
    return (
      <details className={cx('github-Review', {resolved: thread.isResolved})} key={elementId} id={elementId}>
  ...
  renderResolvedHeader = (thread, visible) => {
    return (
      <Fragment>
        ...
        {
          visible ?
            <button onClick={() => this.props.collapseThread(thread.id)}> Hide conversation </button> :
            <button onClick={() => this.props.uncollapseThread(thread.id)}> Show conversation </button>
        }
      </Fragment>
    );
  }
    ...
    return (
      <Fragment>
        <main className="github-Review-comments">
          ...
          <div className={cx('github-Review-reply', {'github-Review-reply--disabled': isPosting})}>
            <AtomTextEditor
  ...
  openFile = evt => {
    if (!this.props.checkoutOp.isEnabled()) {
      ...
      this.props.openFile(target.dataset.path, target.dataset.line, target.dataset.diff);
    }
  }
  ...
  resolveThread = thread => {
    const onComplete = this.collapseThread;
    const onError = this.uncollapseThread;
    this.props.resolveThread(thread, onComplete, onError);
  }

  collapseThread = threadId => {
    document.getElementById(`review-thread-${threadId}`).removeAttribute('open');
  }

  uncollapseThread = threadId => {
    document.getElementById(`review-thread-${threadId}`).setAttribute('open', '');
  }
  ...
  submitReply(replyHolder, thread, lastComment) {
    ...
    return this.props.addSingleComment(body, thread.id, lastComment.id, {didSubmitComment}).catch(err => {
      // Restore the failed comment if it fails to post
      replyHolder.map(editor => editor.setText(body));
