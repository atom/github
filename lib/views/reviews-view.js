...
export default class ReviewsView extends React.Component {
  ...
  render() {
    const hasPendingReview = this.props.summaries.some(summary => summary.state === 'PENDING');
    ...
    return (
      <div className={commentClass} key={comment.id}>
        <header className="github-Review-header">
          ...
          <a className="github-Review-reportAbuseLink" title="report abuse" href={`https://github.com/contact/report-content?report=${encodeURIComponent(author.login)}&content_url=${encodeURIComponent(comment.url)}`}>
            <Octicon icon="alert" />
          </a>
        </header>
