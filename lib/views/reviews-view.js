...
export default class ReviewsView extends React.Component {
  static propTypes = {
    ...
    didRefetch: PropTypes.func.isRequired,
    ...
    return (
      <div className="github-Reviews" ref={this.rootHolder.setter}>
        ...
        <AggregatedReviewsContainer
          ...
          summariesRenderer={this.renderReviewSummaries}>
          {({errors, summaries, commentThreads, loading, refetch}) => {
            ...
            console.log('ReviewsView::render', commentThreads.map(pair => pair.comments.map(c => c.id).join(', ')).join(' | '));
  ...
  renderReviewSummaries = ({errors, summaries}) => {
