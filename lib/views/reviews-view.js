...
export default class ReviewsView extends React.Component {
    ...
    return (
      <div className="github-Reviews" ref={this.rootHolder.setter}>
        ...
        <div className="github-Reviews-list">
          <AggregatedReviewsContainer pullRequest={this.props.repository.pullRequest} ref={c => { this.component = c; }}>
  ...
  renderHeader() {
    return (
      <header className="github-Reviews-topHeader">
        ...
        <button className="github-Reviews-headerButton github-Reviews-clickable icon icon-repo-sync" />
        <CheckoutButton
  ...
  renderReviewCommentThread = commentThread => {
    console.log(commentThread.id, commentThread);
    ...
    return (
      <details className="github-Review" key={`review-comment-${rootComment.id}`}>
        ...
        {this.renderResolveButton(commentThread.thread)}
      </details>
    );
  }

  renderResolveButton = thread => {
    if (thread.id === global.threadId) { debugger; }
    console.log(thread);
    if (thread.isResolved) {
        ...
        <RelayEnvironment.Consumer>
          {relayEnvironment =>
            <footer className="github-Review-footer">
              <button className="github-Review-resolveButton btn btn-primary icon icon-check" title="Unresolve conversation"
                onClick={() => this.unresolveReviewThread(thread.id, relayEnvironment)} >
      ...
      return (
        <RelayEnvironment.Consumer>
          {relayEnvironment =>
            <footer className="github-Review-footer">
              <button className="github-Review-resolveButton btn btn-primary icon icon-check" title="Resolve conversation"
                onClick={() => this.resolveReviewThread(thread.id, relayEnvironment)} >
  ...
  resolveReviewThread = (threadId, environment) => {
    console.log(threadId, environment);
    ResolveReviewThreadMutation(threadId, environment, thread => {
      global.threadId = thread.id;
      console.log('forceUpdate!!!!!!!!!!!!!!!!!!!');
      // debugger;
      const accum = this.component.reviewThreadsAccumulator;
      debugger;
      accum.props.relay.refetchConnection(accum.getFragmentVariables().totalCount, () => {}, accum.getVariables());
    });
  }

  unresolveReviewThread = (threadId, environment) => {
    console.log(threadId, environment);
    UnresolveReviewThreadMutation(threadId, environment);
  }
}
