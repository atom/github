  ...
  render() {
    ...
    return (
      <li className={cx('github-RecentCommit', {'most-recent': this.props.isMostRecent})}>
        ...
        {this.props.isMostRecent && <button className="btn github-RecentCommit-undoButton">Undo</button>}
        <Timeago
      ...
      return (
        <ul className="github-RecentCommits-list">
          {this.props.commits.map((commit, i) => {
            return (
              <RecentCommitView isMostRecent={i === 0} key={commit.getSha()} commit={commit} />
            );
          })}
        </ul>
      );
