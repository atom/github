...
export default class CommitView {
  ...
  constructor(props) {
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      props.tooltips.add(this.element.getElementsByClassName('github-CommitView-info')[0], {
        title: 'Toggle hard wrap on commit',
        class: 'github-CommitView-info-tooltip',
      }),
  ...
  renderHardWrapIcons() {
    ...
    const hardWrap = this.props.config && this.props.config.get('github.automaticCommitMessageWrapping');
    return (
      <div onclick={this.toggleHardWrap} className="github-CommitView-info">
        <div className={cx('icon', 'no-hardwrap', 'icon-no-newline', {hidden: singleLineMessage || !hardWrap})} />
        <div className={cx('icon', 'hardwrap', 'icon-arrow-left', {hidden: singleLineMessage || hardWrap})} />
      </div>
    );
  }
