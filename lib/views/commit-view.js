...
export default class CommitView extends React.Component {
  ...
  render() {
    ...
    // TODO: fix tooltip hackyness (find alternative to using `document.querySelector`)
  ...
  renderCoAuthorToggleIcon() {
    ...
    return (
      <svg className={cx('github-CommitView-coAuthorToggleIcon', {focused: this.state.showCoAuthorInput})} viewBox="0 0 12 7" xmlns="http://www.w3.org/2000/svg">
        <title>Toggle Add or remove co-authors</title>
        <path d={svgPath} />
      </svg>
    );
  }
  ...
  renderCoAuthorInput() {
    ...
    return (
      <Select
        ...
        className="github-CommitView-coAuthorEditor github-coAuthor-select-input input-textarea native-key-bindings"
  ...
  onSelectedCoAuthorsChanged(selectedCoAuthors) {
    ...
    if (newAuthor) {
      this.setState({coAuthorInput: newAuthor.name, showCoAuthorDialog: true});
    } else {
