...
export default class CommitView extends React.Component {
  ...
  onSelectedCoAuthorsChanged(selectedCoAuthors) {
    // if "new author", prompt for name and email.
    // call updateSelectedCoAuthors with new author info
    let newAuthor = false;
    const validCoAuthors = selectedCoAuthors.filter(author => {
    ...
    if (newAuthor) {
      // prompt
      newAuthor = {name: 'New Perons', email: 'newperson@email.com'};
      validCoAuthors.push(newAuthor);
    }
    this.props.updateSelectedCoAuthors(validCoAuthors, newAuthor);
  }
