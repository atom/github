...
export default class CommitView {
  ...
  setMessageAndAmendStatus () {
    if (!this.props.repository) return
    let {message, cursorPosition, amendInProgress} = this.props.repository.getCommitState() || {}
    message = message || ''
  ...
  updateStateForRepository () {
    ...
    if (repository) {
      const state = repository.getCommitState() || {}
      repository.setCommitState({
  ...
  async handleAmendBoxClick () {
    ...
    if (this.props.repository) {
      state = this.props.repository.getCommitState()
    }
    state.amendInProgress = checked
    if (checked) {
      state.messagePriorToAmending = this.editor.getText()
