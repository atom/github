...
export default class CommitView extends React.Component {
  ...
  async commit() {
    if (await this.props.prepareToCommit() && this.isCommitButtonEnabled()) {
      try {
        await this.props.commit(this.editor.getText(), this.state.selectedCoAuthors);
      } catch (e) {
        // do nothing
      }
    } else {
  ...
  matchAuthors(authors, filterText, currentValues) {
    return authors
      ...
      .filter(x => `${x.name}${x.login}${x.email}`.toLowerCase().indexOf(filterText.toLowerCase()) !== -1);
  }
  ...
  renderCoAuthorListItem(author) {
    return (
      <div className="github-CommitView-coAuthorEditor-selectListItem">
        ...
        <span className="github-CommitView-coAuthorEditor-email">{author.email}</span>
      </div>
    );
  }

  renderCoAuthorValue(author) {
    const user = author.login ? `@${author.login}` : author.name;
    return (
      <span>{user}</span>
    );
  }
