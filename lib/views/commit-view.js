...
export default class CommitView extends React.Component {
  ...
  renderHardWrapIcon() {
    const singleLineMessage = this.props.message.split(LINE_ENDING_REGEX).length === 1;
  ...
  async commit(event, amend) {
    if (await this.props.prepareToCommit() && this.commitIsEnabled(amend)) {
      try {
        await this.props.commit(this.props.message, this.props.selectedCoAuthors, amend);
      } catch (e) {
  ...
  commitIsEnabled(amend) {
    const messageExists = this.props.message.length > 0;
  ...
  toggleExpandedCommitMessageEditor() {
    return this.props.toggleExpandedCommitMessageEditor(this.props.message);
  }
