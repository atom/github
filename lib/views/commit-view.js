...
export default class CommitView extends React.Component {
  ...
  componentWillMount() {
    this.triggerShowWorking(this.props);
  }

  componentDidMount() {
    if (this.editor.getText() !== this.props.message) {
      this.editor.setText(this.props.message);
      this.forceUpdate();
    }

    this.subscriptions = new CompositeDisposable(
      this.editor.onDidChange(() => this.props.onChangeMessage(this.editor.getText())),
      this.editor.onDidChangeCursorPosition(() => this.forceUpdate()),
  ...
  componentWillReceiveProps(nextProps) {
    if (this.editor && this.props.message !== nextProps.message && this.editor.getText() !== nextProps.message) {
      this.editor.setText(nextProps.message);
    }

    this.triggerShowWorking(nextProps);
  }
