...
import CoAuthorDialog from './co-author-dialog';

import Tooltip from './tooltip';
import AtomTextEditor from './atom-text-editor';
import {LINE_ENDING_REGEX, NEW_AUTHOR} from '../helpers';
...
export default class CommitView extends React.Component {
  ...
  renderCoAuthorInput() {
    ...
    // TODO: investigate more performant ways of adding "New Author" list item
    ...
    return (
      <Select
        ...
        arrowRenderer={null}
        options={[NEW_AUTHOR, ...this.props.mentionableUsers]}
        labelKey="name"
        valueKey="email"
        ...
        onInputChange={this.onCoAuthorInputChange}
  ...
  @autobind
  onCoAuthorInputChange(inputValue) {
    if (inputValue) {
      this.setState({coAuthorInput: inputValue}, () => {
        console.log('set coAuthorInput', this.state.coAuthorInput);
      });
    }
    return inputValue;
  }
  ...
  renderCoAuthorDialog() {

    if (!this.state.showCoAuthorDialog) {
      return null;
    }

    return (
      <CoAuthorDialog
        ref={c => { this.refCoAuthorDialog = c; }}
  ...
  submitNewCoAuthor(newAuthor) {
    ...
    this.setState({showCoAuthorDialog: false});
  }
  ...
  matchAuthors(authors, filterText, selectedAuthors) {
    return authors
      .filter(author => {
        const isAlreadySelected = selectedAuthors && selectedAuthors.includes(author);
        const isNewAuthorItem = author === NEW_AUTHOR;
        const matchesFilter = `${author.name}${author.email}`.toLowerCase().indexOf(filterText.toLowerCase()) !== -1;
        return (!isAlreadySelected && matchesFilter) || isNewAuthorItem;
      });
  }
  ...
  @autobind
  renderCoAuthorListItem(author, index) {
    const isNewAuthorItem = index === 0;
    return (
      <div className={cx('github-CommitView-coAuthorEditor-selectListItem', {'new-author': isNewAuthorItem})}>
  ...
  onSelectedCoAuthorsChanged(selectedCoAuthors) {
    const newAuthor = !!selectedCoAuthors.find(author => author === NEW_AUTHOR);

    if (newAuthor) {
      this.setState({showCoAuthorDialog: true}, () => {
        this.setState({coAuthorInput: ''});
      });
    } else {
