...
export default class CommitView extends React.Component {
  ...
  static propTypes = {
    ...
    previewCommit: PropTypes.func.isRequired,
  };
  ...
  render() {
    ...
    return (
      <div className="github-CommitView" ref={this.refRoot.setter}>
        ...
        <div className="github-CommitView-commitPreview button-wrapper">
          <button
            ref={this.refCommitPreviewButton.setter}
            className="github-CommitView-button btn"
            disabled={!this.props.stagedChangesExist}
            onClick={this.props.previewCommit}>
            Preview Commit
          </button>
        </div>
  ...
  retreatFocus(event) {
    ...
    switch (current) {
    case f.COMMIT_BUTTON:
      next = this.props.isMerging ? f.ABORT_MERGE_BUTTON : f.COAUTHOR_INPUT;
      break;
    case f.ABORT_MERGE_BUTTON:
