  ...
  renderHardWrapIcons() {
    ...
    return (
      <div onclick={this.toggleHardWrap} className="github-CommitView-hardwrap">
        <span className={cx('icon', 'no-hardwrap', 'icon-hardwrap-disabled', {hidden: notApplicable || hardWrap})} />
        <span className={cx('icon', 'hardwrap', 'icon-hardwrap-enabled', {hidden: notApplicable || !hardWrap})} />
      </div>
    );
  }
