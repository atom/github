...
export default class FilePatchView extends React.Component {
  ...
  renderCommands() {
    ...
    if (this.props.filePatch.didChangeExecutableMode()) {
      const command = this.withSelectionMode({
        unstaged: () => 'github:stage-file-mode-change',
        staged: () => 'github:unstage-file-mode-change',
      });
      stageModeCommand = <Command command={command} callback={this.props.toggleModeChange} />;
    }

    let stageSymlinkCommand = null;
    if (this.props.filePatch.hasSymlink()) {
      const command = this.withSelectionMode({
        unstaged: () => 'github:stage-symlink-change',
        staged: () => 'github:unstage-symlink-change',
      });
      stageSymlinkCommand = <Command command={command} callback={this.props.toggleSymlinkChange} />;
    }
    ...
    return (
      <Fragment>
        ...
        <Commands registry={this.props.commands} target="atom-workspace">
          <Command command="github:undo-last-discard-in-diff-view" callback={this.undoLastDiscardFromCommand} />
        </Commands>
      </Fragment>
    );
  }
