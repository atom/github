...
export default class FilePatchView extends React.Component {
  ...
  renderExecutableModeChange(unstaged) {
    ...
    return (
      <div className="github-FilePatchView-mode-change">
        <span>
          File changed mode from <code>{executableModeChange.oldMode}</code>
  ...
  renderSymlinkChange(unstaged) {
    ...
    if (oldSymlink && !newSymlink) {
      return (
        <div className="github-FilePatchView-mode-change">
          <span>
            Symlink to <code>{oldSymlink}</code> deleted.
          </span>
          <button
            className={cx('btn', 'icon', {'icon-move-down': unstaged, 'icon-move-up': !unstaged})}
      ...
      return (
        <div className="github-FilePatchView-mode-change">
          <span>
            Symlink to <code>{newSymlink}</code> created.
          </span>
          <button
            className={cx('btn', 'icon', {'icon-move-down': unstaged, 'icon-move-up': !unstaged})}
            onClick={this.stageOrUnstageSymlinkChange}>
      ...
      return (
        <div className="github-FilePatchView-mode-change">
          <span>
            Symlink changed from <code>{oldSymlink}</code> to <code>{newSymlink}</code>.
          </span>
          <button
            className={cx('btn', 'icon', {'icon-move-down': unstaged, 'icon-move-up': !unstaged})}
            onClick={this.stageOrUnstageSymlinkChange}>
  ...
  async didClickStageButtonForHunk(hunk) {
    // For the case when there is a symlink change AND added/deleted file, we must first stage/unstage the symlink
    // before staging/unstaging the lines
    const {symlinkChange, stagingStatus} = this.props;
    const unstagedSymlinkChangeWithAddedLines = symlinkChange && symlinkChange.filePatchStatus === 'added' &&
      stagingStatus === 'unstaged';
    const stagedSymlinkChangeWithDeletedLines = symlinkChange && symlinkChange.filePatchStatus === 'deleted' &&
      stagingStatus === 'staged';
    if (unstagedSymlinkChangeWithAddedLines || stagedSymlinkChangeWithDeletedLines) {
      await this.stageOrUnstageSymlinkChange({noSwitchboard: true});
    }
  ...
  @autobind
  stageOrUnstageSymlinkChange(options) {
    return this.props.attemptSymlinkStageOperation(options);
  }
