...
export default class FilePatchView extends React.Component {
  static propTypes = {
    ...
    surfaceFile: PropTypes.func.isRequired,
  ...
  renderCommands() {
    ...
    if (this.props.multiFilePatch.anyHaveSymlink()) {
      const command = this.props.stagingStatus === 'unstaged'
    ...
    return (
      <Commands registry={this.props.commands} target={this.refRoot}>
        ...
        <Command command="github:surface-file" callback={this.props.surfaceFile} />
  ...
  renderHunkHeaders(filePatch) {
    ...
    const selectedHunks = new Set(
      Array.from(this.props.selectedRows, row => this.multiFilePatch.getHunkAt(row)),
    );
  ...
  toggleHunkSelection(filePatch, hunk, containsSelection) {
    if (containsSelection) {
      ...
      return this.props.toggleRows(filePatch, changeRows, 'hunk', {eventSource: 'button'});
    }
  }

  discardHunkSelection(filePatch, hunk, containsSelection) {
    if (containsSelection) {
      return this.props.discardRows(
        filePatch,
        this.props.selectedRows,
      ...
      return this.props.discardRows(filePatch, changeRows, 'hunk', {eventSource: 'button'});
    }
  }
  ...
  didConfirm() {
    return Promise.all(
      Array.from(
        this.getSelectedFilePatches(),
        filePatch => this.props.toggleRows(filePatch, this.props.selectedRows, this.props.selectionMode),
      ),
    );
  }
