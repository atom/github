...
export default class FilePatchView extends React.Component {
    ...
    return (
      <div
        ...
        <main className="github-FilePatchView-container">
          {this.renderModeChange(unstaged)}
  ...
  @autobind
  renderModeChange(unstaged) {
    const {modeChange} = this.props;
    if (!modeChange) { return null; }
    const symlinkDeleted = modeChange.oldMode === '120000';
    const symlinkAdded = modeChange.newMode === '120000';

    if (symlinkDeleted) {
      return (
        <div className="github-FilePatchView-mode-change">
          <span>
            Symlink to <code>{modeChange.symlink}</code> deleted and replaced with the following contents:
          </span>
          <button
            className={cx('btn', 'icon', {'icon-move-down': unstaged, 'icon-move-up': !unstaged})}
    ...
    } else if (symlinkAdded) {
      return (
        <div className="github-FilePatchView-mode-change">
          <span>
            File contents deleted and replaced with symlink to <code>{modeChange.symlink}</code>
          </span>
          <button
            className={cx('btn', 'icon', {'icon-move-down': unstaged, 'icon-move-up': !unstaged})}
            onClick={this.stageOrUnstageModeChange}>
      ...
      return (
        <div className="github-FilePatchView-mode-change">
          <span>
            File changed mode from <code>{modeChange.oldMode}</code> to <code>{modeChange.newMode}</code>
          </span>
          <button
            className={cx('btn', 'icon', {'icon-move-down': unstaged, 'icon-move-up': !unstaged})}
            onClick={this.stageOrUnstageModeChange}>
