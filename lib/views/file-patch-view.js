...
import {fromBufferPosition} from '../models/marker-position';
...
export default class FilePatchView extends React.Component {
      ...
      <div
        ...
        <main className="github-FilePatchView-container">
          <AtomTextEditor
            ...
            <Marker invalidate="never" position={fromBufferPosition([0, 0])}>
  ...
  renderHunkHeaders() {
    ...
    const hunkStartPositions = this.state.presentedFilePatch.getHunkStartPositions();
    ...
    return this.props.filePatch.getHunks().map((hunk, index) => {
      ...
      const onDidChange = event => {
        hunkStartPositions[index] = event.newPosition;
      };

      return (
        <Marker
          key={`hunkHeaderView-${index}`}
          position={hunkStartPositions[index]}
          invalidate="never"
    ...
    return (
      <MarkerLayer>
        {positions.map((position, index) => {
          const onDidChange = event => {
            positions[index] = event.newPosition;
          };
        ...
        {gutter && (
          <Fragment>
            <Decoration type="line-number" gutterName="old-line-numbers" className={lineClass} />
            <Decoration type="line-number" gutterName="new-line-numbers" className={lineClass} />
          </Fragment>
        )}
      </MarkerLayer>
    );
