...
export default class FilePatchView extends React.Component {
    ...
    return (
      <Commands registry={this.props.commands} target={this.refRoot}>
        <Command
          ...
          callback={() => this.props.toggleRows(this.props.selectedRows, this.props.selectionMode)}
        />
        <Command
  ...
  didToggleSelectionMode() {
    const selectedRows = this.refEditor.map(editor => {
      return editor.getSelectedBufferRanges().reduce((acc, range) => {
        acc.push(...range.getRows());
        return acc;
      }, []);
    }).getOr([]);
    ...
    const selectedHunks = new Set(
      Array
        .from(selectedRows, row => this.props.filePatch.getHunkAt(row))
        .filter(Boolean),
    );

    this.withSelectionMode({
      line: () => {
        const hunkRanges = Array.from(selectedHunks, hunk => hunk.getRange());
        this.nextSelectionMode = 'hunk';
