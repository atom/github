...
export default class FilePatchView extends React.Component {
  ...
  constructor(props) {
    ...
    autobind(
      this,
      'didMouseDownOnLineNumber', 'didMouseMoveOnLineNumber', 'didMouseUp', 'oldLineNumberLabel', 'newLineNumberLabel',
    );
  ...
  renderSelectedLines() {
    return (
      <MarkerLayer>
        {Array.from(this.state.selectedLines, line => {
          const position = this.state.presentedFilePatch.getPositionForLine(line);
          return <Marker key={line.getDiffLineNumber()} bufferPosition={position} />;
        })}
      </MarkerLayer>
    );
  }
