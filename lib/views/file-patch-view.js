...
export default class FilePatchView {
  constructor ({filePatch, repository, stagingStatus, selectionMode, registerHunkView}) {
    ...
    this.subscriptions = atom.commands.add(this.element, {
      ...
      'core:move-up': this.focusPreviousHunk.bind(this),
      'core:move-down': this.focusNextHunk.bind(this),
      'core:confirm': this.confirmHunk.bind(this)
    })
  }
  ...
  confirmHunk () {
    return this.didConfirmHunk(this.selectedHunk)
  }

  focusHunk (hunk, index) {
    this.selectedHunk = hunk
    this.selectedHunkIndex = index
    this.setInitialSelection(this.selectedHunk)
  ...
  focusPreviousHunk () {
    ...
    if (--index > -1) return this.focusHunk(hunks[index], index)
  }

  focusNextHunk () {
    const hunks = this.filePatch.getHunks()
    let index = hunks.indexOf(this.selectedHunk)
    if (++index < hunks.length) return this.focusHunk(hunks[index], index)
  }
      ...
      <div className='git-FilePatchView' tabIndex='-1'>{this.filePatch.getHunks().map((hunk) => {
        ...
        return (
          <HunkView
            ...
            didClickStageButton={() => this.didConfirmHunk(hunk)}
