...
export default class FilePatchView {
  ...
  didClickStageButtonForHunk(hunk) {
    if (this.selection.getSelectedHunks().has(hunk)) {
      const selectedLines = this.selection.getSelectedLines();
      const selectedRemainingLines = this.selection.getChangedLines().length === selectedLines.size;
      return this.props.attemptLineStageOperation(selectedLines, selectedRemainingLines);
    } else {
      const selectedRemainingHunk = this.selection.getHunks().length === 1;
      this.selection.selectHunk(hunk);
      return this.props.attemptHunkStageOperation(hunk, selectedRemainingHunk);
    }
  }
