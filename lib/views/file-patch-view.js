...
export default class FilePatchView extends React.Component {
  static propTypes = {
    ...
    modeChange: PropTypes.shape({oldMode: PropTypes.string.isRequired, newMode: PropTypes.string.isRequired}),
  ...
  renderHunks() {
    ...
    if (symlinkChange && symlinkChange.status !== 'typechange') { return null; }
    ...
    return (
      <div
        ...
        <main className="github-FilePatchView-container">
          {this.renderExecutableModeChange(unstaged)}
          {this.renderSymlinkChange(unstaged)}
  ...
  renderExecutableModeChange(unstaged) {
    ...
    if (!executableModeChange) { return null; }
    return (
      <div className="github-FilePatchView-mode-change">
        <span>
          File changed mode from <code>{executableModeChange.oldMode}</code> to <code>{executableModeChange.newMode}</code>
        </span>
        <button
          className={cx('btn', 'icon', {'icon-move-down': unstaged, 'icon-move-up': !unstaged})}
