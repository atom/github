  ...
  mousedownOnHeader(event, hunk) {
    if (event.ctrlKey || event.metaKey) {
      if (this.selection.getMode() === 'hunk') {
        ...
        const lines = hunk.getLines().filter(l => l.isChanged());
        this.selection.addOrSubtractLineSelection(lines[0]);
        this.selection.selectLine(lines[lines.length - 1], true);
      }
    } else {
      if (this.selection.getMode() === 'hunk') {
        ...
        this.selection.addOrSubtractLineSelection(lines[0]);
        this.selection.selectLine(lines[lines.length - 1], true);
        hunk.getLines().forEach(line => this.selection.selectLine(line, event.shiftKey));
      }
    }
