...
export default class FilePatchView extends React.Component {
  static propTypes = {
    ...
    discardHunk: PropTypes.func.isRequired,
  }
  ...
  constructor(props) {
    ...
    autobind(
      ...
      'didConfirm', 'didOpenFile', 'observeSelections', 'oldLineNumberLabel', 'newLineNumberLabel',
      'selectNextHunk', 'selectPreviousHunk',
    );
  ...
  renderNonEmptyPatch() {
    return (
      <AtomTextEditor
        ...
        autoHeight={false}
        readOnly={true}
        observeSelections={this.observeSelections}
    ...
    return (
      <Fragment>
        ...
        */}
        <MarkerLayer>
          {this.props.filePatch.getHunks().map((hunk, index) => {
            const buttonSuffix = ' Selected Line';
            // FIXME Add "s" when plural
  ...
  didMouseDownOnHeader(event, hunk) {
    this.handleSelectionEvent(event.domEvent, hunk.getBufferRange());
  }
  ...
  didConfirm() {
    const [firstHunk] = this.state.selectedHunks;
    if (!firstHunk) {
      return;
    }

    this.toggleSelection(firstHunk);
  }
  ...
  getSelectedBufferRanges() {
    return this.refEditor.map(editor => {
      return editor.getSelections().map(selection => selection.getBufferRange());
    }).getOr([]);
  }

  observeSelections(selection) {
    const selectionSubs = new CompositeDisposable(
      selection.onDidChangeRange(this.didChangeSelectedRanges),
    ...
    this.didChangeSelectedRanges();
  }

  didChangeSelectedRanges() {
    this.props.selectedRangesChanged(this.getSelectedBufferRanges());
  }
  ...
  toggleSelection(hunk) {
    if (this.state.selectedHunks.has(hunk)) {
      ...
      return this.props.toggleHunk(hunk);
    }
  }
  ...
  discardSelection(hunk) {
    if (this.state.selectedHunks.has(hunk)) {
      ...
      return this.props.discardHunk(hunk);
    }
  }
