      ...
      <Fragment>
        ...
        />
        {
          this.state.mentions.map(m =>
            <Tooltip className='github-Popover' showDelay={0} manager={atom.tooltips} target={FakeRef(m)}>
        ...
        }
        {
          this.state.issueLinks.map(a =>
            <Tooltip className='github-Popover' showDelay={0} manager={atom.tooltips} target={FakeRef(a)}>
...
const FakeRef = target => ({
  isEmpty: () => false,
  get: () => target,
  observe: cb => cb(target),
  setter: () => {},
})
