...
export class BareGithubDotcomMarkdown extends React.Component {
  ...
  domDidChange = _record =>
    this.setState({
      mentions: [...this.component.querySelectorAll('.user-mention')].map(withKey),
      issueLinks: [...this.component.querySelectorAll('.issue-link')].map(withKey),
    })
      ...
      <Fragment>
        ...
        />
        {
          this.state.mentions.map(m =>
            <Tooltip key={m.key} className='github-Popover' showDelay={0} manager={atom.tooltips} target={RefHolder.constant(m)}>
        ...
        }
        {
          this.state.issueLinks.map(a =>
            <Tooltip key={a.key} className='github-Popover' showDelay={0} manager={atom.tooltips} target={RefHolder.constant(a)}>
...
let nextKey = 0
const withKey = domNode => domNode.key || (domNode.key = nextKey++)
