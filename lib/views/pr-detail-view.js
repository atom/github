...
export class BarePullRequestDetailView extends React.Component {
  static propTypes = {
    ...
    pullRequest: PropTypes.shape({
      ...
      state: PropTypes.oneOf(['OPEN', 'CLOSED', 'MERGED']).isRequired,
  ...
  renderPrMetadata(pullRequest, repo) {
    return (
      <span className="github-IssueishDetailView-meta">
        <code className="github-IssueishDetailView-baseRefName">
          {pullRequest.isCrossRepository
            ? `${repo.owner.login}/${pullRequest.baseRefName}`
            : pullRequest.baseRefName}
        </code>
        {' ‹ '}
        <code className="github-IssueishDetailView-headRefName">
          {pullRequest.isCrossRepository
            ? `${pullRequest.author.login}/${pullRequest.headRefName}`
      ...
      <Tabs onSelect={this.recordOpenTabEvent}>
        <TabList className="github-IssueishDetailView-tablist">
          ...
          <Tab className="github-IssueishDetailView-tab">
            ...
            />
            Commits
            <span className="github-IssueishDetailView-tab-count">
  ...
  renderCheckoutButton() {
    ...
    if (!checkoutOp.isEnabled()) {
      ...
      extraClass =
        'github-IssueishDetailView-checkoutButton--' +
        reason.when({
          disabled: 'disabled',
          busy: 'busy',
          current: 'current',
      ...
      fragment prDetailView_pullRequest on PullRequest
        ...
        ... on PullRequest {
          ...
          ...prCommitsView_pullRequest
            @arguments(commitCount: $commitCount, commitCursor: $commitCursor)
