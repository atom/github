...
export default class FilePatchSelection {
  ...
  updateHunks (hunks) {
    ...
    if (this.hunks.length > 0) {
      if (this.mode === 'hunk') {
        const newHunkSelectionIndex = Math.min(this.hunkSelections[0].head, this.hunks.length - 1)
        this.hunkSelections = [{head: newHunkSelectionIndex, tail: newHunkSelectionIndex}]
      } else {
        let newLineSelectionLocation
        ...
        hunkLoop: for (let hunkIndex = 0; hunkIndex < this.hunks.length; hunkIndex++) {
          ...
          for (let lineIndex = 0; lineIndex < hunk.lines.length; lineIndex++) {
            const line = hunk.lines[lineIndex]
            if (line.isChanged()) {
              newLineSelectionLocation = {hunkIndex, lineIndex}
              if (changedLineIndex === previousSelectionChangedLineIndex) break hunkLoop
              changedLineIndex++
            }
          }
        }
        this.lineSelections = [{head: newLineSelectionLocation, tail: newLineSelectionLocation}]
      }
    } else {
      this.hunkSelections = []
      this.lineSelections = []
