...
export default class FilePatchSelection {
  ...
  getHeadHunk () {
    if (this.mode === 'hunk') {
      return this.hunksSelection.getHeadItem()
    }
  }
  ...
  getHeadLine () {
    if (this.mode === 'line') {
      return this.linesSelection.getHeadItem()
    }
  }
  ...
  updateHunks (newHunks) {
    ...
    let newSelectedHunk
    if (oldHunks.length > 0 && newHunks.length > 0) {
      const oldSelectionStartIndex = this.hunksSelection.getMostRecentSelectionStartIndex()
      newSelectedHunk = newHunks[Math.min(newHunks.length - 1, oldSelectionStartIndex)]
    }
    this.hunksSelection.setItems(newHunks)
    if (newSelectedHunk) this.hunksSelection.selectItem(newSelectedHunk)
