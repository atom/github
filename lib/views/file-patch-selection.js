...
export default class FilePatchSelection {
  ...
  selectAllHunks () {
    this.selectFirstHunk()
    this.selectLastHunk(true)
  }

  selectFirstHunk (preserveTail) {
    this.selectHunk(this.hunks[0], preserveTail)
  }

  selectLastHunk (preserveTail) {
    this.selectHunk(this.hunks[this.hunks.length - 1], preserveTail)
  }

  selectAllLines () {
    this.selectFirstLine()
    this.selectLastLine(true)
  }
  ...
  selectFirstLine (preserveTail) {
    for (let i = 0; i < this.lines.length; i++) {
      ...
      if (line.isChanged()) {
        this.selectLine(line, preserveTail)
        break
      }
    }
  }
  ...
  getHeadHunk () {
    if (this.mode === 'hunk' && this.selections.length > 0) {
      return this.hunks[this.selections[0].head]
    }
  }

  getHeadLine () {
    if (this.mode === 'line' && this.selections.length > 0) {
      return this.lines[this.selections[0].head]
    }
  }
