...
export default class FilePatchSelection {
  ...
  updateHunks (hunks) {
    ...
    let foundNewSelectionLine = false
    outerLoop: for (let hunk of this.hunks) {
      for (let line of hunk.lines) {
        if (line.isChanged()) {
          if (changedLineIndex === changedLineIndexOfPreviousSelection) {
            foundNewSelectionLine = true
            break outerLoop
          }
          changedLineIndex++
        }
    ...
    if (!foundNewSelectionLine) lineIndexOfNewSelection--
