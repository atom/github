...
export class PrCommitsView extends React.Component {
  ...
  render() {
    return this.props.pullRequest.commits.edges.map(edge => {
      ...
      return <PrCommitView key={commit.abbreviatedOid} committerAvatarUrl={commit.committer.avatarUrl} date={commit.committer.date} messageBody={commit.messageBody} messageHeadline={commit.messageHeadline} abbreviatedOid={commit.abbreviatedOid} url={commit.committer.url} committerName={commit.committer.name}/>;
    });
  }
}
