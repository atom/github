    ...
    return (
      <div {...others} className={`git-FilePatchListView-item is-${fileStatus} ${className}`}>
        ...
        <span className={'git-FilePatchListView ours-theirs-info'}>
          <span className={`git-FilePatchListView-icon icon icon-diff-${oursStatus} status-${oursStatus}`} />
          <span className={`git-FilePatchListView-icon icon icon-diff-${theirsStatus} status-${theirsStatus}`} />
        </span>
      </div>
    )
  }
