...
export default class PullRequestCommentsView extends React.Component {
  ...
  render() {
    ...
    this.props.pullRequest.reviews.edges.forEach(({node}) => {
      ...
      review.comments.edges.forEach(({node}) => {
        const comment = node;
        ...
        if (!comment.replyTo) {
          ...
          if (!commentsByRootCommentId.get(comment.replyTo.id)) {
            commentsByRootCommentId.set(comment.replyTo.id, [comment]);
          } else {
    ...
    return [...commentsByRootCommentId].reverse().map(([rootCommentId, comments]) => {
      ...
      return (
        <Marker key={`pr-comment-${rootCommentId + count++}`} bufferRange={range} invalidate="never">
