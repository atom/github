    ...
    return (
      <div className="github-PrCommitView-container">
        <div>
          ...
          {this.props.messageBody ? <button className="github-PrCommitView-btn" onClick={this.toggleShowCommitMessageBody}>{this.state.showMessageBody ? 'hide' : 'show'} more...</button> : null}
          <div>
          <img className="github-PrCommitView-avatar" src={this.props.committerAvatarUrl}
            alt={'commiter avatar'} title={'committer avatar'}
          />
            <span className="github-PrCommitView-text">committed by {this.props.committerName} {this.humanizeTimeSince(this.props.date)}</span>
          </div>
          {this.state.showMessageBody ? <div><code>{emojify(this.props.messageBody)}</code></div> : null}
        </div>
        <code><a href={this.props.url} title={`open commit ${this.props.abbreviatedOid} on GitHub.com`}>{this.props.abbreviatedOid}</a></code>
      </div>
    );
  }
}
