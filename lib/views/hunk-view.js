...
export default class HunkView {
  constructor ({hunk, isSelected, selectedLines, didSelectLines, didClickStageButton, stageButtonLabelPrefix, registerView}) {
    this.hunk = hunk
  ...
  update ({hunk, isSelected, selectedLines, didSelectLines, didClickStageButton, stageButtonLabelPrefix, registerView}) {
    this.hunk = hunk
    this.isSelected = isSelected
    this.selectedLines = selectedLines
    this.didSelectLines = didSelectLines
    ...
    return (
      <div className={`git-HunkView ${hunkSelectedClass}`}>
        ...
        {this.hunk.getLines().map((line) => {
          ...
          const statusClass = 'is-' + line.getStatus()
          return (
            <div className={`git-HunkView-line ${statusClass} ${lineSelectedClass}`}
                 onmousedown={() => this.onMouseDown(line)}
                 onmousemove={() => this.onMouseMove(line)}
                 onmouseup={() => this.onMouseUp(line)}>
              <div className='git-HunkView-oldLineNumber'>{oldLineNumber}</div>
              <div className='git-HunkView-newLineNumber'>{newLineNumber}</div>
