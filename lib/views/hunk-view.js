...
import SplitDiffLineGrouper from '../models/split-diff-line-grouper';
  ...
  render() {
    ...
    return (
      <div className={`github-HunkView ${hunkModeClass} ${hunkSelectedClass}`} ref={e => { this.element = e; }}>
        ...
        {this.props.splitMode ? this.renderSplitModeLines() : this.renderUnifiedModeLines()}
      </div>
    );
  }

  renderSplitModeLines() {
    const lines = this.props.hunk.getLines();
    const lineElements = [];
    const groups = new SplitDiffLineGrouper(lines).getGroups();
    groups.forEach((group, idx) => {
      const len = Math.max(group.left.length, group.right.length);
      for (let i = 0; i < len; i++) {
        const leftLine = group.left[i];
        const rightLine = group.right[i];
        const leftElem = leftLine && (
          <LineView
            line={leftLine}
            showNewLineNumber={false}
            isSelected={this.props.selectedLines.has(leftLine)}
        ...
        const rightElem = rightLine && (
          <LineView
            line={rightLine}
            showOldLineNumber={false}
            isSelected={this.props.selectedLines.has(rightLine)}
            registerLineElement={this.registerLineElement}
            mousedown={this.mousedownOnLine}
            mousemove={this.mousemoveOnLine}
            contextMenuOnItem={(e, clickedLine) => this.props.contextMenuOnItem(e, this.props.hunk, clickedLine)}
    ...
    return <div className="github-HunkView-split-mode-lines">{lineElements}</div>;
  }

  renderUnifiedModeLines() {
    return this.props.hunk.getLines().map((line, idx) =>
      <LineView
        key={idx}
        line={line}
...
class SplitLine extends React.Component {
  static propTypes = {
    leftLine: PropTypes.element,
    rightLine: PropTypes.element,
  }

  render() {
    const nullDiv = (
      <div className="github-HunkView-line is-empty-space">
        <div className="github-HunkView-lineNumber" />
        <div className="github-HunkView-lineContent" />
      </div>
    );

    return (
      <div className="github-HunkView-split-line">
        <div className="left-line">{this.props.leftLine || nullDiv}</div>
        <div className="right-line">{this.props.rightLine || nullDiv}</div>
      </div>
    );
  }
}
...
class LineView extends React.Component {
  static propTypes = {
    ...
    showOldLineNumber: PropTypes.bool,
    showNewLineNumber: PropTypes.bool,
    isSelected: PropTypes.bool.isRequired,
    ...
    return (
      <ContextMenuInterceptor onWillShowContextMenu={event => this.props.contextMenuOnItem(event, line)}>
        <div
          ...
          {this.props.showOldLineNumber && <div className="github-HunkView-lineNumber is-old">{oldLineNumber}</div>}
          {this.props.showNewLineNumber && <div className="github-HunkView-lineNumber is-new">{newLineNumber}</div>}
          <div className="github-HunkView-lineContent">
