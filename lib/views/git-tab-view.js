...
export default class GitTabView extends React.Component {
  ...
  static propTypes = {
    ...
    didSelectFilePath: PropTypes.func.isRequired,
  };
    ...
    if (this.props.repository.isTooLarge()) {
      ...
      return (
        <div
          ...
          <EtchWrapper ref={c => { this.refStagingView = c; }}>
            <StagingView
  ...
  rememberFocus(event) {
    ...
    if (!currentFocus && this.refCommitViewController) {
      currentFocus = this.refCommitViewController.rememberFocus(event);
    }
  ...
  setFocus(focus) {
    ...
    if (this.refCommitViewController) {
      if (this.refCommitViewController.setFocus(focus)) {
        return true;
      }
    }
  ...
  @autobind
  advanceFocus(evt) {
    if (!this.refStagingView.activateNextList()) {
      if (this.refCommitViewController.setFocus(GitTabView.focus.EDITOR)) {
        evt.stopPropagation();
      }
    } else {
      evt.stopPropagation();
