      ...
      return (
        <div className="github-Panel is-empty" tabIndex="-1">
          <div ref="noRepoMessage" className="github-Panel no-repository">
            ...
            <button onclick={this.initializeRepo} disabled={inProgress} className="btn btn-primary icon icon-repo-create">
              {inProgress ? 'Initializing...' : 'Initialize'}
            </button>
            <hr />
            <div className="initialize-repo-description">Add a new project by cloning a git repository</div>
            <button className="btn btn-primary icon icon-repo-clone">Clone</button>
          </div>
        </div>
      );
    } else {
