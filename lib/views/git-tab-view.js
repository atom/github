...
export default class GitTabView {
  ...
  advanceFocus() {
    if (!this.refs.stagingView.activateNextList()) {
      this.refs.commitViewController.restoreFocus(GitTabView.focus.EDITOR);
    }
  }
  ...
  retreatFocus() {
    ...
    if (commitViewController.element.contains(document.activeElement)) {
      if (stagingView.activateLastList()) {
        this.restoreFocus(GitTabView.focus.STAGING);
      }
    } else {
