...
export default class MultiFilePatchView extends React.Component {
          ...
          <Decoration type="block">
            <Fragment>
              ...
              <FilePatchHeaderView
                ...
                openFile={() => this.didOpenFile(filePatch)}
  ...
  didOpenFile(selectedFilePatch) {
    ...
    const filePatchesWithCursors = new Set(cursorsByFilePatch.values());
    if (!filePatchesWithCursors.has(selectedFilePatch)) {
      const [firstHunk] = selectedFilePatch.getHunks();
      const cursorRow = firstHunk ? firstHunk.getNewStartRow() - 1 : 0;
      return this.props.openFile(selectedFilePatch, [[cursorRow, 0]], {pending: true});
    }
