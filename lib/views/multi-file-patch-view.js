...
export default class MultiFilePatchView extends React.Component {
  static propTypes = {
    ...
    addCommentAtRow: PropTypes.func,
  ...
  renderReviewCommentIcons() {
    if (this.props.itemType !== IssueishDetailItem || this.props.reviewCommentsLoading) {
      return null;
    }

    const lastRow = this.props.multiFilePatch.getBuffer().getEndPosition().row;
    const commentRows = new Set();
    const commentDecorations = [];

    for (const {comments, thread} of this.props.reviewCommentThreads) {
    ...
    for (let row = 0; row <= lastRow; row++) {
      if (commentRows.has(row)) {
        continue;
      }

      commentDecorations.push(
        <Marker
          key={`github-add-comment-gutter-decoration-${row}`}
          exclusive={true}
          invalidate="never"
          bufferRange={Range.fromObject([[row, 0], [row, Infinity]])}>
          <Decoration
            type="gutter"
            gutterName="github-comment-icon"
            className="github-addCommentGutterIcon"
            omitEmptyLastRow={false}>
            <button className="icon icon-comment-add" onClick={() => this.props.addCommentAtRow(row)} />
          </Decoration>
        </Marker>,
      );
    }
