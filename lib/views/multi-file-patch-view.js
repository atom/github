...
export default class MultiFilePatchView extends React.Component {
    ...
    this.subs.add(
      ...
      this.refEditorElement.observe(element => {
        this.props.refInitialFocus &&
          this.props.refInitialFocus.setter(element);
      }),
    );
  }
  ...
  componentDidMount() {
    ...
    this.subs.add(
      this.props.config.onDidChange('github.showDiffIconGutter', ({newValue}) =>
        this.forceUpdate(),
      ),
    );
  }
  ...
  componentDidUpdate(prevProps, prevState, newSelectionRange) {
    if (prevProps.refInitialFocus !== this.props.refInitialFocus) {
      ...
      this.props.refInitialFocus &&
        this.refEditorElement.map(this.props.refInitialFocus.setter);
    }
      ...
      this.refEditor.map(editor => {
        if (this.props.selectionMode === 'line') {
          ...
          const nextHunks = new Set(
            Range.fromObject(newSelectionRange)
              .getRows()
  ...
  render() {
    const rootClass = cx(
      ...
      {
        [`github-FilePatchView--${this.props.stagingStatus}`]: this.props
          .stagingStatus,
      },
  ...
  renderCommands() {
    ...
    if (this.props.multiFilePatch.didAnyChangeExecutableMode()) {
      ...
      stageModeCommand = (
        <Command command={command} callback={this.didToggleModeChange} />
      );
    }
    ...
    if (this.props.multiFilePatch.anyHaveTypechange()) {
      ...
      stageSymlinkCommand = (
        <Command command={command} callback={this.didToggleSymlinkChange} />
      );
    }
    ...
    return (
      ...
      >
        ...
        {this.props.multiFilePatch
          .getFilePatches()
          .map(this.renderFilePatchDecorations)}

        {this.renderLineDecorations(
          Array.from(this.props.selectedRows, row =>
            Range.fromObject([[row, 0], [row, Infinity]]),
          ),
      ...
      return (
        <PullRequestsReviewsContainer
          ...
          getBufferRowForDiffPosition={
            this.props.multiFilePatch.getBufferRowForDiffPosition
          }
        ...
        <Marker invalidate="never" bufferRange={filePatch.getStartRange()}>
          ...
          >
            <FilePatchHeaderView
              ...
              newPath={
                filePatch.getStatus() === 'renamed'
                  ? filePatch.getNewPath()
                  : null
              }
              ...
              diveIntoMirrorPatch={() =>
                this.props.diveIntoMirrorPatch(filePatch)
              }
  ...
  renderHunkHeaders(filePatch) {
    const toggleVerb =
      this.props.stagingStatus === 'unstaged' ? 'Stage' : 'Unstage';
    const selectedHunks = new Set(
      Array.from(this.props.selectedRows, row =>
        this.props.multiFilePatch.getHunkAt(row),
      ),
    );
      ...
      <Fragment>
        <MarkerLayer>
          {filePatch.getHunks().map((hunk, index) => {
            ...
            const isSelected =
              this.props.selectionMode === 'hunk' && selectedHunks.has(hunk);
              ...
              >
                ...
                >
                  <HunkHeaderView
                    ...
                    discardSelection={() =>
                      this.discardHunkSelection(hunk, containsSelection)
                    }
  ...
  handleSelectionEvent(event, rangeLike, opts) {
    ...
    if (
      event.metaKey ||
      /* istanbul ignore next */ (event.ctrlKey && isWindows)
    ) {
      this.refEditor.map(editor => {
          ...
          if (selection.intersectsBufferRange(range)) {
            ...
            if (!range.start.isEqual(selectionRange.start)) {
              ...
              if (range.start.column === 0) {
                const lastColumn = editor
                  .getBuffer()
                  .lineLengthForRow(range.start.row - 1);
            ...
            if (!range.end.isEqual(selectionRange.end)) {
              ...
              const lastColumn = editor
                .getBuffer()
                .lineLengthForRow(range.end.row);
  ...
  didToggleSelectionMode() {
    ...
    this.withSelectionMode({
      line: () => {
        ...
        this.nextSelectionMode = 'hunk';
        this.refEditor.map(editor =>
          editor.setSelectedBufferRanges(hunkRanges),
        );
      },
      hunk: () => {
        ...
        for (const hunk of selectedHunks) {
          ...
          if (
            firstChange &&
            (!firstChangeRow ||
              firstChange.getStartBufferRow() < firstChangeRow)
          ) {
        ...
        this.refEditor.map(editor => {
          editor.setSelectedBufferRanges([
            [[firstChangeRow, 0], [firstChangeRow, Infinity]],
          ]);
          return null;
        });
      },
  ...
  didChangeSelectedRows() {
    ...
    const nextCursorRows = this.refEditor
      ...
      .getOr([]);
    const hasMultipleFileSelections = this.props.multiFilePatch.spansMultipleFiles(
      nextCursorRows,
    );
  ...
  isChangeRow(bufferRow) {
    ...
    return changeLayers.some(
      layer => layer.findMarkers({intersectsRow: bufferRow}).length > 0,
    );
  }
  ...
  pad(num) {
    ...
    if (num === null) {
      ...
      return (
        NBSP_CHARACTER.repeat(maxDigits - num.toString().length) +
        num.toString()
      );
    }
  }
