...
import PullRequestReviewCommentThreadView from './pr-review-comment-thread-view';
...
export default class MultiFilePatchView extends React.Component {
  static propTypes = {
    ...
    commentCounter: PropTypes.shape({
    ...
    if (this.props.itemType === IssueishDetailItem) {
      ...
      return (
        <AggregatedReviewsContainer
          ...
          handleResults={this.handleReviewResults}
          forceRerender={{}}>
          ...
          {({errors, commentThreads, loading}) => {
            ...
            const nonEmptyThreads = commentThreads.filter(({comments}) => comments.length > 0);

            return nonEmptyThreads.map(({comments}) => (
              <PullRequestReviewCommentThreadView
                key={`pr-comment-${comments[0].id}`}
                comments={comments}
  ...
  handleReviewResults = ({errors, commentThreads, loading}) => {
    if (errors.length > 0 || loading) {
      return;
    }

    const threads = commentThreads
      .filter(each => each.comments.length > 0)
      .map(each => each.thread);

    this.props.commentCounter.countAll(threads);
  }
  ...
  scrollToFile = ({changedFilePath, changedFilePosition}) => {
    this.refEditor.map(e => {
      ...
      return e.scrollToBufferPosition({row, column: 0}, {center: true});
    });
  }
