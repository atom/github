...
export default class Repository {
  ...
  async stageFileDiff (fileDiff) {
    let stagedContent = await this.readContentFromIndex(fileDiff.getOldPath())
    let contentToWrite = applyPatch(stagedContent, fileDiff.toString())
    await this.writeContentToIndex(fileDiff.getNewPath(), contentToWrite)
  }
  ...
  async readContentFromIndex (path) {
    const data = {}

    let index = await this.rawRepository.index()
  ...
  async writeContentToIndex (path, content) {
