...
export default function({confirm, notificationManager, workspace}) {
  ...
  const pushPipeline = pipelineManager.getPipeline(
    pipelineManager.actionKeys.PUSH,
  );
  ...
  const pullPipeline = pipelineManager.getPipeline(
    pipelineManager.actionKeys.PULL,
  );
  pullPipeline.addMiddleware(
    'set-pull-in-progress',
      ...
      try {
        ...
        if (error instanceof GitError) {
          ...
          if (
            ...
            /Automatic merge failed; fix conflicts and then commit the result./.test(
              error.stdOut,
            )
          ) {
            ...
            /fatal: Not possible to fast-forward, aborting./.test(error.stdErr)
          ) {
  ...
  const fetchPipeline = pipelineManager.getPipeline(
    pipelineManager.actionKeys.FETCH,
  );
  fetchPipeline.addMiddleware(
  ...
  const checkoutPipeline = pipelineManager.getPipeline(
    pipelineManager.actionKeys.CHECKOUT,
  );
  ...
  checkoutPipeline.addMiddleware(
    ...
    async (next, repository, branchName, options) => {
      try {
        ...
        if (error instanceof GitError) {
          const message = options.createNew
  ...
  const commitPipeline = pipelineManager.getPipeline(
    pipelineManager.actionKeys.COMMIT,
  );
  commitPipeline.addMiddleware('confirm-commit', async (next, repository) => {
    function confirmCommit() {
      const choice = confirm({
        ...
        detailedMessage:
          'Do you want to commit and close all open commit message editors?',
  ...
  commitPipeline.addMiddleware(
    'clean-up-disk-commit-msg',
    async (next, repository) => {
      await next();
      try {
  ...
  const addRemotePipeline = pipelineManager.getPipeline(
    pipelineManager.actionKeys.ADDREMOTE,
  );
  addRemotePipeline.addMiddleware(
    'failed-to-add-remote',
    async (next, repository, remoteName) => {
      try {
        return await next();
      } catch (error) {
        if (error instanceof GitError) {
