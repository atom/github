...
export default class WorkerManager {
  ...
  constructor() {
    ...
    console.log('Creating a new worker in the constructor');
  ...
  createNewWorker({operationCountLimit} = {operationCountLimit: 10}) {
    ...
    worker.getReadyPromise().then(() => {
      console.log(`createNewWorker: ${worker.getPid()}`);
    });
  }

  onDestroyed(destroyedWorker) {
    console.log(`onDestroyed callback called for ${destroyedWorker.getPid()}`);
    this.workers.delete(destroyedWorker);
  }

  onCrashed(crashedWorker) {
    if (crashedWorker === this.getActiveWorker()) {
      console.log('Creating a worker to replace a crashed one');
  ...
  onSick(sickWorker) {
    ...
    console.log('Creating a worker to replace a sick one');
  ...
  registerListeners() {
    const handleMessages = (event, {sourceWebContentsId, type, data}) => {
      if (sourceWebContentsId === this.win.webContents.id) {
        console.log(`ipc message received: ${type}, ${require('util').inspect(data)}`);
