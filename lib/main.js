...
class Git {
  ...
  statusBarTile: ?Tile<StatusBarComponent>;
  fileList: FileList;
  ...
  update (): Promise<void> {
    ...
    if (statusBarTile) {
      promises.push(statusBarTile.getItem().viewModel.update())
    }
  ...
  consumeStatusBar (statusBar: StatusBar) {
    ...
    this.statusBarTile = statusBar.addRightTile({item: new StatusBarComponent(viewModel, () => toggleChangesPanel()), priority: -100})
  }
}
