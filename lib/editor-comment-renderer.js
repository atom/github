...
import PRCommentComponent from './components/pr-comment-component'

import compareSets from 'compare-sets'

const renderersByEditor = new WeakMap()

export default class EditorCommentRenderer {
  static renderCommentsForEditor (comments, editor) {
    let renderer = renderersByEditor.get(editor)
    if (!renderer) {
      renderer = new EditorCommentRenderer(editor)
  ...
  addComment (commentId, comment) {
    ...
    const component = new PRCommentComponent({comment})
