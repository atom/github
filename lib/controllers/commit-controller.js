...
export default class CommitController extends React.Component {
  ...
  constructor(props, context) {
    ...
    this.previewWatcherSub = new Disposable();
    this.watchCommitPreviewItems();
  }

  isCommitPreviewOpen() {
    const items = this.props.workspace.getPaneItems();
    const uriPattern = new URIPattern(CommitPreviewItem.uriPattern);
    for (const item of items) {
      if (item.getURI && uriPattern.matches(item.getURI())) {
        return true;
      }
    }
    return false;
  ...
  /**
   * Track the presence of CommitPreviewItems corresponding to the current repository with this.state.commitPreviewOpen.
   */
  watchCommitPreviewItems() {
    this.subscriptions.remove(this.previewWatcherSub);
    this.previewWatcherSub.dispose();

    this.previewWatcherSub = watchWorkspaceItem(
      this.props.workspace,
      CommitPreviewItem.buildURI(this.props.repository.getWorkingDirectoryPath()),
      this,
      'commitPreviewOpen',
    );
    this.subscriptions.add(this.previewWatcherSub);
  }
