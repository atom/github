...
export default class CommitController extends React.Component {
  ...
  static propTypes = {
    ...
    selectedCoAuthors: PropTypes.arrayOf(PropTypes.string).isRequired,
  ...
  componentWillMount() {
    ...
    if (this.props.isMerging && !this.getRegularCommitMessage()) {
  ...
  componentWillReceiveProps(nextProps) {
    if (!this.props.isMerging && nextProps.isMerging && !this.getRegularCommitMessage()) {
  ...
  @autobind
  async commit(message, coAuthors = [], amend) {
    ...
    if (coAuthors.length) {
      ...
      msg = await this.props.repository.addTrailersToCommitMessage(msg, trailers);
    }

    return this.props.commit(msg.trim(), {amend});
  }

  getCommitMessage() {
    return this.getRegularCommitMessage() || '';
  }
