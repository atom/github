...
export default class EditorConflictController extends React.Component {
  ...
  constructor(props, context) {
    ...
    this.state = {
      conflicts: new Set(
        Conflict.allFromEditor(props.editor, this.layer, props.isRebase),
      ),
    };
        ...
        {this.state.conflicts.size > 0 && (
          ...
          >
            <Command
              ...
              callback={this.getResolverUsing([OURS])}
            />
            <Command
            ...
            <Command
              ...
              callback={this.getResolverUsing([BASE])}
            />
            <Command
  ...
  getCurrentConflicts() {
    ...
    while (!currentCursor.done && !currentConflict.done) {
      ...
      while (
        !currentCursor.done &&
        currentCursor.value.isLessThan(earliestConflictPosition)
      ) {
      ...
      while (
        ...
        if (currentConflict.value.includesPoint(currentCursor.value)) {
          ...
          while (
            !currentCursor.done &&
            currentCursor.value.isLessThan(endPosition)
          ) {
  ...
  dismissCurrent() {
    this.dismissConflicts(
      this.getCurrentConflicts().map(match => match.conflict),
    );
  }
  ...
  updateMarkerCount() {
    this.props.resolutionProgress.reportMarkerCount(
      ...
      Array.from(this.state.conflicts, c => !c.isResolved()).filter(b => b)
        .length,
    );
  }
}
