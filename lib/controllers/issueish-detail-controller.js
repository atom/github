...
export class BareIssueishDetailController extends React.Component {
  static propTypes = {
    repository: PropTypes.shape({
      ...
      pullRequest: PropTypes.any, // FIXME
      issue: PropTypes.any, // FIXME from IssueishPaneItemContainer.propTypes
    }),
  ...
  updateTitle() {
    ...
    if (repository && (repository.issue || repository.pullRequest)) {
      const isPr = repository.pullRequest && repository.pullRequest.__typename === 'PullRequest';
      let prefix, title, number;
      if (isPr) {
        prefix = 'PR:';
      ...
      title = `${prefix} ${repository.owner.login}/${repository.name}#${number} — ${title}`;
  ...
  render() {
    ...
    const isPr = repository.pullRequest.__typename === 'PullRequest';
    if (isPr) {
      return (
