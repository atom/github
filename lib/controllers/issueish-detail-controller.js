...
import MaybeOperation from '../models/maybe-operation';
import IssueishDetailView from '../views/issueish-detail-view';
...
export class BareIssueishDetailController extends React.Component {
  ...
  constructor(props) {
    ...
    this.checkoutOp = new MaybeOperation(this.checkout);
  ...
  nextCheckoutOp() {
    ...
    if (issueish.__typename !== 'PullRequest') {
      return this.checkoutOp.disable({}, 'Cannot check out an issue');
    }

    if (this.props.isMerging) {
      return this.checkoutOp.disable({}, 'Merge in progress');
    }

    if (this.props.isRebasing) {
      return this.checkoutOp.disable({}, 'Rebase in progress');
    }
    ...
    if (fromPullRefspec || fromHeadRepo) {
      return this.checkoutOp.disable({}, 'Current');
    }
