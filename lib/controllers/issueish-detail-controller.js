...
export class BareIssueishDetailController extends React.Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    const typename =
      repository && repository.pullRequest && repository.pullRequest.__typename
        ? repository.pullRequest.__typename
        : null;
  ...
  updateTitle() {
    ...
    if (repository && (repository.issue || repository.pullRequest)) {
      ...
      const title = `${prefix} ${repository.owner.login}/${repository.name}#${
        issueish.number
      } — ${issueish.title}`;
  ...
  async checkout() {
    ...
    if (headRemotes.length > 0) {
      ...
      // it above). ¯\_(ツ)_/¯
      const remote = await this.props.addRemote(
        headRepository.owner.login,
        url,
      );
    ...
    await this.props.checkout(
      `pr-${pullRequest.number}/${headRepository.owner.login}/${
        pullRequest.headRefName
      }`,
      {
        createNew: true,
        track: true,
        startPoint: `refs/remotes/${sourceRemoteName}/${
          pullRequest.headRefName
        }`,
      },
    );
