...
export class BareIssueishDetailController extends React.Component {
  ...
  // Storing `typename` in state to avoid having to do ugly long chained lookups in several places. note that whether
  // we're rendering an Issue or a PullRequest, relay returns both issue and pull request data. So a pullRequest can
  // have a __typename of `Issue` or `PullRequest`, which is then set in state here.
