...
import {extractCoAuthorsAndRawCommitMessage} from '../helpers';
...
@ObserveModelDecorator({
  ...
  fetchData: (r, props) => {
    return yubikiri({
      ...
      stagedChanges: r.getStagedChanges(),
...
export default class GitTabController extends React.Component {
  ...
  @autobind
  updateSelectedCoAuthors(selectedCoAuthors) {
  ...
  async undoLastCommit() {
    ...
    const {message, coAuthors} = extractCoAuthorsAndRawCommitMessage(lastCommit.message);
    repo.setRegularCommitMessage(message);
