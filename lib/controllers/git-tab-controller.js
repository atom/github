...
export default class GitTabController extends React.Component {
  ...
  unstageFiles = filePaths => this.props.repository.unstageFiles(filePaths)

  prepareToCommit = async () => !await this.props.ensureGitTab()

  commit = (message, options) => this.props.repository.commit(message, options)
  ...
  checkout = (branchName, options) => this.props.repository.checkout(branchName, options)
