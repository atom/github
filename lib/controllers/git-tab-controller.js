  ...
  fetchRepositoryData(repository) {
    return yubikiri({
      ...
      aheadCount: async query => {
        const remoteName = await query.remoteName;
        return remoteName ? repository.getAheadCount(await query.branchName) : null;
      },
      behindCount: async query => {
        const remoteName = await query.remoteName;
        return remoteName ? repository.getBehindCount(await query.branchName) : null;
      },
