...
export default class GitTabController extends React.Component {
  ...
  renderWithData = repoData => {
    console.log('renderWithData', repoData.stagedChanges);
  ...
  async stageFiles(filePaths) {
    ...
    console.log(`About to stage ${filePaths.join(', ')}`);
    ...
    console.log(`no rly about to stage ${Array.from(pathsToStage).join(', ')}`);
    const r = await this.props.repository.stageFiles(Array.from(pathsToStage));
    console.log(this.props.repository.getWorkingDirectoryPath());
    console.log('stageFiles complete');
    return r;
  }
