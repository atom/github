...
import {MultiFilePatchPropType, EndpointPropType} from '../prop-types';
...
export default class MultiFilePatchController extends React.Component {
  static propTypes = {
    ...
    reviewSummaries: PropTypes.arrayOf(PropTypes.shape({
      state: PropTypes.oneOf(['COMMENTED', 'PENDING']).isRequired,
    })),
  ...
  render() {
    return (
      <MultiFilePatchView
        ...
        addCommentAtRow={this.addCommentAtRow}
      />
    );
  }
  ...
  addCommentAtRow = async bufferRow => {
    const reviewsTab = await this.props.workspace.open(ReviewsItem.buildURI({
    ...
    reviewsTab.startCommentThreadAt(bufferRow);

    const noReviews = this.props.reviewSummaries.length === 0;
    const noPendingReviews = this.props.reviewSummaries.every(summary => summary.state !== 'PENDING');

    if (noReviews) {
      await reviewsTab.openPendingReviewsTab();
    } else if (noPendingReviews) {
      await reviewsTab.openAllReviewsTab();
    } else {
      await reviewsTab.openPendingReviewsTab();
    }
  }
