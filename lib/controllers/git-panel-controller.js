...
export default class GitPanelController {
  ...
  async fetchRepositoryData(repository) {
    ...
    const data = {
      ...
      currentBranch: await dataPromises.currentBranch,
    ...
    if (data.remoteName) {
      data.aheadCount = await repository.getAheadCount(data.currentBranch.name);
      data.behindCount = await repository.getBehindCount(data.currentBranch.name);
    }
