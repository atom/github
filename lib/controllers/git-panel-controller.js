...
export default class GitPanelController {
  constructor (props) {
    ...
    this.stageResolvedFile = this.stageResolvedFile.bind(this)
    ...
    if (this.repository) {
      return (
        <GitPanelView
          ...
          stageResolvedFile={this.stageResolvedFile}
          commit={this.commit}
          push={this.push}
          pull={this.pull}
          fetch={this.fetch}
  ...
  async _refreshModelData (repository) {
    if (repository) {
      ...
      const mergeConflicts = await repository.getMergeConflictPaths()
  ...
  stageResolvedFile (filePath) {
    return this.repository.stageResolvedFile(filePath)
  }
