...
export default class CommitViewController {
  ...
  constructor(props) {
    ...
    this.subscriptions.add(
      this.props.workspace.onDidAddTextEditor(({textEditor}) => {
        if (textEditor.getPath() === this.getCommitMessagePath()) {
      ...
      this.props.workspace.onDidDestroyPaneItem(async ({item}) => {
        if (item.getPath() === this.getCommitMessagePath() && this.getCommitMessageEditors().length === 0) {
  ...
  async openCommitEditor(messageFromBox) {
    ...
    commitEditor.setText(messageFromBox);
