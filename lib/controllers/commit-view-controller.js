...
export default class CommitViewController {
        ...
        if (this.props.repository.isPresent() && item.getPath && item.getPath() === this.getCommitMessagePath() &&
          ...
          try {
            if (this.props.isAmending && this.props.lastCommit.isPresent()) {
              this.props.repository.setAmendingCommitMessage(await readFile(this.getCommitMessagePath()));
            } else {
              this.props.repository.setRegularCommitMessage(await readFile(this.getCommitMessagePath()));
            }
          } catch (e) {
            if (e.code !== 'ENOENT') {
              throw e;
  ...
  update(props) {
    ...
    if (!wasAmending && this.props.isAmending && !repo.getAmendingCommitMessage() && this.props.lastCommit.isPresent()) {
      repo.setAmendingCommitMessage(props.lastCommit.getMessage());
    } else if (!wasMerging && this.props.isMerging && !repo.getRegularCommitMessage()) {
      repo.setRegularCommitMessage(this.props.mergeMessage || '');
    }
  ...
  getCommitMessage() {
    const repo = this.props.repository;
    const message = this.props.isAmending ? repo.getAmendingCommitMessage() : repo.getRegularCommitMessage();
    return message || '';
  }
  ...
  handleMessageChange(newMessage) {
    ...
    if (this.props.isAmending) {
      repo.setAmendingCommitMessage(newMessage);
    } else {
      repo.setRegularCommitMessage(newMessage);
    }
    etch.update(this);
  }
