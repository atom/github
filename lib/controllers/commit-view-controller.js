...
export default class CommitViewController {
  ...
  activateCommitEditor() {
    ...
    const paneWithEditor = panes.find(pane => {
      const editor = pane.getItems().find(item => item === this.editor);
      return !!editor;
    });
    paneWithEditor.activate();
    paneWithEditor.activateItem(this.editor);
  }
  ...
  async openCommitEditor(messageFromBox) {
    ...
    this.editor = await this.props.workspace.open(atomCommitMessagePath);
    this.editor.setText(messageFromBox);
    this.deactivateCommitBox = true;

    // TODO: don't use atom global?
    ...
    this.editorSubscription = this.editor.onDidDestroy(async () => {
      this.editor = null;
