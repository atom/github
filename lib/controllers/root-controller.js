...
export default class RootController extends React.Component {
  ...
  @autobind
  getSelectedStagingViewItem() {
    if (this.gitTabController) {
      return this.gitTabController.getWrappedComponent().getSelectedStagingViewItem();
    } else {
      return null;
    }
  }
  ...
  async viewChangesForCurrentFile(stagingStatus) {
    ...
    if (absFilePath.startsWith(repoPath)) {
      ...
      const filePatchItem = await this.props.workspace.open(
        `atom-github://file-patch/${filePath}?stagingStatus=${stagingStatus}&${amending ? 'amending&' : ''}lineNum=${lineNum}`,
