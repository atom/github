...
export default class RootController extends React.Component {
  ...
  constructor(props, context) {
    ...
    this.state = {
      ...
      githubTabActive: props.firstRun || props.savedState.githubTabActive || props.savedState.githubTabActive,
  ...
  toggleGitTab() {
    if (!this.state.gitTabActive) {
      this.setState({gitTabActive: true});
    } else if (this.gitDockItem) {
      const item = this.gitDockItem.getDockItem();
      this.props.workspace.toggle(item);
    } else {
      this.setState({gitTabActive: false});
    }
  }

  @autobind
  toggleGithubTab() {
    if (!this.state.githubTabActive) {
      this.setState({githubTabActive: true});
    } else if (this.githubDockItem) {
      const item = this.githubDockItem.getDockItem();
      this.props.workspace.toggle(item);
    } else {
      this.setState({githubTabActive: false});
    }
  }
