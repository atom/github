...
export default class RootController extends React.Component {
  ...
  constructor(props, context) {
    ...
    this.gitTabTracker = new TabTracker('git', {
      getState: () => this.props.gitTabStubItem,
    ...
    this.githubTabTracker = new TabTracker('github', {
      getState: () => this.props.githubTabStubItem,
      setState: (value, {activateTab}, callback) => {
  ...
  renderPanels() {
    const gitTab = this.props.gitTabStubItem && (
      <DockItem
        ...
        onDidCloseItem={() => this.props.gitTabStubItem.destroy()}
    ...
    const githubTab = this.props.githubTabStubItem && (
      <DockItem
        ...
        workspace={this.props.workspace}
        onDidCloseItem={() => this.props.githubTabStubItem.destroy()}
  ...
  didChangeAmending(isAmending) {
    ...
    // set repository amending state, update repository, which should cause the root controller to rerender

    // const itemsToUpdate = this.getFilePatchPaneItems({onlyStaged: true});
    // itemsToUpdate.forEach(item => {
    //   item.component.setAmendingState(isAmending)
    // );
  }
...
class TabTracker {
  ...
  async toggle() {
    ...
    if (!wasRendered) {
      // Not rendered.
      await this.getWorkspace().open(`atom-github://dock-item/${this.name}`);
      await this.reveal();
