...
export default class RootController extends React.Component {
  ...
  renderFilePatches() {
    ...
    console.log('# of filePatchItems', this.props.filePatchItems);
    return this.props.filePatchItems.map(item => {
      const {filePath, stagingStatus, workdir, amending, lineNumber} = getPropsFromUri(item.id);
  ...
  async viewChangesForCurrentFile(stagingStatus) {
    ...
    if (absFilePath.startsWith(repoPath)) {
      ...
      const filePatchItem = await this.props.workspace.open(
        `atom-github://file-patch/${filePath}?workdir=${repoPath}` +
        `&stagingStatus=${stagingStatus}${amending ? '&amending' : ''}`,
  ...
  viewUnstagedChangesForCurrentFile() {
    console.warn('viewUnstagedChangesForCurrentFile');
