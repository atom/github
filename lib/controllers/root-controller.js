...
export default class RootController extends React.Component {
  ...
  constructor(props, context) {
    ...
    autobind(
      this,
      'installReactDevTools', 'clearGithubToken', 'showOpenIssueishDialog',
      'showWaterfallDiagnostics', 'showCacheDiagnostics',
      'acceptOpenIssueish', 'cancelOpenIssueish', 'destroyFilePatchPaneItems',
      'destroyEmptyFilePatchPaneItems', 'quietlySelectItem', 'viewUnstagedChangesForCurrentFile',
  ...
  renderCredentialDialog() {
    ...
    return (
      <Panel workspace={this.props.workspace} location="modal">
        <CredentialDialog commands={this.props.commands} {...this.state.credentialDialogQuery} />
      </Panel>
    );
  }
  ...
  cancelDialog = () => new Promise(resolve => this.setState({dialogRequest: dialogRequests.null}));

  openInitializeDialog = dirPath => {
    if (!dirPath) {
      dirPath = this.props.config.get('core.projectHome');
    }

    const dialogRequest = dialogRequests.init({dirPath});
    dialogRequest.onProgressingAccept(chosenPath => this.props.initialize(chosenPath));
    dialogRequest.onCancel(this.cancelDialog);
  ...
  openCloneDialog = opts => {
    ...
    dialogRequest.onProgressingAccept((url, chosenPath) => this.props.clone(url, chosenPath));
    dialogRequest.onCancel(this.cancelDialog);
