...
import {destroyFilePatchPaneItems, destroyEmptyFilePatchPaneItems, autobind, fileExists} from '../helpers';
...
export default class RootController extends React.Component {
  ...
  async installExtension(id) {
    ...
    await new Promise((resolve, reject) => {
      unzip(extensionFile, extensionFolder, async err => {
        if (err && !await fileExists(path.join(extensionFolder, 'manifest.json'))) {
          reject(err);
        }
    ...
    await fs.mkdir(extensionFolder, {recursive: false, mode: 0o755});
  ...
  async proceedWithLastDiscardUndo(results, partialDiscardFilePath = null) {
    const promises = results.map(async result => {
      ...
      if (deleted && resultPath === null) {
        ...
        await fs.copyFile(resultPath, absFilePath);
      }
      if (conflict) {
  ...
  async refreshResolutionProgress(fullPath) {
    ...
    return this.props.resolutionProgress.reportMarkerCount(fullPath, count);
  }
}
