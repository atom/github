...
export default class RootController extends React.Component {
  ...
  clearGithubToken = () => this.props.loginModel.removeToken('https://api.github.com')
  ...
  showOpenIssueishDialog = () => new Promise(resolve => this.setState({openIssueishDialogActive: true}, resolve))

  showOpenCommitDialog = () => new Promise(resolve => this.setState({openCommitDialogActive: true}, resolve))

  showWaterfallDiagnostics = () => this.props.workspace.open(GitTimingsView.buildURI())

  showCacheDiagnostics = () => this.props.workspace.open(GitCacheView.buildURI())

  acceptClone = async (remoteUrl, projectPath) => {
  ...
  cancelClone = () => new Promise(resolve => this.setState({cloneDialogActive: false}, resolve))

  acceptInit = async projectPath => {
    try {
  ...
  cancelInit = () => {
    ...
    return new Promise(resolve => {
      this.setState({initDialogActive: false, initDialogPath: null, initDialogResolve: null}, resolve);
    });
  }
  ...
  cancelOpenIssueish = () => new Promise(resolve => this.setState({openIssueishDialogActive: false}, resolve))
  ...
  destroyFilePatchPaneItems = () => destroyFilePatchPaneItems({onlyStaged: false}, this.props.workspace)

  destroyEmptyFilePatchPaneItems = () => destroyEmptyFilePatchPaneItems(this.props.workspace)

  openCloneDialog = () => new Promise(resolve => this.setState({cloneDialogActive: true}, resolve))
  ...
  viewUnstagedChangesForCurrentFile = () => this.viewChangesForCurrentFile('unstaged')

  viewStagedChangesForCurrentFile = () => this.viewChangesForCurrentFile('staged')
  ...
  discardWorkDirChangesForPaths = async filePaths => {
  ...
  discardLines = async (multiFilePatch, lines, repository = this.props.repository) => {
  ...
  undoLastDiscard = async (partialDiscardFilePath = null, repository = this.props.repository) => {
  ...
  refreshResolutionProgress = fullPath => {
    ...
    return Conflict.countFromStream(readStream).then(count => {
