...
function getPropsFromUri(uri) {
  // atom-github://file-patch/file.txt?workdir=/foo/bar/baz&stagingStatus=staged&amending&lineNum=12
  const {protocol, hostname, pathname, query} = url.parse(uri, true);
  if (protocol === 'atom-github:' && hostname === 'file-patch') {
    const filePath = pathname.slice(1);
    const {stagingStatus, amending, lineNum, workdir} = query;
    return {
      filePath,
      workdir,
      stagingStatus,
...
export default class RootController extends React.Component {
  ...
  renderFilePatches() {
    ...
    console.log(this.props.filePatchItems.length);
    return this.props.filePatchItems.map(item => {
      // return (<div>{item.uri}</div>);
      const {filePath, stagingStatus, workdir, amending, lineNumber} = getPropsFromUri(item.uri);
      const repository = this.props.repository; // this.getRepositoryForWorkdir(workdir);

      console.log('render file patch', item);
      return (
        <PaneItem
          key={item.uri}
          workspace={this.props.workspace}
          ...
          onDidCloseItem={() => { this.props.removeFilePatch(item.uri); }}
          stubItemSelector="git-file-patch-controller"
          id={item.uri} >
          <FilePatchController
  ...
  getFilePatchPaneItems({onlyStaged} = {}) {
    return this.props.workspace.getPaneItems().filter(item => {
      const isFilePatchPaneItem = item instanceof FilePatchPaneItem;
      if (onlyStaged) {
        return isFilePatchPaneItem && item.stagingStatus === 'staged';
      } else {
        return isFilePatchPaneItem;
      }
    });
  }
  ...
  async viewChangesForCurrentFile(stagingStatus) {
    ...
    if (absFilePath.startsWith(repoPath)) {
      ...
      // TODO: simply add filePatchItem to GithubPackage array
      const filePatchItem = await this.props.workspace.open(
        `atom-github://file-patch/${filePath}?workdir=${this.props.workingDirectoryPath}` +
        `&stagingStatus=${stagingStatus}&${amending ? 'amending&' : ''}lineNum=${lineNum}`,
