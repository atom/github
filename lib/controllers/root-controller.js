...
export default class RootController extends React.Component {
    ...
    return (
      <div>
        <Commands registry={this.props.commandRegistry} target="atom-workspace">
          ...
          <Command command="github:toggle-git-tab" callback={this.toggleGitTab} />
          <Command command="github:toggle-git-tab-focus" callback={this.toggleGitTabFocus} />
          <Command command="github:toggle-github-tab" callback={this.toggleGithubTab} />
          <Command command="github:toggle-github-tab-focus" callback={this.toggleGithubTabFocus} />
  ...
  toggleGitTab() {
    return new Promise(resolve => {
      if (!this.state.gitTabActive) {
        this.setState({gitTabActive: true}, resolve);
      } else if (this.gitDockItem) {
        const item = this.gitDockItem.getDockItem();
        resolve(this.props.workspace.toggle(item));
      } else {
        this.setState({gitTabActive: false}, resolve);
      }
    });
  }
  ...
  @autobind
  toggleGitTabFocus() {
    if (!this.gitTabIsVisible()) {
      this.toggleGitTab().then(this.toggleGitTabFocus);
      return;
    }
  ...
  @autobind
  focusGitTab() {
    this.gitTabController.getWrappedComponent().restoreFocus();
  }
  ...
  gitTabIsVisible() {
    ...
    return this.props.workspace.getPaneContainers()
      .filter(container => container === this.props.workspace.getCenter() || container.isVisible())
  ...
  gitTabHasFocus() {
    return this.gitTabController.getWrappedComponent().element.contains(document.activeElement);
  }
