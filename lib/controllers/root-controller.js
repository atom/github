...
export default class RootController extends React.Component {
  ...
  constructor(props, context) {
    ...
    this.environment = RelayNetworkLayerManager.getHubKit()
  }
  ...
  render() {
    return (
      <RelayEnvironmentContext.Provider value={this.environment}>
        <Fragment>
          {/* <QueryRenderer query={graphql `query WhoAmI { viewer { login } }`} /> */}
          {this.renderCommands()}
  ...
  async viewChangesForCurrentFile(stagingStatus) {
    ...
    console.log('repoPath=', repoPath, this.props)
    if (repoPath === null) {
