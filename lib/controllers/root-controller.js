...
export default class RootController extends React.Component {
  ...
  renderPanels() {
    if (!this.props.useLegacyPanels) {
      const gitPanel = this.state.gitPanelActive && (
        <DockItem
          workspace={this.props.workspace}
      ...
      return <div>{gitPanel}{githubPanel}</div>;
    }
  ...
  @autobind
  initializeRepo() {
    this.props.createRepositoryForProjectPath(this.props.activeWorkingDirectory);
  }
