...
export default class RootController extends React.Component {
  ...
  async discardLines(multiFilePatch, lines, repository = this.props.repository) {
    const filePaths = multiFilePatch.getFilePatches().map(fp => fp.getPath());
    ...
    return await repository.storeBeforeAndAfterBlobs(
      [filePaths],
      () => this.ensureNoUnsavedFiles(filePaths, 'Cannot discard lines.', repository.getWorkingDirectoryPath()),
      destructiveAction,
      // FIXME: Present::storeBeforeAndAfterBlobs() and DiscardHistory::storeBeforeAndAfterBlobs() need a way to store
