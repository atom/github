...
export default class RootController extends React.Component {
  ...
  constructor(props, context) {
    ...
    this.gitTabTracker = new TabTracker(this, 'git');
    this.githubTabTracker = new TabTracker(this, 'github');
  }
...
class TabTracker {
  constructor(component, name) {
    ...
    this.workspace = component.props.workspace;
    this.name = name;

    this.stateKey = `${name}TabActive`;
    this.dockItemKey = `${name}DockItem`;
    this.controllerKey = `${name}TabController`;
  }
  ...
  setStateKey(value) {
    return new Promise(resolve => {
      this.component.setState({[this.stateKey]: value}, resolve);
    });
  }

  getController() {
    return this.component[this.controllerKey];
  }

  getControllerComponent() {
    const controller = this.getController();
    return controller ? controller.getWrappedComponent() : null;
  }

  getDockItem() {
    const item = this.component[this.dockItemKey];
  ...
  toggle() {
    if (!this.getState()) {
      return this.setStateKey(true);
    } else if (this.getDockItem()) {
      return this.workspace.toggle(this.getDockItem());
    } else {
      return this.setStateKey(false);
    }
  }
  ...
  async ensureVisible() {
    if (!this.isVisible()) {
      ...
      if (this.getDockItem()) {
        await this.workspace.open(this.getDockItem());
      }
      return true;
    }
    return false;
  ...
  focus() {
    this.getControllerComponent().setFocus();
  }
  ...
  isVisible() {
    ...
    return this.workspace.getPaneContainers()
      .filter(container => container === this.workspace.getCenter() || container.isVisible())
      .some(container => container.getPanes().some(pane => pane.getActiveItem() === item));
  }

  hasFocus() {
    const component = this.getControllerComponent();
    if (!component) {
      return false;
    }
    return component.element.contains(document.activeElement);
  }
}
