...
export default class RootController extends React.Component {
  ...
  renderPaneItems() {
    return (
      <Fragment>
        ...
        <PaneItem workspace={this.props.workspace} uriPattern={CommitPreviewItem.uriPattern}>
          {({itemHolder, params}) => (
            <CommitPreviewItem
              ref={itemHolder.setter}
  ...
  async viewChangesForCurrentFile(stagingStatus) {
    ...
    if (absFilePath.startsWith(repoPath)) {
      ...
      await ChangedFileItem.getRealItemPromise();
      await ChangedFileItem.getFilePatchLoadedPromise();
      ChangedFileItem.goToDiffLine(lineNum);
      ChangedFileItem.focus();
    } else {
