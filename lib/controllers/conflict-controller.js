...
export default class ConflictController extends React.Component {
  ...
  resolveAsSequence(sources) {
    const {conflict, editor} = this.props;
    const [firstSide, ...restOfSides] = sources
      .map(source => conflict.getSide(source))
      .filter(side => side);

    const textToAppend = restOfSides.map(side => side.getTextIn(editor)).join('');
    ...
    const appendedRange = firstSide.appendTextIn(editor, textToAppend);
