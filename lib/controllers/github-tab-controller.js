...
export default class GithubTabController extends React.Component {
  ...
  fetchModelData(repo) {
    return yubikiri({
      ...
      isUnpublished: async query => {
        const branch = await query.currentBranch;
        const remote = await repo.getRemoteForBranch(branch.getName());
  ...
  renderWithData({remotes, currentBranch, selectedRemoteName, selectedPrUrl, aheadCount, isUnpublished}) {
