...
export class BareReviewsController extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (this.props.initCommentID && this.props.initCommentID !== prevProps.initCommentID) {
      this.showCommentID(this.props.initCommentID);
    }
  }
      ...
      <PullRequestCheckoutController
        ...
        {checkoutOp => (
          <ReviewsView
            ...
            commentIDsOpen={this.state.commentIDsOpen}
            ...
            showCommentID={this.showCommentID}
            hideCommentID={this.hideCommentID}
  ...
  showComments = () => new Promise(resolve => this.setState({summarySectionOpen: true}, resolve));

  hideComments = () => new Promise(resolve => this.setState({summarySectionOpen: false}, resolve));

  showCommentID = commentID => new Promise(resolve => this.setState(prev => {
    prev.commentIDsOpen.add(commentID);
    return {};
  }, resolve));

  hideCommentID = commentID => new Promise(resolve => this.setState(prev => {
    prev.commentIDsOpen.delete(commentID);
    return {};
  }, resolve));

  resolveThread = async thread => {
    if (thread.viewerCanResolve) {
      // optimistically call the onComplete callback to avoid jankiness;
