...
export class BareReviewsController extends React.Component {
      ...
      <PullRequestCheckoutController
        ...
        {checkoutOp => (
          <ReviewsView
            ...
            uncreatedCommentThreads={this.props.uncreatedCommentThreads}
            ...
            addCommentToReview={this.addCommentToReview}
            startPendingReview={this.startPendingReview}
            replyToStartReview={this.replyToStartReview}
  ...
  addSingleComment = async (commentBody, threadID, replyToID, path, position, callbacks = {}) => {
    ...
    try {
      ...
      const reviewResult = await this.startPendingReview();
  ...
  startPendingReview = async (callbacks = {}) => {
    let pendingReview = null;
    try {
      pendingReview = await addReviewMutation(this.props.relay.environment, {
        pullRequestID: this.props.pullRequest.id,
        viewerID: this.props.viewer.id,
      });
      if (callbacks.didSucceed) {
      ...
      this.props.reportRelayError('Unable to start a pending review', error);
      if (callbacks.didFail) {
        callbacks.didFail();
      }
    }
  ...
  addCommentToReview = async (
    reviewID, commentBody, threadID, replyToID, path, position, state = 'PENDING', callbacks = {},
  ) => {
    let commentPromise = null;
    try {
      commentPromise = await addReviewCommentMutation(this.props.relay.environment, {
        body: commentBody,
        inReplyTo: replyToID,
        reviewID,
        threadID,
      ...
      this.props.reportRelayError('Unable to add comment to review', error);
      if (callbacks.didFail) {
        callbacks.didFail();
      }
    }
  ...
  replyToStartReview = async (commentBody, threadID, replyToID, path, position, callbacks = {}) => {
    let pendingReviewID = null;
    try {
      this.setState({postingToThreadID: threadID});
      ...
      await this.addCommentToReview(reviewID, commentBody, threadID, replyToID, path, position, 'PENDING', callbacks);

      pendingReviewID = null;
    } catch (error) {
      if (pendingReviewID !== null) {
        this.deleteReview(pendingReviewID, error);
      }
      this.props.reportRelayError('Unable to submit your comment', error);
    } finally {
      this.setState({postingToThreadID: null});
  ...
  deleteReview = async (reviewID, error = {}) => {
    try {
      await deleteReviewMutation(this.props.relay.environment, {
        reviewID,
        pullRequestID: this.props.pullRequest.id,
