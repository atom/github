...
export default class IssueishPaneItemController extends React.Component {
  static create({owner, repo, prNumber, host}, uri) {
    ...
    return {
      serialize() {
        return {
          uri,
          deserializer: 'IssueishPaneItemController',
        };
      },
      getURI() { return uri; },
      ...
      get element() {
        if (!element) {
          ...
          ReactDom.render(<IssueishPaneItemController {...props} />, element);
        }
        return element;
      },
    };
  ...
  static opener(uri) {
    ...
    if (props) {
      return IssueishPaneItemController.create(props, uri);
    } else {
      return null;
    }
  }
  ...
  renderWithToken(data) {
    ...
    return (
      <RelayRootContainer
        Component={IssueishLookupByNumberContainer}
        route={route}
