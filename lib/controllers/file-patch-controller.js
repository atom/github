...
export default class FilePatchController extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.stagingStatus !== this.props.stagingStatus) {
      this.emitter.emit('did-change-title');
    }
  }

  async componentWillReceiveProps(nextProps) {
    const stagingStatus = nextProps.stagingStatus;
    if (nextProps.isAmending !== this.props.isAmending) {
      const repository = this.repository;
      const staged = stagingStatus === 'staged';
      const amending = staged && nextProps.isAmending;
  ...
  async diveIntoCorrespondingFilePatch() {
    ...
    const staged = stagingStatus === 'staged';
    const amending = staged && this.props.isAmending;
    const filePatch = await this.repository.getFilePatchForPath(this.props.filePath, {staged, amending});
  ...
  @autobind
  undoLastDiscard() {
    return this.props.undoLastDiscard(this.props.filePath, this.props.repository);
  }

  @autobind
  hasUndoHistory() {
    return this.repository.hasDiscardHistory(this.props.filePath);
  }
