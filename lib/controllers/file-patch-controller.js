...
export default class FilePatchController extends React.Component {
  ...
  async onRepoRefresh(repository) {
    ...
    const amending = this.isAmending();
    let filePatch = await this.getFilePatchForPath(this.props.filePath, staged, amending);
  ...
  getFilePatchForPath(filePath, staged, isAmending) {
  ...
  async stageLines(lines) {
    ...
    console.log('didFinishStageOperation');
  }
  ...
  attemptLineStageOperation(lines) {
    ...
    console.log('set stagingOperationInProgress to true');
    this.stagingOperationInProgress = true;
    this.props.switchboard.getChangePatchPromise().then(() => {
      console.log('set stagingOperationInProgress to false');
      this.stagingOperationInProgress = false;
    });
  ...
  async diveIntoCorrespondingFilePatch() {
    ...
    const amending = this.isAmending();
    const filePatch = await this.getFilePatchForPath(this.props.filePath, stagingStatus === 'staged', amending);
    this.props.quietlySelectItem(this.props.filePath, stagingStatus);
    this.setState({filePatch, stagingStatus});
  }

  isAmending() {
    return this.props.repositoryStateRegistry.getStateForModel(this.repositoryObserver.getActiveModel()).amending;
  }
