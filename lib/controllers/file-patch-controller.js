...
export default class FilePatchController extends React.Component {
  ...
  //   const isPartiallyStaged = await this.repository.isPartiallyStaged(this.filePath);
  //   const filePatch = await this.repository.getFilePatchForPath(this.filePath, {staged, amending: staged && this.isAmending});
  ...
  async onRepoRefresh() {
    ...
    let filePatch = await repository.getFilePatchForPath(this.filePath, {staged, amending: staged && this.state.isAmending});
    if (filePatch) {
  ...
  @autobind
  undoLastDiscard() {
    return this.props.undoLastDiscard(this.filePath);
  }
