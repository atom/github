...
export default class FilePatchController extends React.Component {
    ...
    if (filePatch) {
      ...
      if (oldFilePatch) {
        filePatch = oldFilePatch.clone({
          oldFile: oldFilePatch.oldFile.clone({mode: null}),
          newFile: oldFilePatch.newFile.clone({mode: null}),
  ...
  render() {
    ...
    const symlinkChange = fp && fp.hasSymlink() ?
      {oldSymlink: fp.getOldFile().getSymlink(), newSymlink: fp.getNewFile().getSymlink(), status: fp.getStatus()} :
      null;
    const repository = this.repositoryObserver.getActiveModel();
  ...
  async stageSymlinkChange() {
    ...
    const addedLines = filePatch && filePatch.getHunks() && filePatch.getHunks()[0].getLines()[0].getStatus() === 'added';
    if (filePatch.getStatus() === 'typechange' && addedLines) {
      await this.repositoryObserver.getActiveModel().git.exec(['rm', '--cached', this.props.filePath]);
    } else {
      await this.repositoryObserver.getActiveModel().stageFiles([this.props.filePath]);
    }
  ...
  async unstageSymlinkChange() {
    ...
    const filePatch = this.state.filePatch;
    const deletedLines = filePatch && filePatch.getHunks() &&
      filePatch.getHunks()[0].getLines()[0].getStatus() === 'deleted';
    if (filePatch.getStatus() === 'typechange' && deletedLines) {
      await this.repositoryObserver.getActiveModel().git.exec(['rm', '--cached', this.props.filePath]);
    } else {
      await this.repositoryObserver.getActiveModel().unstageFiles([this.props.filePath]);
    }
