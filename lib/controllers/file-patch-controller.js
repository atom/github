...
export default class FilePatchController extends React.Component {
  ...
  async toggleRows(filePatch, rowSet, nextSelectionMode) {
    ...
    return this.stagingOperation(() => {
      const patch = this.withStagingStatus({
        staged: () => filePatch.getUnstagePatchForLines(chosenRows),
        unstaged: () => filePatch.getStagePatchForLines(chosenRows),
      });
  ...
  async discardRows(filePatch, rowSet, nextSelectionMode, {eventSource} = {}) {
    let chosenRows = rowSet;
    if (chosenRows) {
      await this.selectedRowsChanged(chosenRows, nextSelectionMode);
    } else {
    ...
    return this.props.discardLines(filePatch, chosenRows, this.props.repository);
  }
