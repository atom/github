...
export default class FilePatchController extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    if (props.filePatch !== state.lastFilePatch) {
      return {
        selectedRows: new Set(), // FIXME derive new selected rows from old ones
  ...
  toggleRows() {
    return this.stagingOperation(() => {
      const patch = this.withStagingStatus({
        staged: () => this.props.filePatch.getUnstagePatchForLines(this.state.selectedRows),
  ...
  discardRows() {
    return this.props.discardLines(this.props.filePatch, this.state.selectedRows, this.props.repository);
  }

  selectedRowsChanged(rows) {
    if (equalSets(this.state.selectedRows, rows)) {
      return;
    }
