...
export default class GitController extends React.Component {
  ...
  async showFilePatchForPath(filePath, stagingStatus, {amending} = {}) {
    if (!filePath) { return; }
    ...
    if (filePatch) {
      this.setState({filePath, filePatch, stagingStatus, amending}, () => {
        if (this.filePatchControllerPane) {
  ...
  async diveIntoFilePatchForPath(filePath, stagingStatus, {activate, amending} = {}) {
    await this.showFilePatchForPath(filePath, stagingStatus, {activate, amending});
