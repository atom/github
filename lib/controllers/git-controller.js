...
export default class GitController extends React.Component {
  ...
  async discardWorkDirChangesForPaths(filePaths) {
    ...
    const snapshots = await this.props.repository.storeBeforeAndAfterBlobs(
      filePaths,
      () => this.ensureNoUnsavedFiles(filePaths),
      destructiveAction,
    );
    return snapshots;
  }
  ...
  @autobind
  ensureNoUnsavedFiles(filePaths) {
  ...
  async discardLines(lines) {
    ...
    const snapshots = await this.props.repository.storeBeforeAndAfterBlobs(
      [filePath],
      () => this.ensureNoUnsavedFiles([filePath]),
      destructiveAction,
      {partial: true},
    );
    return snapshots;
  ...
  async undoLastDiscardForFilePath(filePath) {
    ...
    try {
      const results = await this.props.repository.undoLastDiscardInTempFiles([filePath], isSafe, {partial: true});
      if (!results) { return; }
