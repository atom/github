...
export default class GitController extends React.Component {
  ...
  async discardLines(lines) {
    ...
    await this.props.repository.storeBeforeAndAfterBlobs(relFilePath, () => {
  ...
  async undoLastDiscard(filePath) {
    ...
    if (editor && editor.getBuffer().isModified()) {
      this.props.notificationManager.addError('Cannot undo last discard.', {description: 'You have unsaved changes.'});
      return;
    }
    try {
      await this.props.repository.attemptToRestoreBlob(filePath);
    } catch (e) {
      if (e.message === 'Cannot restore file. Contents have been modified since last discard.') {
        this.props.notificationManager.addError('Cannot undo last discard.', {description: 'Contents have been modified since last discard.'});
      }
    }
  }
