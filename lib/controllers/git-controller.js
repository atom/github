...
export default class GitController extends React.Component {
  ...
  async discardLines(lines) {
    const filePath = path.join(this.props.repository.getWorkingDirectoryPath(), this.state.filePatch.getPath());
    const editor = this.props.workspace.getTextEditors().find(e => e.getPath() === filePath);
    let buffer;
    if (editor) {
    ...
    // FIXME: importing this directly into tests didn't work
