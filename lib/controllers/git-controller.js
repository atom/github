...
export default class GitController extends React.Component {
  ...
  onRepoRefresh(repo) {
    if (repo) { this.refreshAtomGitRepo(repo); }
  ...
  refreshAtomGitRepo(repository) {
    const directory = repository.getWorkingDirectoryPath();
    const atomGitRepo = this.props.project.getRepositories().find(repo => repo.getWorkingDirectory() === directory);
