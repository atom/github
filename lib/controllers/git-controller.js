  ...
  async discardLines(lines) {
    const isSafe = () => {
      ...
      if (editor && editor.isModified()) {
        this.props.notificationManager.addError('Cannot discard lines.', {description: `You have unsaved changes in ${relFilePath}.`});
        return false;
      } else {
        return true;
      }
    ...
    const snapshots = await this.props.repository.storeBeforeAndAfterBlobs(relFilePath, isSafe, async () => {
  ...
  async undoLastDiscard(filePath) {
    ...
    const isSafe = () => {
      ...
      if (editor && editor.getBuffer().isModified()) {
        this.props.notificationManager.addError('Cannot undo last discard.', {description: `You have unsaved changes in ${relFilePath}.`});
        return false;
      }
      return true;
    };
    try {
      const {resultPath, conflict} = await this.props.repository.undoLastDiscardInTempFile(filePath, isSafe);
      if (!conflict) {
        await this.proceedWithLastDiscardUndo(resultPath, relFilePath, absFilePath);
      } else {
        this.props.confirm({
          message: 'Undo will result in conflicts.',
          detailedMessage: 'Would you like to proceed and resolve conflicts in file?',
          buttons: {
            'Resolve conflicts': () => { this.proceedWithLastDiscardUndo(resultPath, relFilePath, absFilePath); },
            'Open in new file': () => { this.openConflictInNewFile(resultPath); },
            'Cancel undo': () => { /* do nothing */ },
          },
        });
      }
    } catch (e) {
