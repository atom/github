...
export default class GitController extends React.Component {
  ...
  getUnsavedFiles(filePaths) {
    const editors = this.props.workspace.getTextEditors();
    return filePaths.filter(filePath => {
      const absFilePath = path.join(this.props.repository.getWorkingDirectoryPath(), filePath);
      const editor = editors.find(e => e.getPath() === absFilePath);
  ...
  @autobind
  async undoLastDiscard(options = {}) {
    let filePaths;
    if (options.partial && options.filePath) {
      filePaths = [options.filePath];
    } else {
      const lastSnaphots = this.props.repository.getLastHistorySnapshotsForFileDiscards();
      if (!lastSnaphots) { return; }
      filePaths = Object.keys(lastSnaphots);
    }
    try {
      const results = await this.props.repository.undoLastDiscardInTempFiles(
      ...
      if (conflicts.length === 0) {
        await this.proceedWithLastDiscardUndo(results, options.partial);
      } else {
        await this.promptAboutConflicts(results, conflicts, options.partial);
      }
    } catch (e) {
      if (e instanceof GitError && e.stdErr.match(/fatal: Not a valid object name/)) {
        this.cleanUpHistoryForFilePaths(filePaths, options.partial);
      } else {
  ...
  async promptAboutConflicts(results, conflicts, partial) {
    ...
    if (choice === 0) {
      await this.proceedWithLastDiscardUndo(results, partial);
    } else if (choice === 1) {
      await this.openConflictsInNewEditors(conflicts.map(({resultPath}) => resultPath));
    }
  }

  cleanUpHistoryForFilePaths(filePaths, partial) {
    if (partial && filePaths.length === 1) {
      this.props.repository.clearPartialDiscardHistoryForPath(filePaths[0]);
    } else {
      this.props.repository.clearWholeFileDiscardHistory();
    }
  ...
  async proceedWithLastDiscardUndo(results, partial) {
    const promises = results.map(async ({filePath, resultPath}) => {
      const absFilePath = path.join(this.props.repository.getWorkingDirectoryPath(), filePath);
      await copyFile(resultPath, absFilePath);
    });
    await Promise.all(promises);
    if (partial && results.length === 1) {
      await this.props.repository.popUndoHistoryForFilePath(results[0].filePath);
    } else {
      await this.props.repository.popUndoHistoryForFileDiscards();
    }
  }
