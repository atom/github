...
export default class GitController extends React.Component {
            ...
            <Tabs.Panel title="Git">
              <EtchWrapper
                ...
                <GitPanelController
                  ...
                  undoLastDiscard={this.undoLastDiscardForFileList}
                />
              </EtchWrapper>
            </Tabs.Panel>
        ...
        <PaneItem
          ...
          <EtchWrapper ref={c => { this.filePatchController = c; }} reattachDomNode={false}>
            <FilePatchController
              ...
              undoLastDiscardForFilePath={this.undoLastDiscardForFilePath}
            />
          </EtchWrapper>
        </PaneItem>
      </div>
  ...
  @autobind
  async undoLastDiscardForFilePath(filePath) {
    ...
    try {
      const results = await this.props.repository.undoLastDiscardInTempFiles(
        [filePath],
        () => this.ensureNoUnsavedFiles([filePath], 'Cannot undo last discard.'),
        {partial: true},
      );
      if (!results) { return; }
      const {resultPath, conflict} = results[0];
      if (!conflict) {
