...
export default class PullRequestReviewsController extends React.Component {
  static propTypes = {
    ...
    pullRequest: PropTypes.shape({
      reviews: RelayConnectionPropType(PropTypes.object),
    }),
  ...
  render() {
    ...
    const commentThreads = Object.keys(this.state)
      .reverse()
      .map(rootCommentId => {
        return {
          rootCommentId,
  ...
  collectComments = ({
    ...
    if (!stillFetchingReviews) {
      const stillFetchingComments = [...this.reviewsById.values()].some(
        review => review.fetchingMoreComments,
      );
  ...
  groupCommentsByThread() {
    ...
    const sortedReviews = [...this.reviewsById.values()].sort(
      this.compareReviewsByDate,
    );
