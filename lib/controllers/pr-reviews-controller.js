...
export default class PullRequestReviewsController extends React.Component {
  ...
  collectComments = ({reviewId, submittedAt, comments, fetchingMoreCommnts}) => {
    this.reviewsById.set(reviewId, {submittedAt, comments, fetchingMoreCommnts});
    const stillFetchingReviews = this.props.relay.hasMore();
    if (!stillFetchingReviews) {
      const stillFetchingComments = [...this.reviewsById.values()].some(review => review.fetchingMoreCommnts);
  ...
  groupCommentsByThread() {
    ...
    const sortedReviews = [...this.reviewsById.values()].sort(this.sortByDate);
