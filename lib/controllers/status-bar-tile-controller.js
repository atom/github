...
export default class StatusBarTileController extends React.Component {
  ...
  fetchData = repository => {
    return yubikiri({
      ...
      currentRemote: async query => repository.getRemoteForBranch((await query.currentBranch).getName().short()),
      aheadCount: async query => repository.getAheadCount((await query.currentBranch).getName().short()),
      behindCount: async query => repository.getBehindCount((await query.currentBranch).getName().short()),
  ...
  push(data) {
    return ({force, setUpstream} = {}) => {
      return this.props.repository.push(data.currentBranch.getName().full(), {
        force,
        setUpstream,
        refSpec: data.currentBranch.getPushRefSpec(),
      });
    };
  }
  ...
  pull(data) {
    return () => {
      return this.props.repository.pull(data.currentBranch.getName().full(), {
        refSpec: data.currentBranch.getPullRefSpec(),
      });
    };
  }
  ...
  fetch(data) {
    return () => {
      ...
      return this.props.repository.fetch(upstream.getRemoteRefName().full(), {
