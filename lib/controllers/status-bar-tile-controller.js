...
export default class StatusBarTileController {
    ...
    if (modelData) {
      return (
        <div className="github-StatusBarTileController">
          ...
          />
          {atom.inDevMode() && <a style="margin: 0 5px;" onclick={this.handleOpenGitTimingsView}>
            🐛
          </a>}
        </div>
      );
  ...
  async setInProgressWhile(block) {
    if (this.inProgress) {
      return;
    }
  ...
  async push(options) {
    ...
    await this.setInProgressWhile(() => this.getActiveRepository().push(branchName, options));
  }
  ...
  async pull() {
    ...
    await this.setInProgressWhile(() => this.getActiveRepository().pull(branchName));
  }
  ...
  async fetch() {
    ...
    await this.setInProgressWhile(() => this.getActiveRepository().fetch(branchName));
  }
