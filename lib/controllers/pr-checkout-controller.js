...
export class BarePullRequestCheckoutController extends React.Component {
  ...
  nextCheckoutOp() {
    ...
    const fromPullRefspec =
      ...
      headPush.getRemoteRefName().short() === `pull/${pullRequest.number}/head`;
    ...
    const fromHeadRepo =
      ...
      headPush.getRemoteRefName().short() === pullRequest.headRefName;
  ...
  async checkout() {
    ...
    if (pullTargets.length > 0) {
      localRefName = pullTargets[0].getName().full();
