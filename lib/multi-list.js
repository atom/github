...
export default class MultiList {
  ...
  selectNextItem ({wrap, stopAtBounds} = {}) {
    ...
    if (newItemIndex < listInfo.items.length) {
      ...
      if (!stopAtBounds) this.selectNextList({selectFirst: true, wrap})
    }
  }

  selectPreviousItem ({wrap, stopAtBounds} = {}) {
    const key = this.getSelectedListKey()
    const listInfo = listInfoByKey.get(key)
    const newItemIndex = listInfo.selectedIndex - 1
    if (newItemIndex >= 0) {
      ...
      if (!stopAtBounds) this.selectPreviousList({selectLast: true, wrap})
    }
  }
