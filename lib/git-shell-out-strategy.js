...
export default class GitShellOutStrategy {
  ...
  commit(message, {allowEmpty, amend} = {}) {
    const helperPath = path.resolve(__dirname, '..', 'bin', 'gpg-no-tty.sh');
    const args = ['-c', `gpg.program=${helperPath}`, 'commit', '-m', message];
    if (amend) { args.push('--amend'); }
    if (allowEmpty) { args.push('--allow-empty'); }
    return this.exec(args).catch(err => {
  ...
  async pull(branchName) {
    const remote = await this.getRemoteForBranch(branchName);
    return this.exec(['pull', remote, branchName], null, true);
  }
