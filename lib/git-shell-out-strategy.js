...
export default class GitShellOutStrategy {
  ...
  /*
   * Provide an asynchronous callback to be used to asynchronously request input from the user for git operations.
   *
  ...
  exec(args, {stdin, useGitPromptServer, writeOperation} = GitShellOutStrategy.defaultExecArgs) {
    ...
    return this.commandQueue.push(async () => {
      ...
      if (useGitPromptServer) {
        ...
        const {socket, electron, launcher, helper} = await gitPromptServer.start(this.prompt);
        ...
        args.unshift('-c', `credential.helper=${launcher}`);
      }
      }
