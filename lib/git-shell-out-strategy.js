...
export default class GitShellOutStrategy {
  ...
  async resolveDotGitDir() {
    try {
      console.log(`resolveDotGitDir ${this.workingDir} 0`);
      await fsStat(this.workingDir); // fails if folder doesn't exist
      console.log(`resolveDotGitDir ${this.workingDir} 1`);
      const output = await this.exec(['rev-parse', '--resolve-git-dir', path.join(this.workingDir, '.git')]);
      console.log(`resolveDotGitDir ${this.workingDir} 2 ${output}`);
      const dotGitDir = output.trim();
      console.log(`resolveDotGitDir ${this.workingDir} 3 ${dotGitDir}`);
      ...
      console.log(`resolveDotGitDir ${this.workingDir} err`, e);
      return null;
    }
  }
  /* eslint-enable no-console */
