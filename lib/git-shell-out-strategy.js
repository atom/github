...
// win.loadURL('file:///Users/kuychaco/github/github/lib/renderer.html');
//
// const resolveByOperationId = new Map();
// const operationId = 0;
...
//   if (type === 'git') {
//     const resolve = resolveByOperationId.get(data.operationId);
//     global.requestCounter--;
//     if (data.err) {
//       const error = new GitError(data.err.message);
//       Object.assign(error, data.err);
//       return resolve(Promise.reject(error)); // TODO: check this
//     }
//     resolve(data.result);
...
export default class GitShellOutStrategy {
  ...
  constructor(workingDir, options = {}) {
    ...
    this.rendererProcessManager = new RendererProcessManager();
  }
  ...
  exec(args, {stdin, useGitPromptServer, writeOperation} = GitShellOutStrategy.defaultExecArgs) {
    ...
    return this.commandQueue.push(async () => {
      ...
      return new Promise(resolve => {
        ...
        this.rendererProcessManager.send(resolve, {
          args,
          workingDir: this.workingDir,
          options,
          timeSent,
        });
        // resolveByOperationId.set(++operationId, resolve);
        // win.webContents.send('ping', webContentsId, {
        //   args,
        //   workingDir: this.workingDir,
        //   options,
        //   operationId,
