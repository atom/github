...
export default class GitShellOutStrategy {
  ...
  async getDiffsForFilePath(filePath, {staged, baseCommit} = {}) {
    ...
    if (!staged && (await this.getUntrackedFiles()).includes(filePath)) {
      ...
      diffs.push(buildAddedFilePatch(filePath, binary ? null : contents, mode, realpath));
    }
    if (diffs.length > 2) {
      throw new Error(`Expected between 0 and 2 diffs for ${filePath} but got ${diffs.length}`);
    }
    return {diffs, digest};
  }
