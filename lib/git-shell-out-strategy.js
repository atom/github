...
export default class GitShellOutStrategy {
  ...
  getLocalBranches() {
    return forEachRef(this, 'refs/heads/**');
  }

  getRemoteBranches() {
    return forEachRef(this, 'refs/remotes/**');
  }
...
async function forEachRef(strategy, pattern) {
  const format = [
  ...
  const output = await strategy.exec(['for-each-ref', `--format=${format}`, pattern]);
