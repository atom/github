...
export default class GitShellOutStrategy {
  ...
  async getRefSha(ref) {
    const output = await this.exec(['show-ref', '--hash', ref]);
    const firstLineInd = output.search(LINE_ENDING_REGEX);
    if (firstLineInd === -1) {
      return output;
    } else {
      return output.slice(0, firstLineInd);
    }
  }
