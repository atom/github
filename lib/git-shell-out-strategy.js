...
export default class GitShellOutStrategy {
  ...
  async getAuthors(options = {}) {
    ...
    // %cn - committer name
    // %ce - committer email
    // %(trailers) - display the trailers of the body
    // TODO figure out why %(trailers:only,unfold) doesn't work

    const output = await this.exec([
      'log', '--pretty=format:%an %ae%x00%cn %ce%x00%(trailers)', '-z', '-n', max, ref, '--',
    ]);
    return output.split('\0').reduce((acc, line) => {
      if (line.match(/^co-authored-by: /i)) {
        const coAuthorLines = line.trim().split(LINE_ENDING_REGEX);
        coAuthorLines.forEach(coAuthor => {
          const [_, name, email] = coAuthor.match(/co-authored-by: (.*) <(.*)>/i); // eslint-disable-line no-unused-vars
          acc[email] = name;
        });
      } else if (line !== '') {
        const [_, name, email] = line.match(/(.*) (.*@.*)/); // eslint-disable-line no-unused-vars
        acc[email] = name;
      }
      return acc;
    }, {});
