...
export default class GitShellOutStrategy {
  ...
  async getRecentCommits(options = {}) {
    ...
    // %s - subject
    // %b - body
    const output = await this.exec(['log', '--pretty=%H%x00%ae%x00%at%x00%s%x00%b%x00', '--no-abbrev-commit', '-n', max, ref, '--'])
      .catch(err => {
    ...
    return output.trim().split('\n').filter(Boolean).map(line => {
      const [sha, authorEmail, authorDate, message, body] = line.split('\0');
      return {sha, authorEmail, authorDate: parseInt(authorDate), message, body};
    });
  }
