...
export default class GitShellOutStrategy {
  ...
  constructor(workingDir, options = {}) {
    ...
    this.rendererProcessManager = options.rendererProcessManager;
  }
  ...
  exec(args, {stdin, useGitPromptServer, writeOperation} = GitShellOutStrategy.defaultExecArgs) {
    ...
    return this.commandQueue.push(async () => {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        const {stdout, stderr, exitCode} = await this.executeGitCommand(args, options);
  ...
  async executeGitCommand(args, options) {
    const timeSent = performance.now(); // TODO: move to manager
    if (process.env.ATOM_GITHUB_INLINE_GIT_EXEC) {
      return GitProcess.exec(args, this.workingDir, options);
    } else {
      const rendererProcessManager = this.rendererProcessManager || RendererProcessManager.getInstance();
      await rendererProcessManager.getReadyPromise();
      return rendererProcessManager.request({
        args,
        workingDir: this.workingDir,
        options,
        timeSent,
