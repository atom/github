...
export default class GitShellOutStrategy {
  ...
  push (branchName) {
    // TODO: get remote from config
    return this.exec(['push', 'origin', branchName])
  }

  async getAheadCount (branchName) {
    // TODO: get remote from config
    const output = await this.exec(['rev-list', `origin/${branchName}..${branchName}`])
    return output.trim().split(LINE_ENDING_REGEX).filter(s => s.trim()).length
  }

  async getBehindCount (branchName) {
    // TODO: get remote from config
    const output = await this.exec(['rev-list', `${branchName}..origin/${branchName}`])
    return output.trim().split(LINE_ENDING_REGEX).filter(s => s.trim()).length
  }
}
