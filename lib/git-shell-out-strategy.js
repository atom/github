...
export default class GitShellOutStrategy {
  ...
  async commit(rawMessage, {allowEmpty, amend, coAuthors} = {}) {
    ...
    if (amend && rawMessage.length === 0) {
      const {unbornRef, message} = await this.getHeadCommit();
      msg = unbornRef ? rawMessage : message;
    } else {
      msg = rawMessage;
    }
  ...
  addCoAuthorsToMessage(message, coAuthors) {
    ...
    const msg = `${message}\n`.replace(/\s+$/, '\n');
    return this.mergeTrailers(msg, trailers);
  }
   ...
   */
  async getCommit(ref) {
    const [commit] = await this.getRecentCommits({max: 1, ref});
    return commit;
  }

  async getHeadCommit() {
    const [headCommit] = await this.getRecentCommits({max: 1, ref: 'HEAD'});
    return headCommit;
  }
  ...
  async getRecentCommits(options = {}) {
    ...
    if (output === '') { return [{sha: '', message: '', unbornRef: true}]; }
    ...
    for (let i = 0; i < fields.length; i += 5) {
      ...
      const {message, coAuthors} = extractCoAuthorsAndRawCommitMessage(body);
