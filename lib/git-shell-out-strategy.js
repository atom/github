...
export default class GitShellOutStrategy {
   ...
   */
  async getStatus () {
    const output = await this.exec(['status', '--untracked-files=all', '--porcelain'])
    ...
    output && output.split(LINE_ENDING_REGEX).forEach(async (line) => {
      if (line === '') return
      const [_, x, y, filePath] = line.match(/(.)(.)\s(.*)/)
      if (x === 'U' || y === 'U' || (x === 'A' && y === 'A')) {
        return mergeConflictFiles[filePath] = {
          ours: statusMap[x],
          theirs: statusMap[y],
