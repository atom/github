...
export default class GitShellOutStrategy {
  ...
  exec(args, {stdin, useGitPromptServer, writeOperation} = GitShellOutStrategy.defaultExecArgs) {
    ...
    return this.commandQueue.push(async () => {
      ...
      if (useGitPromptServer) {
        ...
        const {
          socket, tmpdir, electron, credentialHelper, askPass, sshWrapper, gpgWrapper, pinentry,
        } = await gitPromptServer.start(this.prompt);

        atomGithubTmp = tmpdir;
        ...
        env.ATOM_GITHUB_GPG_PATH = normalizeGitHelperPath(gpgWrapper.script);
        env.ATOM_GITHUB_PINENTRY_PATH = normalizeGitHelperPath(pinentry.script);
        env.ATOM_GITHUB_PINENTRY_LAUNCHER = normalizeGitHelperPath(pinentry.launcher);
        ...
        args.unshift('-c', `gpg.program=${normalizeGitHelperPath(gpgWrapper.launcher)}`);
      ...
      return new Promise(async (resolve, reject) => {
        ...
        if (diagnosticsEnabled) {
          ...
          if (atomGithubTmp) {
            [gpgLog, pinentryLog] = await Promise.all([
              readFile(path.join(atomGithubTmp, 'gpg-wrapper.log')).catch(() => ''),
              readFile(path.join(atomGithubTmp, 'gpg-pinentry.log')).catch(() => ''),
            ]);
          }
          ...
          if (headless) {
            ...
            if (pinentryLog.length !== 0) {
              summary += 'gpg pinentry:\n';
              summary += pinentryLog;
            }
            ...
            console.log(stderr);
            if (gpgLog.length !== 0) {
              console.log('%cgpg', headerStyle);
              console.log(gpgLog);
            }
            if (pinentryLog.length !== 0) {
              console.log('%cgpg pinentry', headerStyle);
              console.log(pinentryLog);
            }
            console.groupEnd();
          }
        }
  ...
  async getStatusBundle() {
    ...
    const results = parseStatus(output);
