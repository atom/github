...
export default class GitShellOutStrategy {
  ...
  exec(args, stdin = null, useGitPromptServer = false) {
    ...
    const timingMarker = GitTimingsView.generateMarker(formattedArgs);
    timingMarkers.push(timingMarker);
    timingMarker.markStart('queued');

    return this.commandQueue.push(async () => {
      timingMarker.markStart('prepare');
      let gitPromptServer;
      const env = {};
      ...
      timingMarker.markStart('execute');
      return GitProcess.exec(args, this.workingDir, options)
        .then(({stdout, stderr, exitCode}) => {
          timingMarker.markEnd('execute');
          if (process.env.PRINT_GIT_TIMES) {
