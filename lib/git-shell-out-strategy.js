...
export default class GitShellOutStrategy {
  ...
  constructor(workingDir, options = {}) {
    ...
    if (gpgHelperPath === null) {
      gpgHelperPath = path.resolve(getPackageRoot(), 'bin', 'gpg-wrapper.sh');
    }
  }
  ...
  exec(args, {stdin, useGitPromptServer, writeOperation} = GitShellOutStrategy.defaultExecArgs) {
    ...
    return this.commandQueue.push(async () => {
      ...
      if (useGitPromptServer) {
        ...
        const {
          socket, tmpdir, electron, credentialHelper, askPass, sshWrapper, pinentry,
        ...
        env.ATOM_GITHUB_PINENTRY_PATH = pinentry.launcher;
  ...
  gpgExec(args, options = {}) {
    const gpgArgs = ['-c', `gpg.program=${gpgHelperPath}`].concat(args);
