...
export default class GitShellOutStrategy {
    ...
    return this.commandQueue.push(
      async () => {
        ...
        if (useGitPromptServer) {
          ...
          env.ATOM_GITHUB_SOCK_PATH = normalizeGitHelperPath(
            gitTempDir.getSocketPath(),
          );

          env.ATOM_GITHUB_WORKDIR_PATH = this.workingDir;
          env.ATOM_GITHUB_DUGITE_PATH = getDugitePath();
          env.ATOM_GITHUB_KEYTAR_STRATEGY_PATH = getSharedModulePath(
            'keytar-strategy',
          );
  ...
  async getDiffsForFilePath(filePath, {staged, baseCommit} = {}) {
    ...
    if (rawDiffs.length > 2) {
      throw new Error(
        `Expected between 0 and 2 diffs for ${filePath} but got ${
          rawDiffs.length
        }`,
      );
    }
