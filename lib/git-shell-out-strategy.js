...
import {
  ...
  normalizeGitHelperPath, toNativePathSep, toGitPathSep, LINE_ENDING_REGEX,
} from './helpers';
...
export default class GitShellOutStrategy {
  ...
  commit(message, {allowEmpty, amend} = {}) {
    ...
    // determine correct message arguments
    if (typeof message === 'object') {
      args = args.concat(['-F', message.filePath]);
    } else if (typeof message === 'string') {
      if (message.length === 0 && amend) {
        args = args.concat(['--reuse-message=HEAD']);
      } else {
    ...
    try {
      ...
      return output.split('\0')
        .reduce((acc, line) => {
          ...
          trailers
            .split('\n')
            .map(trailer => trailer.match(/^co-authored-by. (.+?) <(.+?)>$/i))
