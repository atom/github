...
import {readFile, fileExists, writeFile, fsStat, deleteFileOrFolder} from './helpers';
...
export default class GitShellOutStrategy {
  ...
  async updateIndex(filePath, commonSha, oursSha, theirsSha) {
    const fileMode = await this.getFileMode(filePath);
    const indexInfo = dedent`
      0 0000000000000000000000000000000000000000\t${filePath}
      ${fileMode} ${commonSha} 1\t${filePath}
      ${fileMode} ${oursSha} 2\t${filePath}
      ${fileMode} ${theirsSha} 3\t${filePath}
    `;
