...
export default class GithubPackage {
  ...
  async activate(state = {}) {
    ...
    this.githubEnabled = !!atom.config.get('github.githubEnabled');
    this.subscriptions.add(
      ...
      atom.config.onDidChange('github.githubEnabled', ({newValue}) => {
        this.githubEnabled = !!newValue;
        this.rerender();
      }),
    );
  ...
  projectPathForItemPath(filePath) {
    ...
    return this.project.getPaths().find(projectPath => {
      const fixedPath = projectPath + path.sep;
      return filePath.startsWith(fixedPath);
    });
  }
