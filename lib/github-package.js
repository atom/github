...
export default class GithubPackage {
  ...
  async activate(state = {}) {
    ...
    const newProject =
      this.savedState.firstRun !== undefined
        ? this.savedState.firstRun
        : this.savedState.newProject;
    ...
    this.subscriptions.add(
      ...
      this.workspace
        .getCenter()
        .onDidChangeActivePaneItem(this.scheduleActiveContextUpdate),
  ...
  serialize() {
    ...
    const activeRepositoryPath = activeRepository
      ? activeRepository.getWorkingDirectoryPath()
      : null;
  ...
  async scheduleActiveContextUpdate(savedState = {}) {
    ...
    await this.activeContextQueue.push(
      this.updateActiveContext.bind(this, savedState),
      {parallel: false},
    );
  }
  ...
  async getNextContext(savedState) {
    ...
    const active = await fromPaneItem(
