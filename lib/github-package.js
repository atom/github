...
export default class GithubPackage {
  ...
  async activate(state = {}) {
    ...
    if (state.activeRepositoryPath) {
      const repo = await this.repositoryForProjectPath(state.activeRepositoryPath);
      if (repo) {
  ...
  async updateActiveRepository() {
    ...
    if (activeItem && typeof activeItem.getPath === 'function') {
      ...
      if (projectPath) {
        nextActiveRepository = await this.repositoryForProjectPath(projectPath);
      }
    } else {
  ...
  async watchRepoForUpdates(repository) {
    ...
    if (!this.changeObserversByProjectPath.get(repository.getWorkingDirectoryPath())) {
      ...
      this.subscriptions.add(
        ...
        changeObserver.onDidUpdateHeadOrMergeHead(() => this.refreshAtomGitRepository(repository)),
      );
  ...
  async repositoryForProjectPath(projectDirectoryPath) {
    let repository = this.repositoriesByProjectPath.get(projectDirectoryPath);
    if (!repository) {
      repository = await Repository.open(projectDirectoryPath);
  ...
  repositoryForWorkdirPath(workdirPath) {
    return this.repositoryForProjectPath(this.projectPathForItemPath(workdirPath));
  }

  getChangeObserverForWorkdirPath(workdirPath) {
    return this.changeObserversByProjectPath.get(workdirPath);
  }
