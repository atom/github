...
export default class GithubPackage {
  constructor (workspace, project) {
    ...
    this.commitPanelComponent = new CommitPanelComponent({repository: null})
  }

  async activate () {
    await this.updateActiveRepository()
    await this.refreshActiveRepositoryStagingArea()
  ...
  refreshActiveRepositoryStagingArea () {
    if (this.getActiveRepository() == null) {
      return Promise.resolve()
    } else {
      return this.getActiveRepository().getStagingArea().refresh()
    }
  }
  ...
  async repositoryForProjectDirectory (projectDirectory) {
    ...
    if (!repository) {
      ...
      if (atomRepository == null || atomRepository.async == null) {
        return
      } else {
