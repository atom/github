...
export default class GithubPackage {
  constructor (workspace, project, commandRegistry, notificationManager) {
    ...
    this.gitPanelController = new GitPanelController({
      ...
      commandRegistry,
      notificationManager,
      didSelectFilePath: this.didSelectFilePath.bind(this),
  ...
  async didSelectFilePath (filePath, stagingStatus, {activate, amending} = {}) {
    ...
    if (filePatch) {
      let containingPane
      const update = this.didSelectFilePath.bind(this, filePath, stagingStatus, {amending})
