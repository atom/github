...
export default class GithubPackage {
  constructor(workspace, project, commandRegistry, notificationManager, config) {
    ...
    this.config = config;
    this.statePerProjectPath = new Map();
  ...
  async deactivate() {
    ...
    await this.destroyModelsForPaths(Array.from(this.statePerProjectPath.keys()));
  }
  ...
  async didChangeProjectPaths(projectPaths) {
    this.updateActiveRepository();
    const previousProjectPaths = Array.from(this.statePerProjectPath.keys());
  ...
  getRepositoryForWorkdirPath(workdirPath) {
    const models = this.statePerProjectPath.get(workdirPath);
    ...
    return Promise.all(
      addedPaths.map(projectPath => {
        ...
        if (repositoryPromise) {
          ...
          this.statePerProjectPath.set(projectPath, {repositoryPromise, changeObserverPromise});
  ...
  destroyModelsForPaths(removedPaths) {
    return Promise.all(
      removedPaths.map(async projectPath => {
        ...
        this.statePerProjectPath.delete(projectPath);
