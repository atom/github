...
export default class GithubPackage {
  ...
  async didSelectFilePatch (filePath, stagingStatus, {focus, amending} = {}) {
    const repository = this.getActiveRepository()
    const filePatch = await repository.getFilePatchForPath(filePath, stagingStatus === 'staged', amending)
    let containingPane
