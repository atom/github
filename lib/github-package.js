...
import {AtomContext} from './context/atom';
import {WorkdirPoolContext, ActiveWorkdirContext} from './context/workdir';
...
export default class GithubPackage {
  constructor({atomEnv, renderFn, loginModel}) {
    ...
    this.atom = atomEnv;
    this.configPath = path.join(this.atom.configDirPath, 'github.cson');

    this.styleCalculator = new StyleCalculator(this.atom.styles, this.atom.config);
    this.startOpen = false;
    this.activated = false;

    const criteria = {
      projectPathCount: this.project.getPaths().length,
      initPathCount: (this.atom.getLoadSettings().initialPaths || []).length,
    };

    this.pipelineManager = getRepoPipelineManager({
      confirm: this.atom.confirm.bind(this.atom),
      notifications: this.atom.notifications,
      workspace: this.atom.workspace,
    });
  ...
  rerender(callback) {
    ...
    this.renderFn(
      <AtomContext.Provider value={this.atom}>
        <WorkdirPoolContext.Provider value={this.contextPool}>
          <ActiveWorkdirContext.Provider value={this.activeContext}>
            <RootController
        ...
        </WorkdirPoolContext.Provider>
      </AtomContext.Provider>, this.element, callback,
    );
  }
