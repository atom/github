...
import GithubPackageController from './controllers/github-package-controller'
...
export default class GithubPackage {
  constructor (workspace, project, commandRegistry, notificationManager) {
    ...
    this.gitPanelController = new GitPanelController({
      ...
      commandRegistry,
      notificationManager,
      didSelectFilePath: this.showFilePatchForPath.bind(this),
      didSelectMergeConflictFile: this.showMergeConflictFileForPath.bind(this),
  ...
  async showFilePatchForPath (filePath, stagingStatus, {activate, amending} = {}) {
    ...
    if (filePatch) {
      let containingPane
      const onRepoRefresh = () => {
        this.showFilePatchForPath(filePath, stagingStatus, {amending})
      }
      if (this.filePatchController) {
        this.filePatchController.update({filePatch, repository, stagingStatus, onRepoRefresh})
        containingPane = this.workspace.paneForItem(this.filePatchController)
      } else {
        this.filePatchController = new FilePatchController({filePatch, repository, stagingStatus, onRepoRefresh})
