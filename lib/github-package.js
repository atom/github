...
import WorkdirContext, {absentWorkdirContext, LOADINGLIKE, ABSENTLIKE} from './models/workdir-context';
...
export default class GithubPackage {
  constructor(workspace, project, commandRegistry, notificationManager, tooltips, styles, config,
    ...
    this.activeContext = this.getInitialContext(getLoadSettings());
  ...
  /**
   * Synchronously determine the initial WorkdirContext of a newly constructed package instance based on:
   * - the number of projects already loaded into the workspace
   * - any initial paths specified in the load settings
   */
  getInitialContext(loadSettings) {
    const projectPathCount = this.project.getPaths().length;
    const initLocationCount = (loadSettings.locationsToOpen || []).length;

    if (projectPathCount === 1 || (projectPathCount === 0 && initLocationCount === 1)) {
      return WorkdirContext.undetermined(LOADINGLIKE);
    } else {
      return WorkdirContext.undetermined(ABSENTLIKE);
    }
  }
