...
export default class GithubPackage {
  static getRootController() {
    return rootController;
  }

  static getActiveWorkingDirectory() {
  ...
  rerender(callback) {
    ...
    activeWorkdir = this.getActiveWorkdir();
    ReactDom.render(
      <RootController
        ref={c => { this.controller = c; rootController = c; }}
        ...
        removeFilePatch={this.removeFilePatch}
  ...
  @autobind
  createFilePatchPaneItem({uri}) {
    return FilePatchPaneItem.opener(uri);
  }
  ...
  createFilePatchControllerStub({uri} = {}) {
    ...
    this.filePatchItems.push({uri, item});
    // this.rerender? or just in opener?
    return item;
  }

  @autobind
  removeFilePatch(uriToRemove) {
    this.filePatchItems = this.filePatchItems.filter(({uri}) => uri !== uriToRemove);
  }
