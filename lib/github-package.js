...
export default class GithubPackage {
  ...
  consumeStatusBar (statusBar) {
    const tile = statusBar.addRightTile({item: this.statusBarController, priority: -100})
  ...
  didSelectMergeConflictFile (filePath) {
    this.workspace.open(filePath)
  }
  ...
  async updateActiveRepository () {
    ...
    if (activeRepository && activeRepository !== this.activeRepository) {
      ...
      await this.statusBarController.update({repository: this.getActiveRepository()})
