...
export default class GithubPackage {
    ...
    this.subscriptions.add(
      ...
      this.workspace.addOpener(uri => {
        if (uri.startsWith('atom-github://file-patch/')) {
          // const item = this.createFilePatchControllerStub({uri: uri + `&id=${++id}`});
          const item = this.createFilePatchControllerStub({uri});
          lastItem = item;
          this.rerender();
          return item;
        } else if (uri.startsWith('testing://repeat')) {
          return lastItem;
        } else {
          return null;
        }
  ...
  getRepositoryForWorkdir(projectPath) {
    return this.contextPool.getContext(projectPath).getRepository();
  }
