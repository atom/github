...
export default class GithubPackage {
  constructor(workspace, project, commandRegistry, notificationManager, tooltips, styles, config,
    ...
    this.activeContext = WorkdirContext.guess(criteria);
  ...
  activate(state = {}) {
    setTimeout(() => {
      const pack = atom.packages.getActivePackage('github');
      if (!(atom.inDevMode() || atom.inSpecMode()) && pack && !pack.bundledPackage) {
