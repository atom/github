...
import {run as HubKit} from '@hubkit/kit';
import Schema from '@hubkit/schema.kit';
...
export default class RelayNetworkLayerManager {
  ...
  static getHubKit() {
    ...
    const use = HubKit();
    const kit = use(Schema({auth: {service: 'atom-github'}}));
    window.hubkitPlayground = (port = 0) => {
      let Http;
      try {
        Http = require('@hubkit/http.kit');
      } catch (e) {
        // eslint-disable-next-line no-console
        console.log("%c Couldn't require @hubkit/http.kit. Try running ", 'font-size:12pt; color:red');
        // eslint-disable-next-line no-console
        console.log('%c npm i github:editor-tools/hubkit#kit.http.kit.npm', 'font-size:12pt; color:red');
      }
      Http && use(Http({port, insecure: true}));
    };
    ...
    const network = Network.create(
      async function fetchQuery(operation, variables, _cacheConfig, _uploadables) {
        const payload = await kit.query(operation.text, variables);

        if (payload.data && payload.data.errors && payload.data.errors.length > 0) {
          const e = new Error(`HubKit returned an error for query ${operation.name}.`);
          e.response = payload;
    ...
    return this._hubKitEnvironment = env;
  }
}
