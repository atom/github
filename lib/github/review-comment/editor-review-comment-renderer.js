...
export default class EditorReviewCommentRenderer {
  ...
  constructor (editor, positioner, gitHubModel) {
    ...
    this.subscriptions.add(
      ...
      this.positioner.onDidUpdateCommentMarker(({comment, range}) => this.updateCommentMarkerRange(comment.id, range))
    )
  }
  ...
  addComment (commentId, comment, range) {
    ...
    const marker = this.editor.markBufferRange(range, {reversed: true, invalidate: 'never'})
    window.mRenderer = marker
  ...
  updateCommentMarkerRange (commentId, range) {
    if (this.markerAndComponentByCommentId.has(commentId)) {
      const {marker} = this.markerAndComponentByCommentId.get(commentId)
