...
export default class BufferReviewCommentPositioner {
  ...
  constructor (buffer) {
    ...
    this.subscriptions.add(
      this.buffer.onDidStopChanging(() => { this.refreshOutdated() }))
  }
  ...
  add (commitId, commitBufferContents, commentsById) {
    ...
    for (let _id of Object.keys(rangesByCommentId)) {
      ...
      const marker = this.buffer.markRange(markerRange, {reversed: true, invalidate: 'touch'})

      const subscription = marker.onDidChange(({isValid, oldHeadPosition, oldTailPosition, newHeadPosition, newTailPosition}) => {
        if (!isValid) {
          for (let row = newHeadPosition.row; row <= newTailPosition.row; row++) {
            if (this.buffer.lineForRow(row) === commentLineText) {
              const newRange = new Range([row, 0], [row, endOfText])
              this.emitter.emit('did-update-comment-marker', {comment, range: newRange})
              marker.setRange(newRange)
              return
            }
          }
