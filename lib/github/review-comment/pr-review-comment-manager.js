...
// checks to see if `child` is contained within `parent`;
// neither `parent` nor `child` actually need to exist
...
export default class PrReviewCommentManager {
  constructor (gitHubModel, rootPath, commentEmitter) {
  ...
  async relativizePath (path) {
    const repo = await this.gitHubModel.getRepo(this.rootPath)
  ...
  addSubscriptions () {
    ...
    this.subscriptions.add(
      atom.workspace.observeActivePaneItem(async (item) => {
        if (this.isValidEditor(item)) { this.renderEditorComments(item) }
      }),
      atom.workspace.onDidDestroyPaneItem(async ({item}) => {
        if (this.isValidEditor(item)) { this.destroyEditorReviewCommentRenderer(item) }
      })
    )
  }

  isValidEditor (item) {
    return isEditor(item) && isContainedWithin(this.rootPath, item.getPath())
  }
  ...
  async fetchComments () {
    const repoComments = await this.gitHubModel.getBufferComments(this.rootPath)

    const prComments = this.getCommentsForUrl(repoComments)
    this.commentsByPath = groupBy(prComments, 'path')
    window.cbp = this.commentsByPath
