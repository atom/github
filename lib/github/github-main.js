...
import GitHubModalSignInComponent from './auth/github-modal-sign-in-component'
import GitHubModel from './github-model'
import ProjectPrReviewCommentManager from './review-comment/project-pr-review-comment-manager'
import CommentEmitter from './review-comment/review-comment-emitter'
...
export default {
  ...
  auth: null,
  gitHubModel: null,
  projectPrReviewCommentManager: null,
  config: {
    authorizationURL: {
  ...
  activate: function (state) {
    ...
    this.projectPrReviewCommentManager = new ProjectPrReviewCommentManager(atom.project, this.gitHubModel, this.commentEmitter)
    this.subscriptions.add(new Disposable(() => this.projectPrReviewCommentManager.destroy()))
  ...
  deactivate: function () {
    ...
    this.auth.destroy()
    this.auth = null
    this.projectPrReviewCommentManager.destroy()
  },
  ...
  showStatusBarItem: function () {
    if (!GitHubStatusBarComponent) {
      GitHubStatusBarComponent = require('./status-bar/github-status-bar-component')
    }
