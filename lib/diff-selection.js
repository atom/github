...
export default class DiffSelection {
  ...
  constructor (diffViewModel: DiffViewModel, options: {mode?: string, headPosition?: Position, tailPosition?: Position} = {}) {
...
function positionComparatorAsc (posA: Position, posB: Position): number {
  const lastElementA = posA[2]
  const lastElementB = posB[2]
  if (posA[0] !== posB[0]) {
    return posA[0] - posB[0]
  } else if (posA[1] !== posB[1]) {
    return posA[1] - posB[1]
  } else if (lastElementA && lastElementB && lastElementA !== lastElementB) {
