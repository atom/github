...
class BareDecoration extends React.Component {
  ...
  addGutterDecoration(decorable, opts) {
    ...
    let gutter = editor.gutterWithName(this.props.gutterName);
    if (!gutter) {
      gutter = editor.addGutter({name: this.props.gutterName, type: 'gutter'});
    }
    gutter.decorateMarker(decorable, opts);
  }
  ...
  createDecoration() {
    ...
    if (this.props.type === 'gutter') {
      this.decorationHolder.setter(this.addGutterDecoration(decorable, opts));
    } else {
