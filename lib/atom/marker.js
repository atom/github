...
import {fromChangeEvent, fromMarker} from '../models/marker-position';
...
class BareMarker extends React.Component {
  ...
  componentWillUnmount() {
    this.markerHolder.map(marker => console.log(`destroying marker ${marker.id} due to unmount`));
  ...
  createMarker() {
    this.markerHolder.map(marker => console.log(`destroying marker ${marker.id} to create a new one`));
    this.markerHolder.map(marker => marker.destroy());
    ...
    this.props.markableHolder.map(markable => {
      ...
      const markableKind = markable.constructor.name.toLowerCase().replace(/^display/, '');
      console.log(`created marker ${marker.id} on ${markableKind} ${markable.id}`);

      this.subs.add(
        marker.onDidChange(this.didChange),
        marker.onDidChange(() => {
          console.log(`marker ${marker.id} was changed to ${marker.getBufferRange().toString()}`);
          return null;
        }),
      );
  ...
  updateMarkerPosition() {
    this.markerHolder.map(marker => {
      const before = fromMarker(marker);
      const after = this.props.position;
      if (!before.matches(after)) {
        console.log(
          `updating marker ${marker.id} from ${before.toString()} to ${after.toString()}`,
        );
      }
      this.props.position.setIn(marker);
      return null;
    });
  }
