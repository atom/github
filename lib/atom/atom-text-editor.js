...
export default class AtomTextEditor extends React.Component {
  ...
  componentDidMount() {
    ...
    // syntax highlighting
    let scopeName = 'text.plain';
    const buffer = modelProps.buffer;
    if (buffer) {
      const bufferPath = buffer.getPath();
      if (bufferPath) {
        // find scope name based on the path and text
        scopeName = atom.grammars.selectGrammarWithScore(
          bufferPath,
          buffer.getText()).scopeName;
      }
    }
