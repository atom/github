...
export default class DiffViewModel {
  ...
  constructor ({uri, pathName, deserializer, fileDiffViewModel, fileList, pending, gitService}: {uri: string, pathName: string, deserializer: string, fileDiffViewModel: FileDiffViewModel, fileList: FileList, pending: boolean, gitService: GitService}) {
  ...
  async toggleSelectedLinesStageStatus (): Promise<void> {
    ...
    for (let fileDiffIndex in this.selectionState) {
      ...
      for (let diffHunkIndex in selectedHunkIndices) {
        ...
        if (stage) {
          linesByHunk[selectedHunk.toString()] = {linesToStage}
        } else {
          linesByHunk[selectedHunk.toString()] = {linesToUnstage: linesToStage}
        }
      }
