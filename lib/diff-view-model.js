...
export default class DiffViewModel {
  ...
  constructor ({uri, deserializer, title, fileList, pathName, pending}: {uri: string, deserializer: any, title?: string, fileList: FileList, pathName: string, pending: boolean}) {
    ...
    if (!title) {
      const firstFile = this.fileList.getFiles()[0]
      title = ifNotNull(firstFile, f => f.getNewPathName()) || ifNotNull(firstFile, f => f.getOldPathName())
    }
    this.title = title || ''
