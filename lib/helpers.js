...
export function createItem(node, props) {
  const item = {
    getElement: () => node,
  };

  if (props.title) { item.getTitle = () => props.title; }
  if (props.iconName) { item.getIconName = () => props.iconName; }
  if (props.defaultLocation) { item.getDefaultLocation = () => props.defaultLocation; }
  if (props.preferredWidth) { item.getPreferredWidth = () => props.getPreferredWidth; }
  if (props.uri) { item.getURI = () => props.uri; }
