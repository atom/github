...
export function getDugitePath() {
  if (!DUGITE_PATH) {
    ...
    console.log("require.resolve says it's", DUGITE_PATH);
    if (!path.isAbsolute(DUGITE_PATH)) {
      console.log("but it's not absolute! (cause we're snapshotted mebbe)");
      // Assume we're snapshotted
      const {resourcePath} = atom.getLoadSettings();
      console.log('atom says our resource path is', resourcePath);
      if (path.extname(resourcePath) === '.asar') {
        DUGITE_PATH = path.join(`${resourcePath}.unpacked`, 'node_modules', 'dugite');
        console.log("we're in an asar, so it's", DUGITE_PATH);
      } else {
        DUGITE_PATH = path.join(resourcePath, 'node_modules', 'dugite');
        console.log("but since we're not using asar it's", DUGITE_PATH);
      }
    } else {
      console.log("and it's absolute so the answer is", DUGITE_PATH);
    }
  }

  console.log("We're done! The final answer is ", DUGITE_PATH);
  return DUGITE_PATH;
}
