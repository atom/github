...
export function fsLStat(absoluteFilePath) {
  return new Promise((resolve, reject) => {
    fs.lstat(absoluteFilePath, (err, stats) => {
      if (err) { reject(err); } else { resolve(stats); }
    });
  });
}
...
export async function isFileSymlink(absoluteFilePath) {
  const stat = await fsLStat(absoluteFilePath);
  return stat.isSymbolicLink();
}

export function getRealPath(absoluteFilePath) {
  return new Promise((resolve, reject) => {
    fs.realpath(absoluteFilePath, (err, resolvedPath) => {
      if (err) { reject(err); } else { resolve(resolvedPath); }
    });
  });
}
