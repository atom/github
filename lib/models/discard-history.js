...
import {
  PartialFileDiscardHistory,
  WholeFileDiscardHistory,
} from './discard-history-stores';
...
export default class DiscardHistory {
  ...
  getLastSnapshots(partialDiscardFilePath = null) {
    if (partialDiscardFilePath) {
      return this.partialFileHistory.getLastSnapshotsForPath(
  ...
  async storeBeforeAndAfterBlobs(
    ...
    if (partialDiscardFilePath) {
      ...
      return await this.storeBlobsForWholeFileHistory(
        filePaths,
        isSafe,
        destructiveAction,
      );
  ...
  async restoreLastDiscardInTempFiles(isSafe, partialDiscardFilePath = null) {
    ...
    const tempFolderPaths = await this.expandBlobsToFilesInTempFolder(
  ...
  async expandBlobsToFilesInTempFolder(snapshots) {
    const tempFolderPath = await getTempDir({
      prefix: 'github-discard-history-',
    });
    const pathPromises = snapshots.map(
      async ({filePath, beforeSha, afterSha}) => {
        ...
        const resultPath = path.join(
          tempFolderPath,
          `~${path.basename(filePath)}-merge-result`,
        );
        return {
          filePath,
          commonBasePath,
  ...
  async mergeFiles(filePaths) {
    const mergeFilePromises = filePaths.map(async (filePathInfo, i) => {
      ...
      if (theirsPath && commonBasePath) {
        mergeResult = await this.mergeFile(
          filePath,
          commonBasePath,
          theirsPath,
          resultPath,
        ...
        // added file
        const fileDoesExist = await fileExists(
          path.join(this.workdirPath, filePath),
        );
        if (!fileDoesExist) {
