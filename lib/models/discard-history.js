...
export default class DiscardHistory {
  ...
  async expandBlobsToFilesInTempFolder(snapshots) {
    ...
    const pathPromises = snapshots.map(async ({filePath, beforeSha, afterSha}) => {
      ...
      return {filePath, basePath, otherPath, resultPath, theirsSha: beforeSha, commonSha: afterSha};
    });
    return await Promise.all(pathPromises);
  }

  async mergeFiles(filePaths) {
    const mergeFilePromises = filePaths.map(async ({filePath, basePath, otherPath, resultPath, theirsSha, commonSha}) => {
      if (otherPath && basePath) {
        const mergeResult = await this.mergeFile(filePath, basePath, otherPath, resultPath);
        return Object.assign({}, mergeResult, {theirsSha, commonSha});
