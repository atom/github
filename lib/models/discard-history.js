...
export default class DiscardHistory {
  ...
  async undoLastDiscardInTempFiles(filePaths, isSafe, partial) {
    ...
    if (partial) {
      ...
      lastDiscardSnapshots = Object.keys(snapshotsByPath).map(filePath => {
        return {filePath, ...snapshotsByPath[filePath]};
      });
    }
