...
export default class DiscardHistory {
  ...
  async storeBeforeAndAfterBlobs(filePaths, isSafe, destructiveAction, partialDiscardFilePath = null, commitSha) {
    if (partialDiscardFilePath) {
      return await this.storeBlobsForPartialFileHistory(partialDiscardFilePath, isSafe, destructiveAction, commitSha);
    } else {
      return await this.storeBlobsForWholeFileHistory(filePaths, isSafe, destructiveAction, commitSha);
    }
  }
  ...
  async storeBlobsForWholeFileHistory(filePaths, isSafe, destructiveAction, commitSha) {
    const snapshotsByPath = {};
    const beforePromises = filePaths.map(async filePath => {
      snapshotsByPath[filePath] = {
        beforeSha: await this.createBlob({filePath}),
        commitSha,
      };
    });
  ...
  async restoreLastDiscardInTempFiles(isSafe, partialDiscardFilePath = null, lastCommitSha) {
    const expandAndMerge = async () => {
    ...
    if (lastDiscardSnapshots[0].commitSha !== lastCommitSha) {
      const choice = atom.confirm({
        message: 'You have committed since discarding.',
        detailedMessage: 'Are you sure you want to undo the last discard?',
        buttons: ['Undo', 'Cancel'],
      });
      if (choice === 0) {
        return expandAndMerge();
      } else {
        return [];
      }
    } else {
