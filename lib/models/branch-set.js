...
export default class BranchSet {
  ...
  add(branch) {
    ...
    if (u.isRemoteTracking()) {
      const k = `${u.getRemoteName()}\0${u.getRemoteRefName().full()}`;
      pushAtKey(this.byUpstreamRef, k, branch);
    }

    const p = branch.getPush();
    if (p.isRemoteTracking()) {
      const k = `${p.getRemoteName()}\0${p.getRemoteRefName().full()}`;
      pushAtKey(this.byPushRef, k, branch);
    }
  }

  getNames() {
    return this.all.map(branch => branch.getName().short());
  }
