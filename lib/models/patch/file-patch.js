...
export default class FilePatch {
  ...
  triggerCollapseIn(patchBuffer) {
    ...
    patchBuffer.getBuffer().deleteRow(position.row);

    const patchMarker = patchBuffer.markPosition(Patch.layerName, position, {invalidate: 'never', exclusive: true});
  ...
  triggerExpandIn(patchBuffer, {before, after}) {
    ...
    const emptyPatch = patchBuffer.getBuffer().isEmpty();

    // Insert a newline *before* the expanding patch if we're inserting at the end, because newlines separate patches
    // and the buffer doesn't end with one. Insert a newline *after* the expanding patch if we're inserting internally,
    // because the insertion point is after the existing newline between the surrounding patches.

    patchBuffer
      .createInserterAt(this.patch.getInsertionPoint())
      .keepBefore(before)
      .keepAfter(after)
      .insert(atEnd && !emptyPatch ? '\n' : '')
