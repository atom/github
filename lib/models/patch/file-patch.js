...
export default class FilePatch {
  ...
  triggerCollapseIn(patchBuffer) {
    ...
    // The collapsed patch's marker must be positioned *before* the separating newline preceding this patch, if there
    // is one. This is for consistency with the placement of HiddenPatch markers created by the builder.
    const oldStart = oldPatch.getRange().start.copy();
    const insertionPosition = oldStart.row === 0
      ? oldStart
      : patchBuffer.getBuffer().clipPosition([oldStart.row - 1, Infinity]);
    ...
    patchBuffer.getBuffer().deleteRow(oldStart.row);
  ...
  triggerExpandIn(patchBuffer, {before, after}) {
    ...
    // The expanding patch's insertion point is just before the unmarked newline that separates adjacent visible
    // patches:
    // <p0> '\n' <p1> * '\n' <p2>
    //
    ...
    // <p0> '\n' <p1> '\n' <p2> *
    //
    // Insert a newline *before* the expanding patch if we're inserting anywhere but the buffer's start. Insert a
    // newline *after* the expanding patch if we are inserting at the buffer's start, but the buffer is non-empty
    // (so it isn't also the end of the buffer).
