...
export default class FilePatch {
  ...
  static createDelayedFilePatch(oldFile, newFile, position, renderStatus, parseFn) {
    return new this(oldFile, newFile, Patch.createDelayedPatch(position, renderStatus, parseFn));
  }
  ...
  triggerDelayedRender() {
    const nextPatch = this.patch.parseFn();
