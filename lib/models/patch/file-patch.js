...
export default class FilePatch {
  ...
  getUnstagePatchForLines(selectedLineSet) {
    if (this.patch.getChangedLineCount() === selectedLineSet.size) {
      // Ensure that newFile is null if the patch is an addition because we're deleting the entire file from the index.
      // If a symlink was deleted and replaced by a non-symlink file, we don't want the symlink entry to muck up the
      // patch.
      return this.clone({
        oldFile: this.getNewFile(),
        newFile: this.getStatus() === 'added' ? nullFile : this.getNewFile(),
        patch: this.patch.getFullUnstagedPatch(),
      });
    } else {
      return this.clone({
        oldFile: this.getNewFile(),
        newFile: this.getNewFile(),
        patch: this.patch.getUnstagePatchForLines(selectedLineSet),
      });
    }
  }
