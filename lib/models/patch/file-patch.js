...
export default class FilePatch {
  ...
  triggerCollapseIn(patchBuffer, {before, after}) {
    ...
    const insertionPosition = oldPatch.getRange().start.copy();
    const exclude = new Set([...before, ...after]);
    const {patchBuffer: subPatchBuffer, markerMap} = patchBuffer.extractPatchBuffer(oldPatch.getRange(), {exclude});
