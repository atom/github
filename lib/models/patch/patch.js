...
export default class Patch {
  ...
  reMarkOn(markable) {
    this.marker = markable.markRange(this.getRange(), {
      invalidate: 'never',
      exclusive: false,
    });
  }
  ...
  buildStagePatchForLines(originalBuffer, nextLayeredBuffer, rowSet) {
    ...
    const marker = layers.patch.markRange([
      [0, 0],
      [buffer.getLastRow() - 1, Infinity],
    ]);
  ...
  latestHunkWasIncluded() {
    ...
    const regions = this.hunkRegions.map(({RegionKind, range}) => {
      return new RegionKind(
        this.layers
          .get(RegionKind)
          .markRange(range, {invalidate: 'never', exclusive: false}),
      );
    });

    const marker = this.layers
      .get('hunk')
      .markRange(this.hunkRange, {invalidate: 'never', exclusive: false});
