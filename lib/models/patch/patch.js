...
export default class Patch {
  ...
  getNextSelectionRange(lastPatch, lastSelectedRows) {
    ...
    for (const hunk of lastPatch.getHunks()) {
      let includesUnselectedChange = false;
      let includesMax = false;
      let hunkSelectionIndex = 0;
      ...
      changeLoop: for (const change of hunk.getChanges()) {
        ...
        for (const {intersection, gap} of intersections) {
          ...
          if (gap) {
            // This hunk includes at least one change row that was *not* selected.
            includesUnselectedChange = true;

            // Range of unselected changes.
            hunkSelectionIndex += delta;
          }
      ...
      if (includesUnselectedChange) {
        lastSelectionIndex += hunkSelectionIndex;
      }
    ...
    return [[newSelectionRow, 0], [newSelectionRow, 0]];
  }
