...
export function buildMultiFilePatch(diffs) {
  ...
  return new MultiFilePatch(layeredBuffer.buffer, filePatches);
}
...
function singleDiffFilePatch(diff, layeredBuffer = null) {
  ...
  const [hunks] = buildHunks(diff, layeredBuffer);
  ...
  const newFile = diff.newPath !== null || diff.newMode !== null
    ...
    : nullFile;
  const patch = new Patch({status: diff.status, hunks, ...layeredBuffer});
...
function dualDiffFilePatch(diff1, diff2, layeredBuffer = null) {
  ...
  const [hunks] = buildHunks(contentChangeDiff, layeredBuffer);
  ...
  const patch = new Patch({status, hunks, ...layeredBuffer});
...
function buildHunks(diff, {buffer, layers}) {
  ...
  let bufferRow = buffer.getLastRow();
