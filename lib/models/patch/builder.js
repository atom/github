  ...
  for (const hunkData of diff.hunks) {
    ...
    const finishCurrentRange = () => {
      ...
      if (LastChangeKind !== null) {
        console.log(`finishing change: currentRangeStart = ${currentRangeStart} bufferRow = ${bufferRow} lastLineLength = ${lastLineLength}`);
        changes.push(
          new LastChangeKind(
            new IndexedRowRange({
              bufferRange: [[currentRangeStart, 0], [bufferRow - 1, lastLineLength - 1]],
              startOffset,
    ...
    hunks.push(new Hunk({
      ...
      rowRange: new IndexedRowRange({
        bufferRange: [[bufferStartRow, 0], [bufferRow - 1, nextLineLength - 1]],
