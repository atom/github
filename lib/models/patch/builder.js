...
export function buildMultiFilePatch(diffs) {
  ...
  const layers = {
    patch: layeredBuffer.layers.patch,
    hunk: layeredBuffer.layers.hunk,
  };
  return new MultiFilePatch(layeredBuffer.buffer, layers, filePatches);
}
...
function singleDiffFilePatch(diff, layeredBuffer = null) {
  ...
  const newFile = diff.newPath !== null || diff.newMode !== null
    ...
    : nullFile;
  const patch = new Patch({status: diff.status, hunks, marker: patchMarker, ...layeredBuffer});
...
function dualDiffFilePatch(diff1, diff2, layeredBuffer = null) {
  ...
  const patch = new Patch({status, hunks, marker: patchMarker, ...layeredBuffer});
