...
export function buildFilePatch(diffs, options) {
  ...
  const workingBuffer = new PatchBuffer();

  let filePatch;
  if (diffs.length === 0) {
    filePatch = emptyDiffFilePatch();
  } else if (diffs.length === 1) {
    filePatch = singleDiffFilePatch(diffs[0], workingBuffer, opts);
  } else if (diffs.length === 2) {
    filePatch = dualDiffFilePatch(diffs[0], diffs[1], workingBuffer, opts);
  } else {
    throw new Error(`Unexpected number of diffs: ${diffs.length}`);
  }

  // Delete the trailing newline.
  workingBuffer.deleteLastNewline();

  let patchBuffer = workingBuffer;
  if (opts.patchBuffer) {
    patchBuffer = opts.patchBuffer;
    const markerMap = opts.patchBuffer.adopt(workingBuffer);
...
export function buildMultiFilePatch(diffs, options) {
  ...
  for (const diff of diffs) {
    ...
    if (diff.status === 'added' || diff.status === 'deleted') {
      ...
      if (otherHalf) {
        ...
        actions[otherIndex] = () => dualDiffFilePatch(diff, otherDiff, workingBuffer, opts);
      ...
      actions[index] = () => singleDiffFilePatch(diff, workingBuffer, opts);
      index++;
    }
  }
  ...
  for (const [unpairedDiff, originalIndex] of byPath.values()) {
    actions[originalIndex] = () => singleDiffFilePatch(unpairedDiff, workingBuffer, opts);
  }
