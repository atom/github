...
function singleDiffFilePatch(diff, layeredBuffer) {
  ...
  if (isDiffLarge([diff])) {
    const delayed = FilePatch.createDelayedFilePatch(
      oldFile, newFile, diff,
      new Point(layeredBuffer.buffer.getLastRow(), 0),
    );
    console.log(delayed.getPatch());
    return delayed;
  }
...
function isDiffLarge(diffs) {
  // TODO: fixme
  const DIFF_BYTE_THRESHOLD = 500;

  const size = diffs.reduce((diffSizeCounter, diff) => {
    return diffSizeCounter + diff.hunks.reduce((hunkSizeCounter, hunk) => {
      return hunkSizeCounter + hunk.lines.reduce((lineSizeCounter, line) => {
        return lineSizeCounter + Buffer.byteLength(line, 'utf8');
      }, 0);
    }, 0);
  }, 0);

  console.log(diffs, `size: ${size}`);
  return size > DIFF_BYTE_THRESHOLD;
}
