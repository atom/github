...
import Change, {nullChange} from './change';
import FilePatch from './file-patch';
import {fromBufferRange, fromBufferPosition} from '../marker-position';
...
function buildHunks(diff) {
  ...
  for (const hunkData of diff.hunks) {
    ...
    const finishCurrentChange = () => {
      ...
      if (changes !== undefined) {
        changes.push(new Change({
          position: fromBufferRange([[currentRangeStart, 0], [bufferRow - 1, 0]]),
          startOffset,
    ...
    hunks.push(new Hunk({
      ...
      bufferStartPosition: fromBufferPosition([bufferStartRow, 0]),
      bufferStartOffset,
