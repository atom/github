...
export function buildMultiFilePatch(diffs) {
  ...
  for (const diff of diffs) {
    ...
    if (diff.status === 'added' || diff.status === 'deleted') {
      ...
      if (otherHalf) {
        ...
        actions[otherIndex] = () =>
          dualDiffFilePatch(diff, otherDiff, layeredBuffer);
  ...
  for (const [unpairedDiff, originalIndex] of byPath.values()) {
    actions[originalIndex] = () =>
      singleDiffFilePatch(unpairedDiff, layeredBuffer);
  }
...
function buildHunks(diff, {buffer, layers}) {
  ...
  if (diff.hunks.length === 0) {
    const patchMarker = layers.patch.markPosition([patchStartRow, 0], {
      invalidate: 'never',
      exclusive: false,
    });
