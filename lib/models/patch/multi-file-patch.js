...
export default class MultiFilePatch {
  ...
  doesPatchExceedLargeDiffThreshold = filePatchPath => {
    ...
    const patches = this.getFilePatches();
    // can avoid iterating through all patches by adding a map
    const patch = patches.filter(p => p.getPath() === filePatchPath)[0];
    // do I need to handle case where patch is null or something?
    if (patch.getRenderStatus() === TOO_LARGE) {
      return true;
    } else {
      return false;
    }
