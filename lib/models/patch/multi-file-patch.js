...
export default class MultiFilePatch {
  ...
  getDiffPositionForBufferRow(bufferRow) {
    if (bufferRow < 0 || bufferRow > this.getBuffer().getLastLine()) {
      // eslint-disable-next-line no-console
      console.error('Attempt to compute diff position for invalid buffer row: buffer row out of range', {
        bufferRow,
      });
    ...
    const filePatchStartRow = filePatch.getStartRange().start.row;
    const hunkIndex = filePatch.getHunks().indexOf(hunk) + 1;

    return {path: filePatch.getPath(), position: bufferRow - filePatchStartRow + hunkIndex};
  }
