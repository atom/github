...
export default class MultiFilePatch {
  ...
  getFilePatchAt(bufferRow) {
    ...
    const markers = this.patchLayer.findMarkers({intersectsRow: bufferRow});
    if (markers.length !== 1) {
      throw new Error(`Expected 1 marker to be found at buffer row ${bufferRow}, but found ${markers.length}`);
    }
    return this.filePatchesByMarker.get(markers[0]);
  }
  ...
  getHunkAt(bufferRow) {
    ...
    const markers = this.hunkLayer.findMarkers({intersectsRow: bufferRow});
    if (markers.length !== 1) {
      throw new Error(`Expected 1 marker to be found at buffer row ${bufferRow}, but found ${markers.length}`);
    }
    return this.hunksByMarker.get(markers[0]);
  }
