...
import Repository, {ABSENTLIKE, LOADINGLIKE} from './repository';
...
export {ABSENTLIKE, LOADINGLIKE};

const initRepoState = Symbol('initRepoState');
const initRepoArg = Symbol('initRepoArgs');
...
export default class WorkdirContext {
  ...
  constructor(directory, options = {}) {
    ...
    this.repository = (token => {
      ...
      if (token === UNDETERMINED) { return Repository.undetermined(options[initRepoArg]); }
  ...
  static undetermined(behavior) {
    return new WorkdirContext(null, {[initRepoState]: UNDETERMINED, [initRepoArg]: behavior});
  }
