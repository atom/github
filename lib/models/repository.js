...
export default class Repository {
  ...
  constructor(workingDirectoryPath, gitStrategy) {
    ...
    this.resolutionProgressByPath = new Map();
  ...
  setConflictMarkerCount(filePath, count) {
    if (!this.resolutionProgressByPath.has(filePath)) {
      this.resolutionProgressByPath.set(filePath, {value: 0, max: count});
    }
  }

  getResolutionProgressMax(filePath) {
    const progress = this.resolutionProgressByPath.get(filePath);
    return progress === undefined ? 1 : progress.max;
  }

  getResolutionProgressValue(filePath) {
    const progress = this.resolutionProgressByPath.get(filePath);
    return progress === undefined ? 0 : progress.value;
  }

  reportConflictResolution(filePath) {
    const progress = this.resolutionProgressByPath.get(filePath);
    if (!progress) {
      throw new Error(`Reporting conflict resolution on unrecognized file: ${filePath}`);
    }
