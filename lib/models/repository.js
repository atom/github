...
export default class Repository {
  static async open(workingDirectoryPath, gitStrategy) {
    ...
    if (await strategy.isGitRepository()) {
      ...
      const history = historySha ? JSON.parse(await strategy.getBlobContents(historySha)) : null;
  ...
  constructor(workingDirectoryPath, gitStrategy, history) {
    ...
    this.fileDiscardHistory = new FileDiscardHistory(this.createBlob, this.restoreBlob, this.setConfig);
    if (history) { this.fileDiscardHistory.updateHistory(history); }
    window.addEventListener('focus', () => this.updateDiscardHistory());
  }
  ...
  @autobind
  restoreBlob(filePath, sha) {
    return this.git.restoreBlob(filePath, sha);
  }

  storeBeforeAndAfterBlobs(filePath, isSafe, destructiveAction) {
    return this.fileDiscardHistory.storeBlobs(filePath, isSafe, destructiveAction);
  }

  attemptToRestoreBlob(filePath, isSafe) {
    return this.fileDiscardHistory.attemptToRestoreBlob(filePath, isSafe);
  }

  hasUndoHistory(filePath) {
    return this.fileDiscardHistory.hasUndoHistory(filePath);
  }

  async updateDiscardHistory() {
    const historySha = await this.git.getConfig('atomGithub.historySha');
    const history = historySha ? JSON.parse(await this.git.getBlobContents(historySha)) : null;
    this.fileDiscardHistory.updateHistory(history);
  }

  clearDiscardHistoryForPath(filePath) {
    return this.fileDiscardHistory.clearHistoryForPath(filePath);
  }

  @autobind
  getDiscardHistoryForPath(filePath) {
    return this.fileDiscardHistory.getHistoryForPath(filePath);
  }
}
