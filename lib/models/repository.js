...
export default class Repository {
  ...
  async push (branchName) {
    ...
    const remote = await this.rawRepository.getRemote(remoteName)
    const refspecs = ["refs/heads/master:refs/heads/master"] // await this.getPushRefspecs(branch)
    return remote.push(refspecs)
  }

  async getBranchRemote (name) {
    const config = await this.rawRepository.configSnapshot()
    return config.getStringBuf(`branch.${name}.remote`)
  }
  ...
  async getAheadBehindCount (branchName) {
    const branchRef = await this.rawRepository.getReference(branchName)
    const upstreamRef = await Git.Branch.upstream(branchRef)
    return Git.Graph.aheadBehind(this.rawRepository, branchRef.target(), upstreamRef.target())
  }
}
