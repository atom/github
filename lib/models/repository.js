...
export default class Repository {
    ...
    return this.autoTransact(async () => {
      ...
      entriesWithConflictsByPath.forEach((entries, path) => {
        ...
        if (existingConflict == null) {
          this.mergeConflictsByPath.set(path, new MergeConflict(path, entries.get(1), entries.get(2), entries.get(3)))
        } else {
          existingConflict.updateFileStatus() // for updating status once getDiffStatusForPaths is implemented correctly
        }
  ...
  async pathHasMergeMarkers (relativePath) {
    ...
    return contents.includes('>>>>>>>') || contents.includes('<<<<<<<') || contents.includes('|||||||') || contents.includes('=======')
  }

  async addPathToIndex (relativePath) {
    await this.autoTransact(async () => {
      const index = await this.rawRepository.refreshIndex()
      try {
        await index.addByPath(relativePath)
      } catch (e) {
        if (e.message.endsWith('No such file or directory')) {
          await index.removeByPath(relativePath)
        } else {
          throw e
        }
      }
