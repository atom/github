...
import {readFile, deleteFileOrFolder} from '../helpers';
...
async function loadHistoryPayload(git) {
  const historySha = await git.getConfig('atomGithub.historySha');
  if (!historySha) {
    return {};
  }
  ...
  let blob;
  try {
    blob = await git.getBlobContents(historySha);
  } catch (e) {
...
export default class Repository {
  static async open(workingDirectoryPath, gitStrategy) {
    ...
    if (await strategy.isGitRepository()) {
      const history = await loadHistoryPayload(strategy);
  ...
  static async init(workingDirectoryPath, gitStrategy) {
    const strategy = gitStrategy || CompositeGitStrategy.create(workingDirectoryPath);
    await strategy.init(workingDirectoryPath);
    return this.open(workingDirectoryPath, strategy);
  }

  static async clone(remoteUrl, destDirectory, gitStrategy) {
    await new Promise((resolve, reject) => {
      fs.mkdirs(destDirectory, err => (err ? reject(err) : resolve()));
    });

    const strategy = gitStrategy || CompositeGitStrategy.create(destDirectory);
    await strategy.clone(remoteUrl, {recursive: true});
    return this.open(destDirectory, strategy);
  }
  ...
  async createDiscardHistoryBlob() {
    return await this.discardHistory.createHistoryBlob();
  }
  ...
  async updateDiscardHistory() {
    const history = await loadHistoryPayload(this.git);
  ...
  async restoreLastDiscardInTempFiles(isSafe, partialDiscardFilePath = null) {
    return await this.discardHistory.restoreLastDiscardInTempFiles(isSafe, partialDiscardFilePath);
  }
  ...
  async clearDiscardHistory(partialDiscardFilePath = null) {
