...
export default class Repository {
  ...
  async fetchStagedChangesSinceParentCommit () {
    let rawDiffs
    try {
      rawDiffs = await this.git.diff({staged: true, baseCommit: 'HEAD~'})
    } catch (e) {
    ...
    for (let newPatch of this.buildFilePatchesFromRawDiffs(rawDiffs)) {
      const path = newPatch.getPath()
      const existingPatch = this.stagedFilePatchesSinceParentCommitByPath.get(path)
      if (existingPatch == null) {
        this.stagedFilePatchesSinceParentCommitByPath.set(path, newPatch)
      } else {
        existingPatch.update(newPatch)
      }
      validFilePatches.add(path)
    ...
    for (let [path, patch] of this.stagedFilePatchesSinceParentCommitByPath) {
      if (!validFilePatches.has(path)) {
        this.stagedFilePatchesSinceParentCommitByPath.delete(path)
        patch.destroy()
      }
    }
    return Array.from(this.stagedFilePatchesSinceParentCommitByPath.values())
      .sort((a, b) => a.getPath().localeCompare(b.getPath()))
  }
