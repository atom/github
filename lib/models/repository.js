...
export default class Repository {
  ...
  getStagedChangesSincePreviousCommit () {
    return this.fetchStagedChanges('HEAD~')
  }
  ...
  async fetchStagedChanges (baseCommit) {
    const rawDiffs = await this.git.diff({staged: true, baseCommit})
    const validFilePatches = new Set()
    for (let newPatch of this.buildFilePatchesFromRawDiffs(rawDiffs)) {
      const path = newPatch.getPath()
