...
export default class Repository {
  ...
  async getMergeMessage () {
    const file = new File(this.rawRepository.path() + '/MERGE_MSG')
    return (await file.exists()) ? file.read(true) : ''
  }
  ...
  async abortMerge () {
    ...
    for (let path of stagedPaths) {
      if (unstagedPathSet.has(path)) {
        throw new Error(`Cannot abort because ${path} is both dirty and staged`)
      }
    }
    ...
    checkoutOptions.checkoutStrategy =
      Git.Checkout.STRATEGY.FORCE | Git.Checkout.STRATEGY.DONT_UPDATE_INDEX
    checkoutOptions.paths = mergeConflictPaths.concat(stagedPaths)
    return Git.Checkout.head(this.rawRepository, checkoutOptions)
  }
