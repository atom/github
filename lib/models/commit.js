...
export default class Commit {
  ...
  abbreviatedBody() {
    ...
    const boundarySearch = this.getMessageBody()
      .substring(LONG_MESSAGE_THRESHOLD - BOUNDARY_SEARCH_THRESHOLD, LONG_MESSAGE_THRESHOLD);

    const boundaryRx = /\r?\n\r?\n|\s+/g;
    let result;
    while ((result = boundaryRx.exec(boundarySearch)) !== null) {
      if (/\r?\n\r?\n/.test(result[0])) {
        // Paragraph boundary. Omit the elipsis
        lastParagraphIndex = result.index;
        elipsis = '';
      } else if (result.index <= BOUNDARY_SEARCH_THRESHOLD - elipsis.length) {
        // Word boundary. Only count if we have room for the elipsis under the cutoff.
        lastWordIndex = result.index;
      }
    }

    const cutoffIndex = Math.min(lastParagraphIndex, lastWordIndex);
    return this.getMessageBody().substring(0, cutoffIndex) + elipsis;
  }
