...
class ConflictVisitor {
  ...
  markSide(position, source, bannerRow, textRowStart, textRowEnd) {
    ...
    return new Side(this.editor, sideMarker, source, position, banner, originalText);
  }
...
export default class Conflict {
  ...
  ensureResolved(methodName) {
    if (this.resolution === null) {
      throw new Error(`Conflict must be resolved before ${methodName} is called`);
    }
  }

  getChosenSide() {
    this.ensureResolved('getChosenSide');
    return this.resolution;
  }

  getUnchosenSides() {
    this.ensureResolved('getUnchosenSides');
