...
class ConflictVisitor {
  ...
  visitSeparator(sepRowStart, sepRowEnd) {
    ...
    this.separator = new Separator(marker);
  }
  ...
  markSide(position, source, bannerRow, textRowStart, textRowEnd) {
    ...
    const bannerMarker = this.editor.markBufferRange([[bannerRow, 0], [bannerRow + 1, 0]], MARKER_OPTIONS);
    const banner = new Banner(bannerMarker, description);
    ...
    return new Side(sideMarker, position, banner, originalText);
  }
 ...
 * row [Integer] - buffer row to begin scanning for conflict markers.
 ...
 */
const parseConflict = function(editor, row, isRebase, visitor) {
  let currentRow = row;
  ...
  const visitBaseSide = () => {
    ...
    while (b === '<') {
      ...
      currentRow = parseConflict(editor, currentRow, new NoopVisitor());
      b = advanceToBoundary('<=');
    }
...
export default class Conflict {
  ...
  static all(editor) {
    ...
    editor.getBuffer().scan(CONFLICT_START_REGEX, m => {
      ...
      try {
        lastRow = parseConflict(editor, conflictStartRow, visitor);
