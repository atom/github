...
class ConflictVisitor {
  ...
  visitSeparator(sepRowStart, sepRowEnd) {
    const marker = this.layer.markBufferRange([[sepRowStart, 0], [sepRowEnd, 0]], MARKER_OPTIONS);
  ...
  markSide(position, source, bannerRow, textRowStart, textRowEnd) {
    ...
    const blockMarker = this.layer.markBufferRange(blockRange, MARKER_OPTIONS);

    const description = this.sideDescription(bannerRow);
    const bannerRange = [[bannerRow, 0], [bannerRow + 1, 0]];
    const bannerMarker = this.layer.markBufferRange(bannerRange, MARKER_OPTIONS);
    ...
    const sideMarker = this.layer.markBufferRange(textRange, MARKER_OPTIONS);
...
export default class Conflict {
  ...
  markerAfter(position) {
    return position.when({
      top: () => (this.bySource.base ? this.bySource.base.getBannerMarker() : this.getSeparator().getMarker()),
      middle: () => this.getSeparator().getMarker(),
      bottom: () => this.bySource.theirs.getBannerMarker(),
    });
  }
