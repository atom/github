export class PartialFileDiscardHistory {
  ...
  addHistory(filePath, snapshots) {
    ...
    if (history) {
      ...
      if (history.length >= this.maxHistoryLength) { this.setHistoryForPath(filePath, history.slice(Math.ceil(this.maxHistoryLength / 2))); }
    } else {
...
export class WholeFileDiscardHistory {
  ...
  addHistory(snapshotsByPath) {
    ...
    if (history) {
      ...
      if (history.length >= this.maxHistoryLength) { this.setHistory(history.slice(Math.ceil(this.maxHistoryLength / 2))); }
    } else {
