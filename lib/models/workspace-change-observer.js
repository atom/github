...
export default class WorkspaceChangeObserver {
  ...
  async watchActiveRepositoryGitDirectory() {
    ...
    const gitDirectoryPath = await repository.getGitDirectoryPath();
    if (repository) {
      this.currentFileWatcher = await nsfw(
        gitDirectoryPath,
        events => {
          const filteredEvents = events.filter(e => ['config', 'index', 'HEAD', 'MERGE_HEAD'].includes(e.file));
    ...
    if (!this.observedBuffers.has(buffer)) {
      ...
      const disposables = new CompositeDisposable(
        buffer.onDidSave(async () => {
          if (await this.activeRepositoryContainsPath(buffer.getPath())) {
