...
export default class WorkspaceChangeObserver {
  ...
  stopCurrentFileWatcher() {
    if (this.currentFileWatcher) {
      const nw = this.currentFileWatcher.nativeWatcher;
      if (nw) {
        nw.onWillStop(() => {
          // eslint-disable-next-line no-console
          console.log(`native watcher ${nw.normalizedPath} about to stop`);
        });
        nw.onDidStop(() => {
          // eslint-disable-next-line no-console
          console.log(`native watcher ${nw.normalizedPath} stopped`);
        });
      } else {
        // eslint-disable-next-line no-console
        console.log('no native watcher');
      }
