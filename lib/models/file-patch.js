...
class File {
  ...
  clone(opts = {}) {
    return new File({
      ...
      symlink: opts.symlink || this.symlink,
    });
  }
}
...
export default class FilePatch {
  ...
  clone(opts = {}) {
    const oldFile = opts.oldFile || this.getOldFile();
    const newFile = opts.newFile || this.getNewFile();
    const patch = opts.patch || this.patch;
