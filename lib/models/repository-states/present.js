...
export default class Present extends State {
  ...
  async isCommitPushed(sha) {
    ...
    const contained = await this.git().getBranchesWithCommit(sha, {
      showLocal: false,
      showRemote: true,
      pattern: upstream.getRefName().short(),
    });
  ...
  getLocalBranches() {
    return this.cache.getOrSet(Keys.localBranches, async () => {
      const payloads = await this.git().getLocalBranches();
      const branches = new BranchSet();
      for (const payload of payloads) {
        branches.add(new Branch(payload.name, payload));
      }
      return branches;
    });
  }
...
const Keys = {
  ...
  localBranches: new CacheKey('local-branches'),

  remoteBranches: new CacheKey('remote-branches'),
