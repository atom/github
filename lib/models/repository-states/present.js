...
export default class Present extends State {
  ...
  getFilePatchForPath(filePath, options) {
    ...
    return this.cache.getOrSet(Keys.filePatch.oneWith(filePath, {staged: opts.staged}), async () => {
      const {diffs, digest} = await this.git().getDiffsForFilePath(filePath, {staged: opts.staged});
      opts.before();
      const patch = this.cache.getOrSet(Keys.filePatch.byDigest(digest), () => buildFilePatch(diffs, opts.builder));
      opts.after(patch);
      return patch;
    });
  }
  ...
  getStagedChangesPatch(options) {
    ...
    return this.cache.getOrSet(Keys.stagedChanges, () => {
      return this.git().getStagedChangesPatch().then(diffs => buildMultiFilePatch(diffs, opts.builder));
    });
  }
...
const Keys = {
  ...
  filePatch: {
    ...
    byDigest: digest => new CacheKey(`file-patch-content:${digest}`),
