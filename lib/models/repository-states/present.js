...
export default class Present extends State {
  ...
  getCommitter() {
    // todo (tt, 4/2018): figure out the best way to just watch `user.name` and `user.email`. ASK ASH!! ask about local: false
    return this.cache.getOrSet(Keys.config.oneWith('user.email', {local: false}), () => {
      return this.git().getCommitter();
    });
  }
