...
export default class Present extends State {
  ...
  async storeBeforeAndAfterBlobs(filePaths, isSafe, destructiveAction, partialDiscardFilePath = null) {
    const commitSha = (await this.getLastCommit()).getSha();
  ...
  async restoreLastDiscardInTempFiles(isSafe, partialDiscardFilePath = null) {
    const lastCommitSha = (await this.getLastCommit()).getSha();
    return this.discardHistory.restoreLastDiscardInTempFiles(isSafe, partialDiscardFilePath, lastCommitSha);
  }
