...
export default class Present extends State {
  ...
  getRefSha(ref) {
    return this.cache.getOrSet(Keys.refs.oneWith(ref), () => {
      return this.git().getRefSha(ref);
    });
  }
  ...
  getBranches() {
    return this.cache.getOrSet(Keys.branches, async () => {
      ...
      for (const payload of payloads) {
        ...
        branches.push(new Branch(payload.name, upstream, push, payload.head, {sha: payload.sha}));
      }
      return branches;
    });
  }
...
const Keys = {
  ...
  refs: {
    oneWith: refName => new CacheKey(`ref:${refName}`, ['refs']),

    all: new GroupKey('refs'),
  },
