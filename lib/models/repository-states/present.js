...
export default class Present extends State {
  ...
  getFilePatchForPath(filePath, {staged, amending} = {staged: false, amending: false}) {
    return this.cache.getOrSet(Keys.filePatch.oneWith(filePath, {staged, amending}), async () => {
      ...
      const rawDiffs = await this.git().getDiffsForFilePath(filePath, options);
...
function buildFilePatchFromRawDiffs(rawDiffs) {
  if (rawDiffs.length === 1) {
    const diff = rawDiffs[0];
    const hunks = buildHunksFromDiff(diff);
    const oldFile = new FilePatch.File({path: diff.oldPath, mode: diff.oldMode, symlink: null});
    const newFile = new FilePatch.File({path: diff.newPath, mode: diff.newMode, symlink: null});
    ...
    if (rawDiffs[0].oldMode === '120000' || rawDiffs[0].newMode === '120000') {
      modeChangeDiff = rawDiffs[0];
      contentChangeDiff = rawDiffs[1];
    } else {
      modeChangeDiff = rawDiffs[1];
      contentChangeDiff = rawDiffs[0];
    }
    ...
    const patch = new FilePatch.Patch({status: 'typechange', hunks});
    return new FilePatch(oldFile, newFile, patch);
  } else {
