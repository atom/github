...
export default class Present extends State {
  ...
  getDiffsForFilePath(filePath, baseCommit) {
    console.log('CALLING getDiffsForFilePath', filePath, {baseCommit});
    return this.cache.getOrSet(Keys.filePatch.oneWith(filePath, {baseCommit}), () => {
      console.log('FETCHING NEW DIFF');
      return this.git().getDiffsForFilePath(filePath, {baseCommit});
    });
  }

  invalidateCachedDiffForFile(filePath, baseCommit) {
    return this.invalidate(
      () => {
        console.log('invalidate', filePath, {baseCommit});
        return Keys.filePatch.oneWith(filePath, {baseCommit});
      },
      () => Promise.resolve(),
    );
  }
