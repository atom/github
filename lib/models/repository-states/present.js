...
export default class Present extends State {
  ...
  @invalidate(paths => [
    ...
    ...Keys.allFilePatchOps(paths, [{staged: true}, {staged: true, amending: true}]),
  ])
  ...
  @invalidate(option => Keys.allConfigOpts(option))
...
const Keys = {
  ...
  allFilePatchOps: (fileNames, ops = [{staged: false}, {staged: true}, {staged: true, amending: true}]) => {
    const keys = [];
    for (let i = 0; i < fileNames.length; i++) {
      for (let j = 0; j < ops.length; j++) {
        keys.push(Keys.filePatch(fileNames[i], ops[j]));
      }
    }
    return keys;
  },
  filePatchOps: (...ops) => ops.map(op => '*' + Keys.filePatch('', op)),
  ...
  allConfigOpts: option => [Keys.config(option, {local: true}), Keys.config(option, {local: false})],
  allConfigs: '*config:',
  ...
  workdirOperationKeys: fileNames => [
    Keys.changedFiles,
    ...Keys.allFilePatchOps(fileNames, [{staged: false}]),
  ],

  cacheOperationKeys: fileNames => [
    ...Keys.workdirOperationKeys(fileNames),
    ...Keys.allFilePatchOps(fileNames, [{staged: true}, {staged: true, amending: true}]),
    Keys.stagedChangesSinceParentCommit,
  ...
  headOperationKeys: () => [
    Keys.changedFiles,
    Keys.stagedChangesSinceParentCommit,
    ...Keys.filePatchOps({staged: true, amending: true}),
    Keys.lastCommit,
    Keys.allAheadCounts,
    Keys.allBehindCounts,
  ],
