...
export default class Present extends State {
  ...
  invalidateCacheAfterFilesystemChange(events) {
    ...
    for (let i = 0; i < paths.length; i++) {
      ...
      if (filePathEndsWith(fullPath, '.git', 'index')) {
        ...
        // todo: stagedChangesSinceParentCommit appears to be dead code and can be removed
  ...
  applyPatchToIndex(multiFilePatch) {
    const filePathSet = multiFilePatch.getFilePatches().reduce((pathSet, filePatch) => {
      pathSet.add(filePatch.getOldPath());
      pathSet.add(filePatch.getNewPath());
      return pathSet;
    }, new Set());
    return this.invalidate(
      () => Keys.cacheOperationKeys(Array.from(filePathSet)),
      () => {
