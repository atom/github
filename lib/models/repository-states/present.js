...
export default class Present extends State {
  ...
  getFilePatchForPath(filePath, {staged} = {staged: false}) {
    return this.cache.getOrSet(
      Keys.filePatch.oneWith(filePath, {staged}),
      async () => {
  ...
  async isCommitPushed(sha) {
    const remoteBranchesWithCommit = await this.git().getBranchesWithCommit(
      sha,
      {showLocal: false, showRemote: true},
    );
...
const Keys = {
  ...
  filePatch: {
    ...
    eachWithOpts: (...opts) =>
      opts.map(
        opt => new GroupKey(`file-patch:${Keys.filePatch._optKey(opt)}`),
      ),
