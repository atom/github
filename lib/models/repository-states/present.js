...
export default class Present extends State {
  ...
  async fetchInitialMessage() {
    ...
    const template = await this.getCommitMessageFromTemplate();
    if (template) {
      this.commitMessageTemplate = template;
    }
    if (mergeMessage) {
  ...
  getCommitMessageFromTemplate() {
    return this.git().getCommitMessageFromTemplate();
  }
  ...
  async updateCommitMessageAfterFileSystemChange(events) {
    for (let i = 0; i < events.length; i++) {
      ...
      if (endsWith('.git', 'config')) {
        ...
        // question -- can filewatcher watch folders that have symlinks to files that change?
        // should we create a folder in .atom that symlinks to global config file and commit message template file? to watch for changes and update correctly?
        const template = await this.getCommitMessageFromTemplate();
