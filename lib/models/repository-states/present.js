...
export default class Present extends State {
  constructor(repository, history) {
    ...
    // RepositoryOperations/InProgress with nice API
    this.operationStates = {
      pushInProgress: false,
      pullInProgress: false,
      fetchInProgress: false,
      commitInProgress: false,
      checkoutInProgress: false, // value is branchName when in progress. better way?
    };
  ...
  setOperationProgressState(type, value) {
    const oldValue = this.operationStates[type + 'InProgress'];
    this.operationStates[type + 'InProgress'] = value;
    if (oldValue !== value) {
      this.didUpdate();
    }
  }
  ...
  commit(rawMessage, options) {
    return this.executePipelineAction('COMMIT', async (rawMessage, options) => {
  ...
  checkout(revision, options = {}) {
    return this.executePipelineAction('CHECKOUT', async (revision, options) => {
