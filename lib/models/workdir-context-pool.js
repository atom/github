...
export default class WorkdirContextPool {
  ...
  add(directory, options = {}) {
    ...
    const forwardPromise = (subMethod, emitEventName) => {
      context[subMethod]().then(() => this.emitter.emit(emitEventName, context));
    };
    forwardPromise('getRepositoryPromise', 'did-load-repository');
    forwardPromise('getResolutionProgressPromise', 'did-load-resolution-progress');
    forwardPromise('getChangeObserverPromise', 'did-load-change-observer');
  ...
  onDidLoadRepository(callback) {
    return this.emitter.on('did-load-repository', callback);
  }

  onDidLoadResolutionProgress(callback) {
    return this.emitter.on('did-load-resolution-progress', callback);
  }

  onDidLoadChangeObserver(callback) {
    return this.emitter.on('did-load-change-observer', callback);
  }
