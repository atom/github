...
export default class FileListComponent {
  ...
  acceptProps ({fileListViewModel}: FileListComponentProps): Promise<void> {
    ...
    this.subscriptions.add(this.fileListViewModel.onDidUpdate(() => etch.update(this)))
    this.subscriptions.add(this.fileListViewModel.onSelectionChanged(() => this.selectionDidChange()))
