...
export default class GitService {
  ...
  getCurrentBranchName (): Promise<string> {
    const repo = GitRepositoryAsync.open(this.repoPath)
  ...
  _calculatePatchText (hunk: DiffHunk, selectedLines: Array<HunkLine>, offset: number, stage: boolean): {patchText: string, offset: number} {
    ...
    const hunkLineCount = hunk.getLines().length
    const selectedLineCount = selectedLines.length
    let needsNoNewlineToken = false
    // If we're staging all lines but one, and that one line is telling us we're
    // removing the trailing newline, then we also need to tell git that.
    if (hunkLineCount === selectedLineCount + 1) {
      const hunkLineSet = new Set(hunk.getLines())
      const selectedLineSet = new Set(selectedLines)
      for (let line of selectedLineSet) {
        hunkLineSet.delete(line)
      }

      if (hunkLineSet.size === 1) {
        const line = Array.from(hunkLineSet)[0]
        if (line.isRemovedNewlineAtEOF()) {
          needsNoNewlineToken = true
        }
      }
    }
    ...
    return Git.Repository
      ...
      .then(content => {
        ...
        if (fileDiff.isDeleted()) {
          // $FlowSilence: If it's deleted then we know it has an old path name.
        ...
        // $FlowFixMe: Unclear whether this is right.
        if (oldPath !== newPath) data.index.removeByPath(oldPath)

        // In both branches above, `entry` is non-null by the time we're here.
        // $FlowSilence
