...
export default class GitService {
  repoPathPromise: Promise<string>;
  ...
  constructor (gitRepo: GitRepositoryAsync) {
    ...
    this.repoPathPromise = gitRepo.getPath()
    ...
    this.repoPathPromise.then(gitDirPath => {
  ...
  gatherDiffs (): Promise<Diffs> {
    ...
    this.diffsPromise = this.repoPathPromise
      .then(repoPath => Git.Repository.open(repoPath))
      .then(repo => {
        data.repo = repo
  ...
  async pull (branchName: string, creds: {username: string, password: string}, progress: (progress: number) => void): Promise<void> {
    ...
    const repoPath = await this.repoPathPromise
