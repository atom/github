...
export default class GitService {
  ...
  createNetworkOptions (remote: Git.Remote, {username, password}: {username: string, password: string}, progress: (progress: number) => void): Object {
    let authAttempts = 0
    const credentialsCallback = (url, username) => {
      console.log('auth for', url)
      if (authAttempts > 2) {
        remote.stop()
        remote.disconnect()
    ...
    return this.getBranchRemote(branch)
      .then(remote => {
        ...
        return this.fetch(remote, creds, progress)
          .then(() => {
            return this.getBranchMerge(branch)
              .then(mergeBranch_ => {
                if (!mergeBranch_) {
                  const error = new Error()
                  error.name = GitService.NoRemoteErrorName()
                ...
                const mergeBranch = mergeBranch_
                return Git.Repository
                  .open(this.repoPath)
                  .then(repo => {
                    return repo.mergeBranches(branch, mergeBranch)
                  })
                  .then(_ => { return })
              })
          })
  ...
  push (branch: string, creds: {username: string, password: string}, progress: (progress: number) => void): Promise<void> {
    return this.getBranchRemote(branch)
      .then(remoteName_ => {
        if (!remoteName_) {
          const error = new Error()
        ...
        const remoteName = remoteName_
        return Git.Repository
          .open(this.repoPath)
          .then(repo => repo.getRemote(remoteName))
      })
      .then(remote_ => {
        if (!remote_) {
          const error = new Error()
        ...
        const remote = remote_
        return remote.getPushRefspecs()
          .then(refspecs => remote.push(refspecs, this.createNetworkOptions(remote, creds, progress)))
      })
  }
  ...
  getBranchMerge (name: string): Promise<?string> {
    return Git.Repository
      .open(this.repoPath)
      .then(repo => repo.configSnapshot())
      .then(config => config.getStringBuf(`branch.${name}.merge`))
  }
}
