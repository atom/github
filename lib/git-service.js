...
export default class GitService {
  ...
  fetch (remote: string, {username, password}: {username: string, password: string}, progress: (progress: number) => void): Promise<void> {
    const credentialsCallback = () => Git.Cred.userpassPlaintextNew(username, password)
    const options = {
      callbacks: {
    ...
    return Git.Repository
      .open(this.repoPath)
      .then(repo => repo.fetch(remote, options))
  }
}
