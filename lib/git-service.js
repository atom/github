  ...
  stageFile (file: FileDiff): Promise<number> {
    return Git.Repository
      ...
      .then(index => {
        if (file.isDeleted()) {
          // $FlowSilence
          index.removeByPath(file.getOldFileName())
        } else if (file.isRenamed()) {
          // $FlowSilence
          index.removeByPath(file.getOldFileName())
          // $FlowSilence
          index.addByPath(file.getNewFileName())
        } else {
          // $FlowSilence
          index.addByPath(file.getNewFileName())
        }
      ...
      .then(repo => {
        if (repo.isEmpty()) {
          return repo
            ...
            .then(index => {
              // $FlowSilence
              index.removeByPath(file.getNewFileName())
              return index.write()
            })
        } else {
          return repo
            ...
            .then(commit => {
              ...
              if (file.isRenamed()) {
                // $FlowSilence
                promises.push(Git.Reset.default(repo, commit, file.getOldFileName()))
                // $FlowSilence
                promises.push(Git.Reset.default(repo, commit, file.getNewFileName()))
              } else {
                // $FlowSilence
                promises.push(Git.Reset.default(repo, commit, file.getNewFileName()))
              }
