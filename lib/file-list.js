...
export default class FileList {
  ...
  async handleDidChange (event) {
    ...
    for (let hunkString in stagableLinesPerHunk) {
      ...
      if (linesToStage.length) {
        stagePromises.push(this.stageLines(hunk, linesToStage, true))
      } else if (linesToUnstage.length) {
        stagePromises.push(this.stageLines(hunk, linesToUnstage, false))
      }
    }
  ...
  async stageLines (hunk, lines, stage) {
    console.log(stage ? 'stageLines' : 'unstageLines', lines)

    const fileDiff = hunk.diff
    const patchText = await this.gitService.calculatePatchText(hunk, lines, stage)
    if (stage) {
