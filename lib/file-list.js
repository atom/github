...
export default class FileList {
  ...
  async handleDidChange (event) {
    ...
    for (let hunk in stagableLinesPerHunk) {
      ...
      if (linesToStage.length) {
        stagePromises.push(this.stageLines(h, linesToStage, true))
      } else if (linesToUnstage.length) {
        stagePromises.push(this.stageLines(h, linesToUnstage, false))
      }
    }
  ...
  async stageLines (hunk, lines, isStaged) {
    console.log(isStaged ? 'stageLines' : 'unstageLines', lines)

    let gitUtilsLines = lines.map(line => line.line)
    const patchText = await this.gitService.calculatePatchText(hunk, gitUtilsLines, isStaged)
    return this.gitService.stagePatch(patchText)
  }
