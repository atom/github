...
export default class FileList {
  ...
  async handleDidChange (event) {
    ...
    const {stagableLinesByHunk, fileDiff} = this.extractLinesFromEventByHunk(event)
    await this.stageLines(fileDiff, stagableLinesByHunk)
    this.didUserChange()
  }

  async stageLines (fileDiff, stagableLinesPerHunk) {
    const patches = await this.gitService.calculatePatchTexts(stagableLinesPerHunk, true)
    await this.stagePatches(fileDiff, patches, true)
  }
  ...
  extractLinesFromEventByHunk (event) {
    ...
    return {stagableLinesByHunk, fileDiff}
  }
