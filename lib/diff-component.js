    ...
    return (
      <div className="git-diff-container" tabIndex="-1" style={style}>{
        this.diffViewModel.getFileDiffs().map((fileDiff, index) =>
          <FileDiffComponent fileDiff={fileDiff}, index={index} diffViewModel={diffViewModel}/>
        )
      }</div>
    )
  }
    ...
    return (
      <div className="git-file-diff">
        ...
        {this.fileDiff.getHunks().map((diffHunk, index) =>
          <DiffHunkComponent diffHunk={diffHunk} fileDiffIndex={index} diffViewModel={diffViewModel}/>
        )}
      </div>
    )
  }
