...
export default class ReviewsContainer extends React.Component {
  static propTypes = {
    ...
    uncreatedThreadRows: PropTypes.arrayOf(PropTypes.number.isRequired).isRequired,
  ...
  renderWithResult({aggregationResult, queryProps, repoData, patch}) {
    const commentPositionSet = new Set();
    for (const {comments} of aggregationResult.commentThreads) {
      commentPositionSet.add(`${comments[0].position}-${comments[0].path}`);
    }

    // This mirrors the GraphQL fields requested in ReviewThreadsAccumulator and ReviewCommentsAccumulator, with the
    // addition of the `isUncreated` field in the root comment.
    const uncreatedCommentThreads = [];
    for (const bufferRow of this.props.uncreatedThreadRows) {
      const {path, position} = patch.getDiffPositionForBufferRow(bufferRow);

      if (commentPositionSet.has(`${position}-${path}`)) {
        continue;
      }

      uncreatedCommentThreads.push({
        thread: {
          id: `uncreated-thread-${bufferRow}`,
          isResolved: false,
          resolvedBy: null,
        ...
        comments: [{
          isUncreated: true,
          createdAt: null,
          id: `uncreated-comment-${bufferRow}`,
          author: queryProps.viewer,
          bodyHTML: '',
          body: '',
          isMinimized: false,
          state: 'PENDING',
    ...
    return (
      <CommentPositioningContainer
        ...
        commentThreads={aggregationResult.commentThreads.concat(uncreatedCommentThreads)}
        ...
        {commentTranslations => {
          return (
            <ReviewsController
              {...this.props}
              {...aggregationResult}
              uncreatedCommentThreads={uncreatedCommentThreads}
