...
export default class RemoteContainer extends React.Component {
  ...
  fetchToken(loginModel) {
    return loginModel.getToken(this.endpoint().getLoginAccount());
  }
  ...
  handleLogin(token) {
    ...
    this.props.loginModel.setToken(this.endpoint().getLoginAccount(), token);
  }

  handleLogout() {
    incrementCounter('github-logout');
    this.props.loginModel.removeToken(this.endpoint().getLoginAccount());
  }

  endpoint() {
    return this.props.remote.getEndpoint();
  }
}
