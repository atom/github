...
class PrInfo extends React.Component {
  ...
  render() {
    ...
    const prLink = (
      <a style={{color: 'gray'}} href={`https://github.com/${repo.owner.login}/${repo.name}/pull/${pr.number}`}>
        {repo.name}/{repo.owner.login}#{pr.number}
      </a>
    );
    return (
      <div style={{padding: '0 20px', maxWidth: 400, overflow: 'scroll'}}>
        <h3>{pr.title}<span style={{color: 'gray'}} /> {prLink}</h3>
