...
export class PrStatuses extends React.Component {
  ...
  static MAXIMUM_REFRESH_INTERVAL = 15 * 1000
  ...
  scheduleNextRefresh() {
    ...
    this.refreshTimer = setTimeout(() => {
      this.refresh();
      this.scheduleNextRefresh();
    }, this.isPendingResults() ? PrStatuses.PENDING_REFRESH_TIMEOUT : PrStatuses.SUCCESS_REFRESH_TIMEOUT);
  }
  ...
  @autobind
  refresh() {
    const lastRefresh = PrStatuses.lastRefreshPerPr.get(this.props.pullRequest.id);
    if (!lastRefresh || performance.now() - lastRefresh > PrStatuses.MAXIMUM_REFRESH_INTERVAL) {
      PrStatuses.lastRefreshPerPr.set(this.props.pullRequest.id, performance.now());
