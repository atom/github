...
export class PrSelectionByBranch extends React.Component {
  ...
  isOnDefaultBranchRef() {
    ...
    if (!currentBranch.isPresent()) { return false; }
    if (!currentBranch.getPush().isPresent()) { return false; }

    return currentBranch.getPush().getFullRef() === this.getDefaultBranchRef();
  }
  ...
  isSameAsMainBranch() {
    ...
    const mainBranches = this.props.branches.getPushSources(this.getDefaultBranchRef());
  ...
  hasUpstreamBranch() {
    ...
    return currentBranch.getUpstream().isPresent();
  }
}
