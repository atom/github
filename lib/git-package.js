...
export default class GitPackage {
  ...
  getFileListViewModel (): FileListViewModel {
    if (!this.fileListViewModel) {
      const gitStore = new GitStore(this.getGitService())
      this.subscriptions.add(gitStore.onDidChange(() => this.update()))

      this.fileListViewModel = new FileListViewModel(gitStore, this.getGitService())
    }
  ...
  getGitStore (): GitStore {
    return this.getFileListViewModel().getGitStore()
  }
