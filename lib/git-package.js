...
export default class GitPackage {
  ...
  async stageBufferRanges (ranges: Array<Range>, filePath: string): void {
    ...
    const linesToStage = fileDiff.getHunks()
      .map(hunk => hunk.getLines())
      .reduce((acc, lines) => acc.concat(lines), [])
      .filter(line => line.isChanged() && !line.isStaged() && anyRangesIntersectLine(ranges, line))
    await this.getGitStore().stageLines(linesToStage, true)
  }
