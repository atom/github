...
import FileCommentManager from './file-comment-manager'
import EditorCommentRenderer from './editor-comment-renderer'
...
export default class PullRequestCommentManager {
  ...
  addSubscriptions () {
    ...
    this.subscriptions.add(
      ...
      atom.workspace.onDidDestroyPaneItem(async ({item}) => {
        if (isEditor(item)) { this.destroyEditorCommentRenderer(item) }
      })
    )
  }
  ...
  onFetchedComments () {
    ...
    editorsByPath.forEach(async (editors, path) => {
      ...
      editors.forEach(editor => EditorCommentRenderer.renderCommentsForEditor(comments, editor))
    })
  ...
  async destroyEditorCommentRenderer (editor) {
    EditorCommentRenderer.destroyRendererForEditor(editor)
  }

  getFileCommentManager (path) {
    let manager = this.fileCommentManagersByPath.get(path)
    if (!manager) {
      manager = new FileCommentManager(path, this.commentEmitter)
