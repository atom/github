...
ipc.on(channelName, (event, {type, data}) => {
  if (type === 'git-exec') {
    ...
    event.sender.sendTo(managerWebContentsId, channelName, {type: 'exec-started', data: {id}});
...
ipc.sendTo(managerWebContentsId, channelName, {type: 'renderer-ready', data: {pid: process.pid}});
