...
export default function(controller, {confirm, notificationManager, workspace}) {
  ...
  commitPipeline.addMiddleware('failed-to-commit-error', async (next, repository) => {
    try {
      const result = await next();
      // TODO: modelStateRegistry.setStateForModel(repository, {amending: false, amendingCommitMessage: '', regularCommitMessage: ''})
