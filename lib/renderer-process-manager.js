...
export default class RendererProcessManager {
  ...
  onGitDataReceived(event, data) {
    ...
    resolve(data);
    this.resolveByOperationId.delete(data.operationId);
  }
...
class RendererProcess {
  constructor(hostWebContentsId) {
    ...
    this.rendererWebContentsId = this.win.webContents.id;
  ...
  onRendererReady(event, {webContentsId}) {
    if (webContentsId === this.rendererWebContentsId) {
      this.resolveReady();
    }
  }

  send(data) {
    this.win.webContents.send(`request-git-exec-${this.rendererWebContentsId}`, this.hostWebContentsId, data);
  }
